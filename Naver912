...
enum ENUM_MARKET_REGIME_DETAILED
{
  ...
  MARKET_REGIME_TRENDING_STRONG_DETAILED,
  MARKET_REGIME_TRENDING_WEAK_DETAILED,
  MARKET_REGIME_RANGING_CALM_DETAILED,
  MARKET_REGIME_RANGING_VOLATILE_DETAILED,
  MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED,
  MARKET_REGIME_HIGH_VOLATILITY_DETAILED,
  MARKET_REGIME_NEWS_EVENT_DETAILED
};
...
{
  double atr = GetATRValue(_Symbol, PERIOD_H1, ATR_Period_Global, 0);
  double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
  if(bid <= 0 || atr <= 0) return MARKET_REGIME_UNKNOWN_DETAILED;
  double volPercent = (atr / bid) * 100.0;
  if(IsNewsEventTime()) return MARKET_REGIME_NEWS_EVENT_DETAILED;
  if(volPercent > 0.2) return MARKET_REGIME_HIGH_VOLATILITY_DETAILED;
  if(volPercent > 0.08 && IsBreakoutPotential()) return MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED;
  if(volPercent <= 0.08 && volPercent > 0.02) return MARKET_REGIME_RANGING_CALM_DETAILED;
...
double GetEMAValue(string symbol, ENUM_TIMEFRAMES tf, int period, int shift = 0)
{
  ...
  if(handle == INVALID_HANDLE) { Print("GetEMAValue: can't create handle"); return 0.0; }
  double buf[1];
...
double GetATRValue(string symbol, ENUM_TIMEFRAMES tf, int period, int shift = 0)
{
  ...
  if(handle == INVALID_HANDLE) { Print("GetATRValue: can't create handle"); return 0.0; }
  double buf[1];
  if(CopyBuffer(handle, 0, shift, 1, buf) <= 0) { IndicatorRelease(handle); return 0.0; }
  IndicatorRelease(handle);
  return buf[0];
...
double GetATRAverage(string symbol, ENUM_TIMEFRAMES tf, int period, int bars)
{
  ...
  if(CopyBuffer(handle, 0, 0, bars, buf) <= 0) { IndicatorRelease(handle); return 0.0; }
  IndicatorRelease(handle);
  double sum = 0;
...
{
  ENUM_MARKET_REGIME_DETAILED r = DetectMarketRegime(); // if you have your own DetectMarketRegime, keep it
  return (r==MARKET_REGIME_RANGING_CALM_DETAILED || r==MARKET_REGIME_RANGING_VOLATILE_DETAILED);
}
bool IsBreakoutPotential()
{
  double atrNow = GetATRValue(_Symbol, PERIOD_H1, ATR_Period_Global, 0);
  double atrAvg = GetATRAverage(_Symbol, PERIOD_H1, ATR_Period_Global, 10);
  if(atrAvg <= 0) return false;
  return (atrNow > atrAvg * 1.2);
}
...
{
  if(currentRegime != MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED) return false;

  if(currentClose == 0.0) currentClose = SymbolInfoDouble(_Symbol, SYMBOL_BID);
  ...
  long prevVol= iVolume(_Symbol, _Period, 1);

  bool priceBreakout = (curHigh > prevHigh && currentClose > prevHigh) ||
                       (curLow  < prevLow  && currentClose < prevLow);

  bool volConfirm = (prevVol>0) ? (curVol > prevVol * 1.2) : false;

  double atrNow = GetATRValue(_Symbol, _Period, ATR_Period_Global, 0);
  double point  = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
  bool atrConfirm = (atrNow > point * 5);

  return priceBreakout && (volConfirm || atrConfirm);
}
void RecordBreakoutEvent()
{
  string fn = "breakout_log_" + Symbol() + ".txt";
  int h = FileOpen(fn, FILE_READ|FILE_WRITE|FILE_TXT|FILE_SHARE_READ);
  if(h==INVALID_HANDLE) { h = FileOpen(fn, FILE_WRITE|FILE_TXT); if(h==INVALID_HANDLE) return; }
  FileSeek(h,0,SEEK_END);
  FileWrite(h, TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS) + " - Breakout " + Symbol());
  FileClose(h);
}

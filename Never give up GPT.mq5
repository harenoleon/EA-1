...
input bool     UseWeeklyProfitTarget = true;
input double   WeeklyProfitTargetPercent = 10.0;
...
// --- Recovery System Inputs ---
input group "=== RECOVERY COOPERATION SYSTEM ==="
input double Recovery_StartLevel    = -200.0;
input double Recovery_StopLevel     = -50.0;
input bool   EnableRecoveryCoop     = true;
input bool   UseMarketAnalysis    = true;
input bool   AllowVolatileTrading = false;
input bool   TradeDuringNews      = false;
input double MaxLossForNewTrade = -50.0;
input int    MaxAllowedSpread     = 25;
input int    MaxRangeOrders       = 3;
input double MaxRangeDrawdown     = 20.0;
input bool   UseHedgeInVolatileMarket = true;
input double MaxDrawdown          = 100.0;
input int    PartialClosePct      = 50;
input int    MinTradeDistance         = 50;
input bool   CheckPreviousOrderLevel  = true;
input bool  EnableMarketAdaptiveRecovery = true;
...
input double localCounterSL = 50.0;
input double localCounterTP = 100.0;
...
// ประกาศเป็น input parameter ที่ต้นไฟล์:
input group "=== Range Trading Settings ===";
input bool IsRangeTradingEnabled = true; // Enable Range Trading Strategy
input double RangeTradingLotSize = 0.1;   // Lot size for range trading
input int RangeTradingMaxPositions = 2;   // Max simultaneous range positions
input int RangeTradingStopLossPips = 50;  // SL in pips for range trading
input int RangeTradingTakeProfitPips = 30; // TP in pips for range trading

input group "=== Adaptive Strategy Settings ===";
input bool AdaptiveStrategyEnabled = true;     // Enable Adaptive Strategy
input bool UseRegimeBasedTrading = true;       // Use market regime for trading decisions
input bool UseRegimeBasedRiskManagement = true; // Adjust risk based on regime
input bool UseRegimeBasedEntryExit = true;     // Adjust entry/exit based on regime

input group "=== MAGIC NUMBERS FOR RECOVERY SYSTEMS ==="
input int MAGIC_INDIVIDUAL_RECOVERY           = 8881;
input int MAGIC_STEPUP_RECOVERY               = 8882;
input int MAGIC_COUNTER_RECOVERY              = 8883;
input int MAGIC_HEDGE__RECOVERY               = 8884;
input int MAGIC_ORPHAN__RECOVERY              = 8885;
input int MAGIC_ULTIMATE__RECOVERY            = 8886;
...
bool     g_regimeChanged = false;
datetime g_lastRegimeCheck = 0;
int      g_regimeCheckInterval = 3600;
...
string   g_currentTrendStr = "NEUTRAL";
double   g_trendStrength = 0.0;
double   g_volatility = 0.0;
double   g_support = 0.0;
double   g_resistance = 0.0;
bool     g_isMarketOpen = true;
datetime g_lastBarTime = 0;     // เวลาแท่งเทียนล่าสุด
bool g_newBar = false;          // แท่งใหม่เกิดขึ้นหรือไม่
bool g_enableDebugMode = true;
double g_maxRiskTrendTrading = 3.0;
double g_maxRiskRangeTrading = 2.0;
double g_maxRiskScalpTrading = 1.5;
double g_EmergencyDrawdownThreshold = 20.0;
...
#define MARKET_REGIME_UNKNOWN       0
#define MARKET_REGIME_TRENDING      1
#define MARKET_REGIME_RANGING       2
#define MARKET_REGIME_BREAKOUT      3
#define MARKET_REGIME_VOLATILE      4
#define MARKET_REGIME_CALM          5
#define MARKET_REGIME_NORMAL        6
#define MARKET_REGIME_EXTREME       7
#define MARKET_REGIME_RECOVERY      8
#define MARKET_REGIME_DECLINE       9
#define MARKET_REGIME_SIDEWAYS      10
#define MARKET_REGIME_ACCUMULATION  11
#define MARKET_REGIME_DISTRIBUTION  12

// Trading signal structure
struct TradingSignal {
    int direction;      // 1 for buy, -1 for sell, 0 for no signal
...
#ifndef ENUM_TREND_DIRECTION_DEFINED
...
#define ENUM_TREND_DIRECTION_DEFINED
#endif
...
// Market regime enums
enum ENUM_MARKET_REGIME_DETAILED
{
    MARKET_REGIME_UNKNOWN_DETAILED = 0, 
    MARKET_REGIME_RANGING_CALM_DETAILED = 1,
    MARKET_REGIME_RANGING_VOLATILE_DETAILED = 2,
    MARKET_REGIME_TRENDING_WEAK_DETAILED = 3,
    MARKET_REGIME_TRENDING_STRONG_DETAILED = 4,
    MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED = 5,
    MARKET_REGIME_HIGH_VOLATILITY_DETAILED = 6,
    MARKET_REGIME_NEWS_EVENT_DETAILED = 7
};
ENUM_MARKET_REGIME_DETAILED g_regime;
ENUM_MARKET_REGIME_DETAILED g_newRegime;

enum ENUM_MARKET_REGIME_BASIC
{
    MARKET_REGIME_UNKNOWN_BASIC = 0,
    MARKET_REGIME_RANGING_CALM_BASIC = 1,
    MARKET_REGIME_RANGING_VOLATILE_BASIC = 2,
    MARKET_REGIME_TRENDING_BASIC = 3,
    MARKET_REGIME_BREAKOUT_POTENTIAL_BASIC = 4,
    MARKET_REGIME_HIGH_VOLATILITY_BASIC = 5,
    MARKET_REGIME_NEWS_EVENT_BASIC = 6
};

// --- Current System State ---
double MaxVolatilityForRange = 0.0020;    // 0.20%
double MinVolatilityForRange = 0.0005;    // 0.05%
//ENUM_MARKET_REGIME_DETAILED currentMarketRegime = MARKET_REGIME_RANGING_CALM_DETAILED;
ENUM_SYSTEM_MODE SystemMode = MODE_NORMAL;
ENUM_TREND_DIRECTION g_currentTrend = TREND_NONE;

ENUM_MARKET_REGIME_DETAILED g_currentRegime = MARKET_REGIME_RANGING_CALM_DETAILED;
...
#define VOLATILITY_THRESHOLD_LOW 0.002
#define VOLATILITY_THRESHOLD_HIGH 0.008
#define DIRECTION_UP 1
#define DIRECTION_DOWN -1
#define DIRECTION_NEUTRAL 0
#define MAGIC_RANGE 5000
#define MAGIC_RANGE_CALM 5001
#define MAGIC_RANGE_VOLATILE 5002
#define MAGIC_TREND 6000
#define MAGIC_TEST 9999
...
double MaxRiskForTrendTrading = 3.0;
double MaxRiskForRangeTrading = 2.0;
double MaxRiskForScalpTrading = 1.5;
double MaxRiskForBreakoutTrading = 4.0;

bool IsBreakoutTradingEnabled = true;
bool IsTrendTradingEnabled = true;
bool IsScalpTradingEnabled = true;
...
int ema9_handle_m5, ema21_handle_m5;
int rsi_handle_h1, stoch_handle_h1;
...
// --- Independent Strategies Variables ---
...
void UpdatePortfolioHealth() 
{
   ...
   if(TotalPortfolioProfit <= -Recovery_StartLevel && SystemMode != MODE_RECOVERY) 
   {
      ...
      Print("🆘 SYSTEM ENTERING RECOVERY MODE - Portfolio: $" + DoubleToString(TotalPortfolioProfit, 2));
      
      if(cooperativeSystemInitialized) 
      {
         recoveryModeActive = true;
         Print("🤝 NOTIFYING COOPERATIVE SYSTEM - Recovery Mode Activated");
      }
   } 
   // ออกจากโหมด Recovery
   else if(TotalPortfolioProfit >= Recovery_StopLevel && SystemMode == MODE_RECOVERY) 
   {
      ...
      Print("✅ SYSTEM RETURNING TO NORMAL MODE - Portfolio: $" + DoubleToString(TotalPortfolioProfit, 2));
      
      if(cooperativeSystemInitialized) 
      {
         recoveryModeActive = false;
         Print("🤝 NOTIFYING COOPERATIVE SYSTEM - Recovery Mode Ended");
      }
   }
}
...
bool IsTrendScalpAllowedInRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED: // บางทีอาจอนุญาตใน ranging ที่มี volatility
            return true;
            
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            return false; // หยุดเทรดในตลาดเสี่ยง
            
        default:
            return true; // อนุญาตในสภาวะอื่นๆ
    }
}
double GetTrendScalpLotMultiplierForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 1.2; // เพิ่ม lot ใน trending strong
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            return 1.0; // ปกติ
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 0.7; // ลด lot ใน ranging volatile
            
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
            return 0.3; // ลด lot มากใน high volatility
...
//+------------------------------------------------------------------+
void ExecuteSimpleTrendScalp(ENUM_MARKET_REGIME_DETAILED regime = MARKET_REGIME_UNKNOWN_DETAILED)
{
    // ตรวจจับ regime ถ้าไม่ได้ส่งมา
    if(regime == MARKET_REGIME_UNKNOWN_DETAILED) 
    {
    ...
    // ตรวจสอบว่า Trend Scalp ควรทำงานใน regime นี้หรือไม่
    if(!IsTrendScalpAllowedInRegime(regime)) 
    {
        DebugPrint(2, "Trend Scalp not allowed in " + RegimeToString(regime) + " regime");
        return;
    }
    ...
    if(EnableDebugMode)
    {
        Print("=== 🎯 SIMPLE TREND SCALP ===");
        Print("Market Regime: " + RegimeToString(regime));
    }
    
    // 1. ตรวจสอบจำนวนออเดอร์ปัจจุบัน
    int trendScalpCount = CountPositionsByMagic(MAGIC_TREND_SCALP);
    
    // ปรับ max positions ตาม regime
    int allowedPositions = GetTrendScalpMaxPositionsForRegime(regime);
    if(trendScalpCount >= allowedPositions) 
    {
        DebugPrint(3, "Max Trend Scalp positions reached for " + RegimeToString(regime) + 
                  ": " + IntegerToString(trendScalpCount) + "/" + IntegerToString(allowedPositions));
        return;
    }

    // 2. วิเคราะห์เทรนด์หลัก (ใช้หลาย timeframe)
    ENUM_TREND_DIRECTION mainTrend = AnalyzeMultiTimeframeTrend();
    double trendStrength = CalculateTrendStrength();
    
    // ✅ แก้ไขตรงนี้: เพิ่มตัวแปร isStrongTrend
    bool isStrongTrend = (trendStrength > 7.0); // หรือใช้ threshold อื่น
    ...
    if(EnableDebugMode)
    {
        ...
        Print("4. Is Strong Trend: ", isStrongTrend);
    }

    // 3. ตรวจสอบเงื่อนไขเทรดตามเทรนด์และ regime
    if(ShouldTradeBasedOnTrend(mainTrend, trendStrength, regime))
    {
        // ปรับ lot size ตาม regime และ trend strength
        ...
        // ✅ แก้ไขตรงนี้: ประกาศตัวแปรก่อนใช้
        double riskMultiplier = isStrongTrend ? 1.0 : 0.7;
        double tpMultiplier = isStrongTrend ? 3.0 : 2.0;
        double slMultiplier = isStrongTrend ? 1.0 : 1.5;
        
        if(mainTrend == TREND_UP)
        {
            ExecuteTrendBuyWithMultipliers(riskMultiplier, tpMultiplier, slMultiplier);
        }
        else if(mainTrend == TREND_DOWN)
        {
            ExecuteTrendSellWithMultipliers(riskMultiplier, tpMultiplier, slMultiplier);
        }
    }
    else
    {
        DebugPrint(3, "No clear trend signal for " + RegimeToString(regime) + " - waiting");
    }
}

int GetTrendScalpMaxPositionsForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    ...
    switch(regime)
    {
        ...
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            return MathMin(baseMax + 1, 6);
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            return MathMax(1, (int)(baseMax * 0.7));
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return MathMax(1, (int)(baseMax * 0.5));
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return MathMax(1, (int)(baseMax * 0.6));
            
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
            return MathMax(1, (int)(baseMax * 0.3));
            
        default:
            return baseMax;
    }
}
int GetTrendScalpMaxPositionsWithStrength(ENUM_MARKET_REGIME_DETAILED regime)
{
    int baseMax = Scalp1_MaxPositions; // ตรวจสอบว่ามีตัวแปรนี้จริง
    
    double normalizedStrength = GetTrendStrengthNormalized();
    ...
    switch(regime)
    {
        ...
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            if(normalizedStrength == 1.0) {
                return MathMin(baseMax + 2, 8); // Strong trend
            } else if(normalizedStrength == 0.5) {
                return MathMin(baseMax + 1, 6); // Moderate trend
            } else {
                return baseMax; // Weak or neutral
            }
        ...
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            return MathMax(1, (int)(baseMax * 0.2));
            
        default:
            return baseMax;
    }
}
// 🔥 ฟังก์ชันสำหรับตรวจสอบว่าอยู่ใน uptrend หรือไม่
bool IsInUptrend()
{
    double strength = GetTrendStrength(DIRECTION_UP);
    return (strength > 0);
}

// 🔥 ฟังก์ชันสำหรับตรวจสอบว่าอยู่ใน downtrend หรือไม่
bool IsInDowntrend()
{
    double strength = GetTrendStrength(DIRECTION_DOWN);
    return (strength > 0);
}
bool IsNewsEventTime()
{
    ...
    // ตัวอย่าง: หลีกเลี่ยงช่วง Non-Farm Payroll (วันศุกร์แรกของเดือน)
    if(currentTime.day_of_week == 5) // วันศุกร์
    {
        // ตรวจสอบว่าเป็นวันศุกร์แรกของเดือนหรือไม่
        if(currentTime.day <= 7)
        {
            // เวลา 20:30 GMT (อาจปรับตาม broker time)
            if(currentTime.hour == 20 && currentTime.min >= 30)
            {
                return true;
            }
            if(currentTime.hour == 21 && currentTime.min <= 30)
            {
                return true;
            }
        }
    ...
    // หลีกเลี่ยงช่วงอื่นๆ
    // 1. FOMC Meetings (ประมาณทุก 6 สัปดาห์)
    // 2. ECB Meetings
    // 3. CPI Data Releases
    
    // สามารถเพิ่ม calendar API จริงได้ในอนาคต
    
    return false;
}
// 🔥 ฟังก์ชันสำหรับตรวจสอบว่าตลาดไม่มี trend
bool IsMarketNeutral()
{
    return (GetTrendDirection() == DIRECTION_NEUTRAL);
}
// Function declaration (ถ้าต้องการ)
bool UseBreakoutStrategy();

// Function implementation
bool UseBreakoutStrategy()
{
    // ตรรกะการตัดสินใจใช้ Breakout Strategy
    ENUM_MARKET_REGIME_DETAILED currentRegime = g_currentRegime;
    double volatility = CalculateVolatility();
    double atrValue = GetATRValue(_Symbol, _Period, 14, 0);
    
    // เงื่อนไขการใช้ Breakout Strategy
    // 1. ตลาดอยู่ในสถานะที่เหมาะสำหรับ breakout
    if(currentRegime == MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED)
    {
        return true;
    }
    
    // 2. Volatility สูงและมีแนวโน้ม breakout
    if(volatility > 100 && atrValue > SymbolInfoDouble(_Symbol, SYMBOL_POINT) * 50)
    {
        // ตรวจสอบว่าราคาอยู่ใกล้ระดับสำคัญ
        if(IsNearKeyLevel())
        {
            return true;
        }
    ...
    // 3. ตลาดผันผวนต่ำและคาดว่าจะมี breakout
    if(volatility < 50 && IsCompressionPattern())
    {
        return true;
    }
    
    // 4. ตามเวลา session (เช่น session overlap)
    if(IsHighProbabilitySession())
    {
        return true;
    }
...
bool IsNearKeyLevel()
{
    ...
    // ค้นหาระดับสำคัญ (Support/Resistance)
    double resistance = iHigh(_Symbol, _Period, iHighest(_Symbol, _Period, MODE_HIGH, 20, 0));
    double support = iLow(_Symbol, _Period, iLowest(_Symbol, _Period, MODE_LOW, 20, 0));
    
    double pivot = (iHigh(_Symbol, PERIOD_D1, 0) + iLow(_Symbol, PERIOD_D1, 0) + iClose(_Symbol, PERIOD_D1, 0)) / 3;
    
    // ตรวจสอบว่าราคาอยู่ใกล้ระดับใดระดับหนึ่งภายใน 2x ATR
    return (MathAbs(currentPrice - resistance) < atrValue * 2 ||
            MathAbs(currentPrice - support) < atrValue * 2 ||
            MathAbs(currentPrice - pivot) < atrValue * 2);
}

bool IsCompressionPattern()
{
    // ตรวจสอบว่าราคากำลังบีบตัว (เช่น Bollinger Bands squeeze)
    ...
    CopyBuffer(bbHandle, 1, 0, 1, upperBand); // Upper
    CopyBuffer(bbHandle, 2, 0, 1, lowerBand); // Lower
    
    double bandWidth = upperBand[0] - lowerBand[0];
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    
    // Bollinger Bands squeeze: bandwidth แคบมาก
    return (bandWidth < currentPrice * 0.005); // 0.5%
}

bool IsHighProbabilitySession()
{
    // Session ที่มีโอกาส breakout สูง
    MqlDateTime dt;
    TimeCurrent(dt);
    ...
    // Asian Session Break (00:00-02:00 GMT)
...
ENUM_MARKET_REGIME_DETAILED DetectMarketRegime()
{
    // 1. คำนวณ volatility
    ...
    if(atrHandle == INVALID_HANDLE) 
        return MARKET_REGIME_UNKNOWN_DETAILED;
    ...
    // คำนวณ volatility percentage (ATR เป็นจุด, แปลงเป็นเปอร์เซ็นต์)
    double volatilityPercent = (atrValue / currentPrice) * 10000;
    
    // อัพเดท global variable (ถ้ามี)
    // currentVolatility = volatilityPercent;
    ...
    if(trendDirection == DIRECTION_UP) {
        trendStrength = GetTrendStrength(DIRECTION_UP);
    } else if(trendDirection == DIRECTION_DOWN) {
        trendStrength = GetTrendStrength(DIRECTION_DOWN);
    }
    
    // แปลงเป็น normalized strength (0-1)
    double normalizedStrength = 0;
    if(trendStrength == 30.0) 
        normalizedStrength = 1.0;
    else if(trendStrength == 20.0) 
        normalizedStrength = 0.5;
    else if(trendStrength > 0) 
        normalizedStrength = trendStrength / 30.0; // Normalize
    
    // 3. ตรวจสอบเงื่อนไขต่างๆ
    
    // 3.1 High Volatility
    if(volatilityPercent > 200) {
        return MARKET_REGIME_HIGH_VOLATILITY_DETAILED; // ✅ แก้ไข: เพิ่ม _DETAILED
    }
    
    // 3.2 Volatile Ranging
    else if(volatilityPercent > 100 && volatilityPercent <= 150) {
        return MARKET_REGIME_RANGING_VOLATILE_DETAILED;
    }
    
    // 3.3 Trending Market
    else if(normalizedStrength >= 0.3) {
        if(normalizedStrength > 0.7) {
            return MARKET_REGIME_TRENDING_STRONG_DETAILED; // ✅ แก้ไข: เพิ่ม _DETAILED
        } else {
            return MARKET_REGIME_TRENDING_WEAK_DETAILED; // ✅ แก้ไข: เพิ่ม _DETAILED
        }
    }
    
    // 3.4 Calm Ranging
    else if(volatilityPercent <= 80) {
        return MARKET_REGIME_RANGING_CALM_DETAILED;
    }
    
    // 3.5 Breakout Potential
    else if(volatilityPercent > 80 && volatilityPercent <= 100) {
        // ตรวจสอบเพิ่มเติมสำหรับ breakout
        if(IsBreakoutPotential()) {
            return MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED; // ✅ แก้ไข: เพิ่ม _DETAILED
        } else {
            return MARKET_REGIME_RANGING_VOLATILE_DETAILED;
        }
    }
    
    // 3.6 Check for News Events
    if(IsNewsEventTime()) {
        return MARKET_REGIME_NEWS_EVENT_DETAILED; // ✅ แก้ไข: เพิ่ม _DETAILED
    }
    
    return MARKET_REGIME_UNKNOWN_DETAILED; // ✅ แก้ไข: เพิ่ม _DETAILED
}

ENUM_TREND_DIRECTION ConvertToTrendDirection(int direction)
{
    // ตรวจสอบว่าค่า int ตรงกับ enum values หรือไม่
    if(direction == 0) return TREND_NONE;
    if(direction == 1) return TREND_UP;
    if(direction == -1) return TREND_DOWN;
    
    // ถ้าไม่ตรง ให้ใช้ TREND_NONE
    Print("Warning: Invalid trend direction value: ", direction);
    return TREND_NONE;
}
void ExecuteStrategyByRegime(ENUM_MARKET_REGIME_DETAILED regime, int trendDirection)
{
    switch(regime)  // ❌ ผิด: switch(regime) → ✅ ถูก: switch(regime)
    {
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            UseRangeStrategy(MARKET_REGIME_RANGING_CALM_DETAILED);
            break;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            if(ShouldTradeInVolatileMarket(MARKET_REGIME_RANGING_VOLATILE_DETAILED)) {
         ...
         case MARKET_REGIME_TRENDING_STRONG_DETAILED:
         {   // ✅ ถูกต้อง: มี {} ครอบ
             ENUM_TREND_DIRECTION trendDir = (ENUM_TREND_DIRECTION)trendDirection;
             bool isStrong = (regime == MARKET_REGIME_TRENDING_STRONG_DETAILED);
             UseTrendStrategyWithDirection(trendDir, isStrong);
             break;
         }
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            if(IsBreakoutConfirmed(MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED)) {
                UseBreakoutStrategy();
            }
            break;
            
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
            Print("⚠️ High volatility - Trading paused");
            break;
            
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            Print("📰 News event - Trading paused");
            break;
            
        default:
            Print("❓ Unknown market regime: ", RegimeToString(regime));
            break;
    }
}
void UseTrendStrategyWithDirection(ENUM_TREND_DIRECTION direction, bool isStrongTrend)
{
    // ปรับ parameters ตาม trend strength
    double riskMultiplier = isStrongTrend ? 1.0 : 0.7;
    double tpMultiplier = isStrongTrend ? 3.0 : 2.0;
    double slMultiplier = isStrongTrend ? 1.0 : 1.5;
    ...
    if(EnableDebugMode)
    {
        Print("Executing ", (isStrongTrend ? "STRONG" : "WEAK"), 
              " trend strategy in ", 
              (direction == TREND_UP ? "UP" : "DOWN"), " direction");
    }
}
int GetTrendScalpMaxPositionsForRegimeDetailed(ENUM_MARKET_REGIME_DETAILED regime)
{
    int baseMax = Scalp1_MaxPositions;
    
    switch(regime)  // ✅ ถูกต้อง: ใช้แค่ชื่อตัวแปร
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // Trending strong - สามารถเปิด position ได้มากขึ้น
            return MathMin(baseMax * 2, 10); // สูงสุด 10 positions
        
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            // Trending weak - เปิดปานกลาง
            return MathMin(baseMax, 5); // สูงสุด 5 positions
        
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            // Ranging calm - เปิดน้อยลง
            return MathMin(baseMax / 2, 3); // สูงสุด 3 positions
        
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // Ranging volatile - ระมัดระวัง
            return MathMin(baseMax / 3, 2); // สูงสุด 2 positions
        
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
            // High volatility - เปิดน้อยมากหรือไม่เปิด
            return MathMin(baseMax / 4, 1); // สูงสุด 1 position
        
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            // Breakout potential - พร้อมเปิดเมื่อ breakout เกิดขึ้น
            return MathMin(baseMax, 4); // สูงสุด 4 positions
        
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            // News event - หลีกเลี่ยงการเปิด position ใหม่
            return 0; // ไม่เปิด position ใหม่
        
        case MARKET_REGIME_UNKNOWN_DETAILED:
        default:
            // Unknown or other regimes - ใช้ค่า default
            return MathMin(baseMax, 3); // สูงสุด 3 positions
    }
}

bool ShouldTradeBasedOnTrend(ENUM_TREND_DIRECTION trend, double strength, ENUM_MARKET_REGIME_DETAILED regime)
{
    // ตรวจสอบชื่อตัวแปรที่ถูกต้อง
    if(trend == TREND_NONE) 
        return false;
    
    // แก้ไข syntax error: ลบ ENUM_MARKET_REGIME_DETAILED ออก
    ...
    // ตรวจสอบเงื่อนไขเพิ่มเติมตาม regime
    switch(regime)  // ✅ ใช้แค่ชื่อตัวแปร ไม่ต้องใส่ type
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // ในเทรนด์แรง ต้องการ confirmation น้อยกว่า
            return true;
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            // ในเทรนด์อ่อน ต้องการ confirmation มากขึ้น
            return HasMultipleTrendConfirmations(trend);
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            // ใน sideways เงียบ ควรหลีกเลี่ยง trend trade
            return false;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // ในตลาดผันผวน ต้องการ strict confirmation
            return IsTrendValidInVolatileMarket(trend);
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            // ใน breakout ต้องแน่ใจว่าไม่ใช่ false breakout
...
double GetRequiredTrendStrengthForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)  // ✅ ถูก: ใช้แค่ชื่อตัวแปร ไม่ต้องมี type
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 0.6; // ความแรงขั้นต่ำ 60%
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            return 0.4; // ความแรงขั้นต่ำ 40%
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 0.7; // ต้องการความแรงมากในตลาดผันผวน
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return 0.5; // ปานกลาง
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            return 0.8; // ต้องการความแรงมากใน sideways เงียบ
            
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
            return 0.9; // ต้องการความแรงมากใน high volatility
            
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            return 0.9; // ต้องการความแรงมากในช่วงข่าว
...
// 🔥 แก้ไขฟังก์ชัน ExecuteTrendBuy และ ExecuteTrendSell
void ExecuteTrendBuyWithMultipliers(double riskMultiplier, double tpMultiplier, double slMultiplier)
{
    // คำนวณ lot size
    double baseLot = 0.01; // หรือใช้ค่าจาก input
    double lot = baseLot * riskMultiplier;
    
    // ใช้ฟังก์ชัน ExecuteTrendBuy ที่มีอยู่ (รับแค่ lot)
    ExecuteTrendBuy(lot);
    
    Print("Trend Buy with multipliers - Risk: ", riskMultiplier, 
          ", TP: ", tpMultiplier, ", SL: ", slMultiplier);
}

void ExecuteTrendSellWithMultipliers(double riskMultiplier, double tpMultiplier, double slMultiplier)
{
    // คำนวณ lot size
    double baseLot = 0.01; // หรือใช้ค่าจาก input
    double lot = baseLot * riskMultiplier;
    
    // ใช้ฟังก์ชัน ExecuteTrendSell ที่มีอยู่ (รับแค่ lot)
    ExecuteTrendSell(lot);
    
    Print("Trend Sell with multipliers - Risk: ", riskMultiplier, 
          ", TP: ", tpMultiplier, ", SL: ", slMultiplier);
}

double GetTrendScalpTPMultiplierForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 2.0; // TP ยาวในเทรนด์แรง
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 0.5; // TP สั้นในตลาดผันผวน
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return 1.8; // TP ยาวใน breakout
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            return 0.8; // TP สั้นใน sideways เงียบ
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            return 1.2; // TP ยาวขึ้นเล็กน้อย
...
double GetTrendScalpSLMultiplierForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 1.5; // SL กว้างในเทรนด์แรง
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 0.7; // SL แคบในตลาดผันผวน
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return 1.3; // SL กว้างใน breakout
            
        default:
            return 1.0;
    }
}
bool IsMAAlignmentConfirming(int trend)  // หรือ double trend ตามที่คุณใช้
{
    // กำหนดค่าคงที่
...
bool IsMACDConfirming(int trend)
{
    ...
    if(trend == TREND_UP)
    {
        // MACD bullish: MACD > Signal และ MACD > 0
        return (macd > signal && macd > 0);
    }
    else if(trend == TREND_DOWN)
    {
        // MACD bearish: MACD < Signal และ MACD < 0
        return (macd < signal && macd < 0);
    }
...
bool IsRSIConfirming(int trend)
{
    ...
    if(trend == TREND_UP)
    {
        // RSI bullish: RSI > 50 และไม่ overbought
        return (rsiValue > 50 && rsiValue < 70);
    }
    else if(trend == TREND_DOWN)
    {
        // RSI bearish: RSI < 50 และไม่ oversold
        return (rsiValue < 50 && rsiValue > 30);
    }
...
bool IsPriceActionConfirming(int trend)
{
    ...
    {
        // ลอง timeframe ที่เล็กกว่า
        if(CopyRates(_Symbol, PERIOD_M15, 0, 3, rates) < 3) return false;
    }
    
    if(trend == TREND_UP)
    {
        // ตรวจสอบ higher highs หรือ higher lows
    ...
    {
        // ตรวจสอบ lower highs หรือ lower lows
...
bool IsMomentumConfirming(int trend)
{
    ...
    if(trend == TREND_UP)
    {
        // Stochastic bullish: main > signal และไม่อยู่ใน overbought (>80)
        return (stochMain > stochSignal && stochMain < 80);
    }
    else if(trend == TREND_DOWN)
    {
        // Stochastic bearish: main < signal และไม่อยู่ใน oversold (<20)
        return (stochMain < stochSignal && stochMain > 20);
    }
    
    return false;
}
// 🔥 ฟังก์ชันเพิ่มเติมสำหรับ trend confirmation
...
bool IsTrendConfirmedAfterBreakout(int trend)  // trend: 1=UP, 2=DOWN
{
    ...
    // ตรวจสอบว่า trend เกิดขึ้นหลัง breakout จริง
    return IsPriceAboveKeyLevel(trend) && HasVolumeConfirmation();
}
// สร้างฟังก์ชัน HasVolumeConfirmation (แบบง่าย)
bool HasVolumeConfirmation()
{
    // ใน MQL5 อาจใช้ tick volume หรือแท้จริงแล้วควรใช้ OnBalanceVolume, Money Flow Index
    MqlRates rates[2];
    if(CopyRates(_Symbol, PERIOD_H1, 0, 2, rates) < 2) return false;
    
    // ตรวจสอบ volume ของแท่งปัจจุบันมากกว่าแท่งก่อนหน้า
    // ใน MQL5, rates[].tick_volume คือ tick volume
    if(rates[0].tick_volume > rates[1].tick_volume * 1.2)
    {
        return true; // Volume เพิ่มขึ้นอย่างมีนัยสำคัญ
    }
    
    // หรือใช้ค่าเฉลี่ย volume
...
double GetAverageVolume(int bars)
{
    ...
    {
        total += (double)rates[i].tick_volume;  // แปลงจาก long เป็น double
    }
    
    return total / bars;
}
// ฟังก์ชัน GetEMA สำหรับ MQL5
...
// สร้างฟังก์ชัน IsPriceAboveKeyLevel
...
// 3. ใน GetTrendFromSingleTimeframe()
int GetTrendFromSingleTimeframe(ENUM_TIMEFRAMES tf)
{
    ...
    // ตรวจสอบ EMA alignment และ strength
    ...
    // ต้องการความแตกต่างอย่างน้อย 0.4 * ATR
    if(diffInATR < 0.4) 
    {
        return TREND_NONE; // Trend ไมแรงพอ
    }
    
    // ตรวจสอบ price position
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    
    if(bullishAlignment)
    {
        // สำหรับ uptrend ราคาควรอยู่เหนือ EMA ทั้งสอง
    ...
    {
        // สำหรับ downtrend ราคาควรอยู่ใต้ EMA ทั้งสอง
        if(currentPrice < ema9 && currentPrice < ema21)
        {
            return TREND_DOWN;
        }
...
double GetKeyLevelForBreakout(int trend)
{
    ...
    // หา recent high/low เป็น key level
    MqlRates rates[20];
    if(CopyRates(_Symbol, PERIOD_H1, 0, 20, rates) < 20)
    {
        // ถ้าไม่สำเร็จ ใช้ค่าเริ่มต้น
    ...
    if(trend == TREND_UP)
    {
        // สำหรับ breakout up ใช้ recent high
    ...
    {
        // สำหรับ breakout down ใช้ recent low
...
//+------------------------------------------------------------------+
double CalculateTotalProfit(int specificMagic = -1)
{
    double totalProfit = 0;
    
    long magicList[] = {
        MAGIC_TEST_SIMPLE, MAGIC_TREND_SCALP,
        MAGIC_SCALP1, MAGIC_SCALP1_HF, MAGIC_SCALP2, MAGIC_SCALP3,
        MAGIC_RECOVERY, MAGIC_INDIVIDUAL, MAGIC_ORPHAN,
        MAGIC_HEDGE, MAGIC_AI_NEWS, MAGIC_AI_TREND,
        MAGIC_BREAKOUT, MAGIC_COUNTER, MAGIC_MAIN
    };
    ...
    for(int i = 0; i < PositionsTotal(); i++) {
        ...
        if(symbol == _Symbol) {
            ...
            if(specificMagic == -1) {
                for(int m = 0; m < ArraySize(magicList); m++) {
                    if(magic == (ulong)magicList[m]) {
                        magicMatch = true;
                        break;
                    }
                }
            } else {
                magicMatch = (magic == (ulong)specificMagic);
            }
...
bool ShouldCloseIndividualRecovery(ulong recoveryTicket)
{
    ...
    // ปิดเมื่อกำไรถึง MinSetTP (ปลอดภัย)
...
void NotifyUniversalOrphan(ulong ticket, string strategyName = "Unknown", long originalMagic = 0)
{
    ...
    // Legacy logging
    Print("🆘 LEGACY ORPHAN NOTIFICATION: Ticket #" + TicketToStr(ticket) + 
          " | Strategy: " + strategyName);
}
// +------------------------------------------------------------------+
//| Notify Orphan Job System (แทน NotifyUniversalOrphan)            |
//+------------------------------------------------------------------+
void NotifyOrphanJobSystem(ulong ticket, string strategyName = "Unknown", long originalMagic = 0)
{
    if(!PositionSelectByTicket(ticket)) return;
    
    // ตรวจสอบว่า ticket นี้เป็น Orphan ที่ควรสร้าง Job หรือไม่
    ...
    {
        // พบ Orphan ที่ Main หาย → แจ้งระบบ Job
        Print("🆘 ORPHAN DETECTED | Ticket: #" + TicketToStr(ticket) + 
              " | Main: #" + TicketToStr(mainTicket) +
              " | Strategy: " + strategyName);
        
        // ระบบ Orphan Universal จะจัดการสร้าง Job อัตโนมัติ
        // ไม่ต้องสร้าง Job ที่นี่ เพราะ ManageOrphanUniversal() จะตรวจสอบเอง
    }
    else
    {
...
void CheckAndNotifyRecoverySetOrphans()
{
    for(int i = 0; i < totalRecoverySets; i++)
    {
        ...
        // ตรวจสอบว่า Main หายไป
        if(!PositionSelectByTicket(recoverySets[i].mainTicket))       
        {
            Print("🔔 RECOVERY SET ORPHAN DETECTED: Set #" + IntegerToString(recoverySets[i].setNumber) + 
                  " | Main #" + TicketToStr(recoverySets[i].mainTicket) + " is missing!");
            
            // ไม่ต้องเรียก NotifyUniversalOrphan อีก
            // ระบบ Orphan Universal จะตรวจสอบและสร้าง Job อัตโนมัติ
        }
    }
}
// +------------------------------------------------------------------+
//| Register Orphan Tickets From Set (แบบใหม่)                     |
//+------------------------------------------------------------------+
void RegisterOrphanTicketsFromSet(int setIndex)
{
    ...
    Print("📋 Set #" + IntegerToString(recoverySets[setIndex].setNumber) + 
          " has " + IntegerToString(registeredCount) + " orphan tickets");
    
    // Mark set as completed เพราะ Main หาย
...
void CheckOtherStrategiesForOrphans()
{
    for(int i = PositionsTotal()-1; i >= 0; i--)
    {
        ...
        if(PositionSelectByTicket(ticket))
        {
            ...
            // ตรวจสอบว่าเป็น Recovery Position ที่ Main หาย
               ...
               StringFind(comment, "Recovery-") != -1)
            {
                ...
                if(mainTicket > 0 && !PositionSelectByTicket(mainTicket))
                {
                    // พบ Orphan → ระบบ Orphan Universal จะจัดการเอง
...
void CheckForMissingPair(ulong ticket, long magic, string comment)
{
    ...
    if(mainTicketStr != "" && mainTicketStr != "0")
    {
        ...
        {
            // ✅ ใช้ระบบใหม่: บันทึก Log แทนการเรียก NotifyUniversalOrphan
            Print("🔔 MISSING PAIR DETECTED | Helper: #" + TicketToStr(ticket) + 
                  " | Main: #" + mainTicketStr + " missing" +
                  " | Type: " + GetMagicStrategyName(magic));
            
            // ✅ ระบบ Orphan Universal จะตรวจสอบและสร้าง Job อัตโนมัติ
            // ไม่ต้องทำอะไรเพิ่ม - ManageOrphanUniversal() จะจัดการเอง
        }
    }
}
...
//+------------------------------------------------------------------+
// +------------------------------------------------------------------+
//| Calculate Safe Orphan Lot (สำหรับระบบ Orphan Job ใหม่)         |
//+------------------------------------------------------------------+
double CalculateSafeOrphanLot(ulong orphanTicket, int recoveryCount = 0)
{
    if(!PositionSelectByTicket(orphanTicket)) 
    {
        Print("❌ CalculateSafeOrphanLot: Ticket not found #" + TicketToStr(orphanTicket));
        return BaseLot;
    }
    ...
    // ✅ ปรับ multiplier ตามจำนวน recovery ที่เคยทำ
    if(recoveryCount == 0)
        baseMultiplier *= 1.0;
    ...
    Print("🧮 SAFE ORPHAN LOT CALC | Ticket: #" + TicketToStr(orphanTicket) +
...
{
   double base_scalp_lot = BaseLot * Current_Scalp1_Lot_Multiplier;  // ✅ ใช้ตัวแปรใหม่
...
double CalculateSetProfit(int setIndex)
{
    ...
    else
    {
        ...
        for(int i = 0; i < recoverySets[setIndex].subCount; i++)
        {
            ...
            if(PositionSelectByTicket(subTicket))
            {
                // ✅ Use the new Orphan Job System instead of NotifyUniversalOrphan
...
void ManageSetsWithMain()
{
    for(int i = 0; i < totalRecoverySets; i++)
    {
        ...
        {
            Print("✅ Closing complete set - Recovery covers main loss + profit");
            CloseCompleteSet(i);
...
void ManageOrphanedRecoverySets()
{
    ...
    {
        if(recoverySets[i].isCompleted) continue;
        if(!IsMainTicketMissing(i)) continue; // ข้ามกรณี Main อยู่
        ...
        // คำนวณกำไร Recovery รวมปัจจุบัน
        ...
        // ✅ เงื่อนไขปิด: Recovery รวมต้องครอบคลุม Main Loss
        if(currentRecoveryTotal >= mainLossBeforeMissing + MinSetTP)
        {
            // ปิด Recovery Orders ทีละตัว
            for(int j = 0; j < recoverySets[i].subCount; j++)
            {
                ulong subTicket = recoverySets[i].subTickets[j];
                if(PositionSelectByTicket(subTicket))
                {
                    ...
                    Print("✅ Closed orphan recovery: #", TicketToStr(subTicket));
                }
            }
            recoverySets[i].isCompleted = true;
            Print("📦 Orphan set completed - Recoveries covered main loss");
        }
        else
        {
            // ✅ ตรวจสอบแต่ละ Recovery ว่าต้องการความช่วยเหลือไหม
            for(int j = 0; j < recoverySets[i].subCount; j++)
            {
                ulong subTicket = recoverySets[i].subTickets[j];
                if(PositionSelectByTicket(subTicket))
                {
                    ...
                    // ขาดทุนถึง Trigger → ส่งให้ Orphan System
                    if(profit <= -localOrphanTrigger)
                    {
                        Print("🆘 Orphan recovery needs help: #", TicketToStr(subTicket), " | Loss: $", profit);
...
//+------------------------------------------------------------------+
//| OpenSingleRecoveryOrder - เปิดออร์เดอร์ Recovery แต่ละตัว      |
//+------------------------------------------------------------------+
bool OpenSingleRecoveryOrder(ulong mainTicket)
{
    ...
    // ✅ ตรวจสอบว่า Main ยังต้องการ Recovery หรือไม่
    ...
    // ✅ หา Set และนับจำนวน Recovery ที่มีอยู่
    int setIndex = FindSetByMainTicket(mainTicket);
    int recoveryCount = (setIndex != -1) ? recoverySets[setIndex].subCount : 0;
    
    // ✅ ตรวจสอบจำนวน Recovery สูงสุด
    if(recoveryCount >= localMaxRecovery)
    {
    ...
    if(success)
    {
        ...
        // ✅ เพิ่ม Recovery ลงใน Set
        if(setIndex != -1)
        {
            AddSubOrderToSet(recoverySets[setIndex].setNumber, recoveryTicket);
            
            Print("✅ SINGLE INDIVIDUAL RECOVERY | Set #" + IntegerToString(recoverySets[setIndex].setNumber) +
                  " | Main: #" + TicketToStr(mainTicket) +
    ...
    {
        Print("❌ Failed to open individual recovery: " + trade.ResultRetcodeDescription());
    }
...
{
   // ✅ ตรวจสอบว่าครบจำนวนต่อรอบแล้วหรือยัง
   if(currentIndividualProcessCount >= Individual_MaxPerCycle)
   {
      // ✅ โหมดรอปิด Set ก่อนเริ่มรอบใหม่
      if(Individual_WaitForClose)
      {
         // ตรวจสอบว่ามี Set ที่ยังไม่ปิดหรือไม่
         ...
         if(hasActiveSets)
         {
            DebugPrint(2, "⏳ Waiting for active sets to close before next cycle...");
            return; // รอจนกว่า Set จะปิดทั้งหมด
         }
         else
         {
            // ไม่มี Set ที่ใช้งานอยู่ - สิ้นสุดรอบ
            individualCycleCompleted = true;
            Print("✅ INDIVIDUAL CYCLE COMPLETED | All sets closed | Processed: " + IntegerToString(currentIndividualProcessCount));
         }
      }
      else
      {
         individualCycleCompleted = true;
         Print("✅ INDIVIDUAL CYCLE COMPLETED | Processed: " + IntegerToString(currentIndividualProcessCount));
      }
      return;
   }
   
   // ✅ หา Main Ticket ที่เหมาะสมที่สุด
   ulong bestMainTicket = FindBestRecoveryCandidate();
   
   if(bestMainTicket == 0)
   {
      individualCycleCompleted = true;
      Print("✅ INDIVIDUAL CYCLE COMPLETED | No suitable candidates found");
      return;
   }
   ...
   if(OpenSingleRecoveryOrder(bestMainTicket))
   {
      ...
      Print("📈 INDIVIDUAL PROGRESS: " + IntegerToString(currentIndividualProcessCount) + 
            "/" + IntegerToString(Individual_MaxPerCycle));
   }
...
{
    // คำนวณ base target จาก indicators
    
    // 1. จาก pivot point (ถูกต้องแล้ว)
    ...
    // 2. จาก moving average (แก้ไข)
    // ใช้ฟังก์ชัน GetMAValue ที่รับ shift parameter
...
{  
    ...
    g_regime = MARKET_REGIME_UNKNOWN_DETAILED;
    g_newRegime = MARKET_REGIME_UNKNOWN_DETAILED;
    ...
    // ตรวจสอบว่า handles ถูกสร้างขึ้นหรือไม่
    if(ema9_handle == INVALID_HANDLE || ema21_handle == INVALID_HANDLE || ema50_handle == INVALID_HANDLE || ema200_handle == INVALID_HANDLE)
    {
      ...
      // สร้าง handles สำหรับ EMA บน H1
    ...
    // สร้าง handles สำหรับ indicators อื่นๆ
    ...
    stoch_handle_h1 = iStochastic(_Symbol, PERIOD_H1, 5, 3, 3, MODE_SMA, STO_LOWHIGH);
      ...
      // สร้าง handles สำหรับ M15 timeframe
    ema9_handle_m15 = iMA(_Symbol, PERIOD_M15, 9, 0, MODE_EMA, PRICE_CLOSE);
    ema21_handle_m15 = iMA(_Symbol, PERIOD_M15, 21, 0, MODE_EMA, PRICE_CLOSE);
   ...
   Print("🤝 Cooperative System Activated - All strategies work together!");
...
{
   // 1. คำนวณ Drawdown พื้นฐาน (แบบเดิม)
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   ...
   // 2. ปรับตามความลึกของ Recovery
   int recoveryStep = GetCurrentRecoveryStep();
   double recoveryRisk = recoveryStep * 0.15; // แต่ละขั้นเพิ่ม risk 15%
   
   // 3. ปรับตามสภาวะตลาด
   ENUM_MARKET_REGIME_DETAILED regime = DetectMarketRegime(); // ใช้ ENUM
   double marketRisk = 0;
   switch(regime) {
      case MARKET_REGIME_RANGING_CALM_DETAILED:
         marketRisk = 0.1;    // ตลาดเงียบ → risk ต่ำ
         break;
      case MARKET_REGIME_TRENDING_STRONG_DETAILED:
         marketRisk = 0.3;    // ตลาดแรง → risk สูง
         break;
      case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
         marketRisk = 0.2;    // โอกาส breakout → risk ปานกลาง
         break;
      case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
         marketRisk = 0.25;   // ตลาดผันผวน → risk สูง
         break;
      case MARKET_REGIME_TRENDING_WEAK_DETAILED:
         marketRisk = 0.2;    // เทรนด์อ่อน → risk ปานกลาง
         break;
      default: // MARKET_REGIME_NORMAL
         marketRisk = 0.15;   // ปกติ
   }
   
   // 4. รวมความเสี่ยงทั้งหมด
   double totalRisk = baseRisk + recoveryRisk + marketRisk;
   
   if(EnableDebugMode) {
      Print("   📊 RECOVERY RISK ANALYSIS:");
...
void OnDeinit(const int reason)
{
   ...
   // ล้างเส้นเมื่อปิด EA
    DeleteBreakevenLine();
    ObjectsDeleteAll(0, "BE_"); // ล้าง text objects ทั้งหมด
}
...
// ===== ฟังก์ชั่นตรวจสอบว่ามี Orphan ที่ต้องดูแลไหม =====
bool NeedActiveOrphanMonitoring()
{
    // ตรวจสอบว่ามี Sets ที่ Main หายและยังมี Recovery Orders ที่ต้องดูแลไหม
    for(int i = 0; i < totalRecoverySets; i++)
    {
        if(recoverySets[i].isCompleted) continue;
        if(!IsMainTicketMissing(i)) continue;
        
        // ตรวจสอบว่ายังมี Recovery Orders ที่ต้องดูแลไหม
        for(int j = 0; j < recoverySets[i].subCount; j++)
        {
            ulong subTicket = recoverySets[i].subTickets[j];
            if(PositionSelectByTicket(subTicket))
            {
                double profit = PositionGetDouble(POSITION_PROFIT);
                // ถ้ามี Order ที่ยังไม่ปิด (ไม่ว่าจะกำไรหรือขาดทุน)
                return true;
            }
        }
    }
...
bool HasNewMissingMain()
{
    for(int i = 0; i < totalRecoverySets; i++)
    {
        ...
        {
            // ตรวจสอบว่าเป็น Main ที่หายใหม่ (ยังไม่ได้จัดการ)
...
{
    // ✅ ยังทำงานระบบจัดการที่สำคัญ
    UpdateAllSetProfits();
    CheckAndCloseHedgedPairs();
    ManageTradingPairs();
    RunSafetyCheck();
    
    // ❌ แต่ข้ามการเปิดออร์เดอร์ใหม่
}

//+------------------------------------------------------------------+
//| Check Portfolio Total Target (ใช้ SmartCloseProfitTarget)       |
//+------------------------------------------------------------------+
void CheckPortfolioTotalTarget()
{
    double totalProfit = CalculateTotalProfit();
    double portfolioTarget = SmartCloseProfitTarget; // 🎯 ใช้ SmartCloseProfitTarget
    
    // ตรวจสอบเงื่อนไขจำนวนออเดอร์ขั้นต่ำ
    int totalPositions = PositionsTotal();
    bool minPositionsMet = (totalPositions >= SmartCloseMinPositions);
    
    if(totalProfit >= portfolioTarget && minPositionsMet)
    {
        Print("🎯 SMART PORTFOLIO TARGET HIT!");
        Print("💰 Total Profit: $" + DoubleToString(totalProfit, 2));
        Print("🎯 Target: $" + DoubleToString(portfolioTarget, 2));
        Print("📊 Positions: " + IntegerToString(totalPositions) + " (Min: " + IntegerToString(SmartCloseMinPositions) + ")");
        Print("⚙️ Using SmartCloseProfitTarget with conditions");
        ...
        Alert("🎉 SMART PORTFOLIO TARGET ACHIEVED!");
        Alert("💰 Profit: $" + DoubleToString(totalProfit, 2));
        Alert("📊 Positions: " + IntegerToString(totalPositions));
        Alert("🎯 Target: $" + DoubleToString(portfolioTarget, 2));
        
        return;
    }
    
    // 📈 แจ้งเตือนเมื่อใกล้เป้า
    else if(totalProfit >= portfolioTarget * 0.8 && minPositionsMet)
    {
        Print("📈 APPROACHING SMART TARGET: " + DoubleToString((totalProfit/portfolioTarget)*100, 1) + "%");
        Print("📊 Positions: " + IntegerToString(totalPositions) + "/" + IntegerToString(SmartCloseMinPositions));
    }
    
    // ⚠️ แจ้งเตือนถ้าได้กำไรแต่จำนวนออเดอร์ไม่ถึง
    else if(totalProfit >= portfolioTarget && !minPositionsMet)
    {
        Print("⚠️ Profit target reached but insufficient positions: " + 
...
{
    // 🚨 แจ้งเตือนก่อนปิด
    Alert("🛑 SMART PORTFOLIO CLOSURE - " + callerName);
    Alert("🎯 Reason: " + reason);
    Alert("💰 Current Profit: $" + DoubleToString(CalculateTotalProfit(), 2));
    Alert("🎯 Target: $" + DoubleToString(SmartCloseProfitTarget, 2));
    Alert("📊 Min Positions: " + IntegerToString(SmartCloseMinPositions));
    ...
    // 📊 สรุปผล
    Alert("✅ SMART PORTFOLIO CLOSURE COMPLETED");
    Alert("📊 Closed: " + IntegerToString(closedCount) + "/" + IntegerToString(totalPositions));
    Alert("💰 Final P/L: $" + DoubleToString(totalClosedProfit, 2));
    Alert("⚙️ Settings: Target=$" + DoubleToString(SmartCloseProfitTarget, 2) + 
...
{
    Print("🔄 RESETTING ALL SYSTEMS AFTER SMART TARGET ACHIEVEMENT");
    Print("🎯 Target: $" + DoubleToString(SmartCloseProfitTarget, 2) + " (SmartCloseProfitTarget)");
    Print("📊 Min Positions: " + IntegerToString(SmartCloseMinPositions));
    
    // 1. รีเซ็ต Cooperative System
    ResetCollaborativeSystem();
    
    // 2. รีเซ็ต Recovery System
    ResetRecoverySystem();
    
    // 3. รีเซ็ต Battle Mode
    currentFightingPhase = 0;
    isInBattleMode = false;
    ...
    // 6. รีเซ็ตตัวแปรผลกำไร
    scalp2TotalProfit = 0;
    scalp3TotalProfit = 0;
    recoveryTotalProfit = 0;
    breakoutTotalProfit = 0;
    
    Print("✅ ALL SYSTEMS RESET - READY FOR NEW SESSION");
    Print("🎯 Next Target: $" + DoubleToString(SmartCloseProfitTarget, 2) + 
...
void EmergencyRecoveryStop()
{
    ...
    if(recoveryLayerCount >= localMaxSteps * 0.8)
    {
        ...
        // ตรวจสอบว่าได้กำไรตาม Target หรือยัง
        ...
        {
            Print("🆘 EMERGENCY RECOVERY STOP - Excessive drawdown detected!");
            Print("🆘 Current Drawdown: " + DoubleToString(currentDD, 1) + "%");
            Print("🆘 Current Profit: " + DoubleToString(currentProfit, 2));
            Print("🆘 Profit Target: " + DoubleToString(SmartCloseProfitTarget, 2));
        ...
        {
            Print("✅ Drawdown high but profit target already met!");
            Print("✅ Current Profit: " + DoubleToString(currentProfit, 2) + 
                  " >= Target: " + DoubleToString(SmartCloseProfitTarget, 2));
            // ไม่ต้องหยุด เพราะได้กำไรตามเป้าแล้ว
        }
    }
}
//---------MARKET ANALYSIS--------------------------//
// 🔥 เพิ่มฟังก์ชัน Market Analysis ที่ขาดหาย
void UpdateMarketAnalysis()
{
    // อัพเดทข้อมูลตลาดตลอดเวลา (ทำงานทุก Tick)
    static datetime lastUpdate = 0;
    if(TimeCurrent() - lastUpdate < 1) return; // อัพเดททุก 1 วินาที
    
    // อัพเดทข้อมูลที่ต้องการติดตามตลอดเวลา
    UpdateVolatility();
    UpdateSpread();
    UpdateTrendStrength();
...
string AnalyzeMarket()
{
    ...
    int spread = (int)SymbolInfoInteger(_Symbol, SYMBOL_SPREAD); // แก้เป็น int
    int atrHandle = iATR(_Symbol, PERIOD_H1, 14);
...
double GetTrendStrength(int direction = DIRECTION_NEUTRAL)
{
    ...
    return 0; // DIRECTION_NEUTRAL หรือค่าอื่นๆ
}
...
{
    // อัพเดทค่าความผันผวน (ใช้ในฟังก์ชันอื่น)
...
{
    // อัพเดทความแข็งแกร่งของเทรนด์
    currentTrendStrength = GetTrendStrength();
}
void AdjustPositionSizeForHighVolatility()
{
    if(!EnableDebugMode) return;
    
    Print("⚠️ HIGH VOLATILITY DETECTED - Adjusting position sizes");
    
    // ลด lot size ลง 50% ในช่วง high volatility
    double originalLot = 0.01; // หรือดึงจาก input parameter
    
    // บันทึกค่าเดิม (ถ้าจำเป็น)
    static double savedLotSize = originalLot;
    
    // ปรับ lot size
    double adjustedLot = originalLot * 0.5; // ลด 50%
    
    // จำกัดค่า minimum lot
    ...
    if(EnableDebugMode)
    {
        Print("   Original Lot: ", originalLot);
        Print("   Adjusted Lot: ", adjustedLot);
        Print("   Reduction: 50%");
    }
    
    // สามารถเก็บค่า adjusted lot ใน global variable สำหรับใช้ในฟังก์ชันอื่น
    // g_adjustedLotHighVolatility = adjustedLot;
}
// เพิ่มฟังก์ชันนี้ถ้ายังไม่มี
double GetBaseLotSize()
{
    // วิธีที่ 1: ใช้จาก input parameter
    // input double InpLotSize = 0.01; // ใน input section
    // return InpLotSize;
    
    // วิธีที่ 2: คำนวณตาม balance
    double balance = AccountInfoDouble(ACCOUNT_BALANCE);
    double riskPercent = 1.0; // 1% risk
    double stopLossPips = 50; // 50 pips SL
    double pipValue = 10; // สำหรับ forex standard
    
    double lot = (balance * riskPercent / 100) / (stopLossPips * pipValue);
...
void UseDefaultStrategy()
{
    ...
    // Strategy มาตรฐานเมื่อไม่รู้จัก regime หรือไม่ต้องการ strategy เฉพาะ
    
    // 1. ตรวจสอบ basic conditions
    double currentRisk = CalculateCurrentRiskPercent();
    if(currentRisk > 5.0) // Risk threshold
    {
        Print("   ❌ Risk too high for default strategy: ", currentRisk, "%");
        return;
    }
    
    // 2. ใช้ simple trend following
    ENUM_TREND_DIRECTION trend = GetCurrentTrendDirection();
    double trendStrength = CalculateTrendStrength();
    
    if(trend == TREND_UP && trendStrength > 4.0)
    {
        // Simple buy
        double lot = GetBaseLotSize() * 0.8; // ใช้ 80% ของ normal lot
        ExecuteTrendBuy(lot);
    }
    else if(trend == TREND_DOWN && trendStrength > 4.0)
    {
        // Simple sell
        double lot = GetBaseLotSize() * 0.8;
        ExecuteTrendSell(lot);
    }
    else
    {
        Print("   ⏸️ No clear signal for default strategy");
    }
}

// หรือแบบง่ายๆ:
void UseDefaultStrategySimple()
{
    Print("Default Strategy: No specific action");
    // ไม่ทำอะไร หรือใช้ strategy ที่ conservative
}
void ExecuteTraditionalStrategy()
{
    Print("Executing Traditional Strategy");
    ExecuteAdvancedBreakout(MARKET_REGIME_UNKNOWN_DETAILED);
}
//----------------ontick New system ---------------//
void OnTick()
{
   ...
   {
      if(EnableDebugMode) Print("❌ Market conditions not suitable");
      ExecuteEssentialManagementOnly();
      return;
   }
   ...
   g_newBar = (currentBar != g_lastBarTime);
   
   // ===== LIGHT SYSTEMS =====
   ManageScalp3PairClosing();
   RunSafetyCheck();
   
   // ===== MARKET REGIME DETECTION =====
   ENUM_MARKET_REGIME_DETAILED detectedRegime = DetectMarketRegime(); // ใช้ฟังก์ชันที่แก้ไขแล้ว
   
   if(detectedRegime != g_currentRegime)
      {
    Print("Market regime changed: ", EnumToString(g_currentRegime), 
          " -> ", EnumToString(detectedRegime));
    g_currentRegime = detectedRegime;
    
    // ปรับแผนการเทรด
    AdjustTradingPlanForNewRegime(g_currentRegime);
      }
   ...
   if(g_newBar) 
   {
      ...
      ProcessNewBarOperations(g_currentRegime);
      
      // ✅ แก้ไขตรงนี้: ต้องแปลงหรือได้ค่า ENUM_TREND_DIRECTION
      ENUM_TREND_DIRECTION currentTrend = GetCurrentTrendDirection(); // ใช้ฟังก์ชันที่คืนค่า ENUM
      ExecuteRegimeBasedStrategies(g_currentRegime, currentTrend);
      
      g_lastBarTime = currentBar;
   }
   
   // ===== POSITION MANAGEMENT =====
   ManageOpenPositionsByRegime(g_currentRegime);
}


void ProcessNewBarOperations(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
   // ✅ ระบบหนักทั้งหมดย้ายมาทำที่นี่ (แค่แท่งใหม่)
   
   // 1. อัพเดทข้อมูลพื้นฐาน
   UpdatePortfolioHealth();
   UpdateStrategyContributions();
   UpdateAllSetProfits();
   CheckAndCloseHedgedPairs();
   
   // 🔥 ปรับ Adaptive Logic ตาม Market Regime
   AdjustTradingPlanForNewRegime(currentRegime);
   
   // 🔥 เรียกใช้ Market Analysis ก่อนตัดสินใจใดๆ
   if(UseMarketAnalysis) 
   {
      ExecuteAdaptiveStrategy(currentRegime); 
   ...
   // 3. ระบบเทรดหลัก (ปรับตาม Market Regime)
   ...
   // 6. Counter Position (เฉพาะโหมดปกติ)
...
// 🔥 ปรับปรุงระบบเทรดให้ใช้ Market Analysis
void ExecuteNormalTrading(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
   // เรียกใช้แบบใหม่: ส่ง currentRegime ไปด้วย
   // ตรวจสอบเงื่อนไข ShouldOpenTrade ก่อน
...
bool ShouldTradeInRangeMarket();

// Function implementation
bool ShouldTradeInRangeMarket()
{
    // ตรวจสอบว่าเหมาะสำหรับ range trading หรือไม่
    ENUM_MARKET_REGIME_DETAILED currentRegime = g_currentRegime;
    double volatility = CalculateVolatility();
    double atrValue = GetATRValue(_Symbol, _Period, 14, 0);
    
    // เงื่อนไขสำหรับ range market trading
    // 1. ตลาดต้องอยู่ในสถานะ ranging
    bool isRangingMarket = (currentRegime == MARKET_REGIME_RANGING_CALM_DETAILED || 
                           currentRegime == MARKET_REGIME_RANGING_VOLATILE_DETAILED);
    
    if(!isRangingMarket)
    {
        if(EnableDebugMode)
            Print("Not a ranging market: ", EnumToString(currentRegime));
        return false;
    }
    
    // 2. Volatility ต้องอยู่ในระดับที่เหมาะสม
    if(volatility > 150) // Volatility สูงเกินไป
    {
        if(EnableDebugMode)
            Print("Volatility too high for range trading: ", volatility);
        return false;
    }
    
    if(volatility < 20) // Volatility ต่ำเกินไป (ตลาดเงียบเกิน)
    {
        if(EnableDebugMode)
            Print("Volatility too low for range trading: ", volatility);
        return false;
    }
    
    // 3. ตรวจสอบว่าราคาอยู่ใน range ที่ชัดเจน
    if(!IsPriceInDefinedRange())
    {
        if(EnableDebugMode)
            Print("Price not in defined range");
        return false;
    }
    
    // 4. ตรวจสอบ support/resistance levels
    if(!HasClearSupportResistance())
    {
        if(EnableDebugMode)
            Print("No clear support/resistance levels");
        return false;
    }
    
    // 5. ตรวจสอบว่าไม่ใช่ช่วงข่าว
    if(IsNewsEventTime())
    {
        if(EnableDebugMode)
            Print("News event time - avoid range trading");
        return false;
    }
    
    // 6. ตรวจสอบ risk level
    double currentRisk = CalculateCurrentRiskPercent();
    if(currentRisk > MaxRiskForRangeTrading)
    {
        if(EnableDebugMode)
            Print("Risk too high for range trading: ", currentRisk, "%");
        return false;
    }
...
// Helper functions
bool IsPriceInDefinedRange()
{
    // ตรวจสอบว่าราคาอยู่ใน range ที่ชัดเจน
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    double atrValue = iATR(_Symbol, PERIOD_H1, 14);
    
    // หา range จาก recent price action
    ...
    double rangeHeight = recentHigh - recentLow;
    
    // Range ต้องมีความสูงอย่างน้อย 2x ATR
    if(rangeHeight < atrValue * 2)
    {
        return false;
    }
    
    // ราคาต้องอยู่ภายใน range (ไม่ใกล้ขอบเกินไป)
    double distanceToHigh = recentHigh - currentPrice;
    double distanceToLow = currentPrice - recentLow;
    
    // ต้องไม่อยู่ใกล้ขอบ range ภายใน 0.5x ATR
    return (distanceToHigh > atrValue * 0.5 && distanceToLow > atrValue * 0.5);
}




void ExecuteRecoveryTrading(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
   // Recovery systems กับ market condition
...
bool IsGoodPullbackForTrend(ENUM_TREND_DIRECTION direction)
{
    // ตรวจสอบว่าเป็น pullback ที่ดีสำหรับเข้าเทรดต่อ trend
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    double atrValue = GetATRValue(_Symbol, _Period, 14, 0);
    
    // หา dynamic support/resistance สำหรับ trend
    double maValue = GetMAValue(_Symbol, PERIOD_CURRENT, _Period, 0, MODE_EMA);
    
    if(direction == TREND_UP || direction == DIRECTION_UP)
    {
        // สำหรับ uptrend: ราคาต้องอยู่เหนือ MA และ pullback มาหา MA
        if(currentPrice < maValue) return false;
        
        double distanceToMA = currentPrice - maValue;
        return (distanceToMA < atrValue * 1.5); // Pullback ไม่ลึกเกินไป
    }
    else if(direction == TREND_DOWN || direction == DIRECTION_DOWN)
    {
        // สำหรับ downtrend: ราคาต้องอยู่ใต้ MA และ pullback มาหา MA
        if(currentPrice > maValue) return false;
        
        double distanceToMA = maValue - currentPrice;
        return (distanceToMA < atrValue * 1.5); // Pullback ไม่ลึกเกินไป
    }
    
    return false;
}
bool ShouldTradeInTrendMarket(ENUM_TREND_DIRECTION trendDirection)
{
    // ตรวจสอบ basic conditions
    if(trendDirection == TREND_NONE)
    {
        if(EnableDebugMode)
            Print("ShouldTradeInTrendMarket: No trend direction");
        return false;
    }
    
    // ตรวจสอบ market regime
    ENUM_MARKET_REGIME_DETAILED regime = g_currentRegime;
    
    if(regime != MARKET_REGIME_TRENDING_WEAK_DETAILED && 
       regime != MARKET_REGIME_TRENDING_STRONG_DETAILED)
    {
        if(EnableDebugMode)
            Print("ShouldTradeInTrendMarket: Not in trending regime - ", EnumToString(regime));
        return false;
    }
    
    // ตรวจสอบ trend strength
    double trendStrength = CalculateTrendStrength();
    double requiredStrength = (regime == MARKET_REGIME_TRENDING_STRONG_DETAILED) ? 5.0 : 7.0;
    
    if(trendStrength < requiredStrength)
    {
        if(EnableDebugMode)
            Print("ShouldTradeInTrendMarket: Trend strength too low - ", trendStrength, " < ", requiredStrength);
        return false;
    }
    
    // ตรวจสอบ risk
    double currentRisk = CalculateCurrentRiskPercent();
    if(currentRisk > g_maxRiskTrendTrading)
    {
        if(EnableDebugMode)
            Print("ShouldTradeInTrendMarket: Risk too high - ", currentRisk, "% > ", g_maxRiskTrendTrading, "%");
        return false;
    }
    
    return true;
}
// 🔥 ฟังก์ชัน ExecuteRegimeBasedStrategies
void ExecuteRegimeBasedStrategies(ENUM_MARKET_REGIME_DETAILED regime, ENUM_TREND_DIRECTION trendDirection)  // ✅ เปลี่ยนจาก int เป็น ENUM_TREND_DIRECTION
{
   switch(regime)
   {
      case MARKET_REGIME_RANGING_CALM_DETAILED:
         if(ShouldTradeInRangeMarket()) {
            UseRangeStrategy(MARKET_REGIME_RANGING_CALM_DETAILED);
         }
         break;
         
      case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
         // ตรวจสอบ volatility ก่อนเทรด
          if(currentVolatility < 150 && ShouldTradeInVolatileMarket(MARKET_REGIME_RANGING_VOLATILE_DETAILED)) {
        ExecuteVolatileMarketStrategy();
         }
         break;
      ...
      case MARKET_REGIME_TRENDING_WEAK_DETAILED:
      case MARKET_REGIME_TRENDING_STRONG_DETAILED:
         if(ShouldTradeInTrendMarket(trendDirection)) {
            bool isStrong = (regime == MARKET_REGIME_TRENDING_STRONG_DETAILED);
            UseTrendStrategyWithDirection(trendDirection, isStrong);
         }
         break;
            
      case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
          // ✅ เรียกใช้ถูกต้องแล้ว (ส่ง enum)
         if(IsBreakoutConfirmed(MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED)) {
        UseBreakoutStrategy();
         }
          break;
      ...
      case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
         // ปรับ position size ให้เล็กลงหรือหยุดเทรด
         AdjustPositionSizeForHighVolatility();
         Print("⚠️ HIGH VOLATILITY - Reduced position sizes");
         break;
      ...
      case MARKET_REGIME_UNKNOWN_DETAILED:
      default:
         // ใช้ strategy มาตรฐานหรือหยุดเทรด
         UseDefaultStrategy();
         break;
   }
}
void ManageOpenPositionsByRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
   switch(regime)
   {
      case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
         // ปรับ SL/TP ให้กว้างขึ้นในตลาด volatile
         AdjustStopLossForVolatility();
         break;
         
      case MARKET_REGIME_TRENDING_WEAK_DETAILED:
      case MARKET_REGIME_TRENDING_STRONG_DETAILED:
         // ใน trending market อาจใช้ trailing stop
         ApplyTrailingStopForTrend();
         break;
         
      case MARKET_REGIME_RANGING_CALM_DETAILED:
         // ใน ranging market ใช้ tight stop
         ApplyTightStopLoss();
         break;
         
      case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
         // ใน ranging volatile ใช้ volatility adjustment
         AdjustStopLossForVolatility();
         ApplyTightStopLoss(); // หรือไม่ใช้ตามต้องการ
         break;
         
      case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
         // ใน breakout market ใช้ trailing stop ที่ aggressive
         ApplyTrailingStopForStrongTrend();
         break;
         
      case MARKET_REGIME_NEWS_EVENT_DETAILED:
         // ในช่วงข่าว ใช้ tight stop เพื่อป้องกันความเสี่ยง
         ApplyTightStopLossForCondition(true);
         break;
         
      default:
         // สำหรับ regime อื่นๆ ใช้การจัดการปกติ
         if(EnableDebugMode)
            Print("No specific position management for regime: ", EnumToString(regime));
         break;
   }
}
// ========== VOLATILITY ADJUSTMENT FUNCTIONS ==========

void AdjustStopLossForVolatility()
{
    // ปรับ Stop Loss ตาม volatility ปัจจุบัน
    double volatility = CalculateVolatility();
    double atrValue = GetATRValue(_Symbol, _Period, 14, 0);

    
    if(EnableDebugMode)
        Print("🔧 Adjusting Stop Loss for Volatility: ", volatility, "%, ATR: ", atrValue);
    
    // ปรับ SL ตาม volatility
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
        if(PositionGetSymbol(i) == _Symbol)
        {
            ...
            // คำนวณ SL ใหม่ตาม volatility
            double newSL = 0;
            double slDistance = 0;
            
            if(volatility > 200) // High volatility
            {
                slDistance = atrValue * 3.0; // Wider SL
            }
            else if(volatility > 100) // Medium volatility
            {
                slDistance = atrValue * 2.0; // Normal SL
            }
            else // Low volatility
            {
                slDistance = atrValue * 1.5; // Tighter SL
            }
            
            if(type == POSITION_TYPE_BUY)
            {
                newSL = openPrice - slDistance;
                // ตรวจสอบว่าไม่ทำให้ SL ใกล้กว่่าที่ควร
                if(newSL < currentPrice - (atrValue * 0.5))
                    newSL = currentPrice - (atrValue * 0.5);
            }
            ...
            // ปรับ SL ถ้าจำเป็น
            if(newSL > 0 && MathAbs(newSL - currentSL) > SymbolInfoDouble(_Symbol, SYMBOL_POINT) * 10)
            {
                trade.PositionModify(ticket, newSL, PositionGetDouble(POSITION_TP));
                if(EnableDebugMode)
                    Print("   Adjusted SL for ticket #", ticket, 
                          " from ", currentSL, " to ", newSL,
                          " (Volatility: ", volatility, "%)");
            }
        }
    }
}
// เพิ่มฟังก์ชันนี้ถ้ายังไม่มี (ง่ายกว่า GetATRValue)
...
// เวอร์ชันง่ายๆ สำหรับใช้ทันที
void AdjustStopLossForVolatilitySimple()
{
    double atrValue = GetATRValue(_Symbol, _Period, 14, 0);
...
void ApplyTrailingStopForTrend()
{
    // Apply trailing stop สำหรับเทรนด์
    ENUM_TREND_DIRECTION currentTrend = GetCurrentTrendDirection();
    double atrValue = GetCurrentATR(14);
    
    if(EnableDebugMode)
        Print("🎯 Applying Trailing Stop for Trend: ", TrendToString(currentTrend));
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
        if(PositionGetSymbol(i) == _Symbol)
        {
            ...
            // ตรวจสอบว่าออเดอร์นี้สอดคล้องกับเทรนด์หรือไม่
            bool isInTrendDirection = false;
            if(currentTrend == TREND_UP && type == POSITION_TYPE_BUY)
                isInTrendDirection = true;
            else if(currentTrend == TREND_DOWN && type == POSITION_TYPE_SELL)
                isInTrendDirection = true;
            
            if(isInTrendDirection)
            {
                // Trailing stop สำหรับเทรนด์
                double newSL = 0;
                double trailingDistance = atrValue * 1.5; // 1.5 x ATR
                
                if(type == POSITION_TYPE_BUY)
                {
                    newSL = currentPrice - trailingDistance;
                    // ตรวจสอบว่า SL ใหม่ต้องดีกว่าเดิม
                    if(newSL > currentSL && newSL > openPrice)
                    {
                        trade.PositionModify(ticket, newSL, PositionGetDouble(POSITION_TP));
                        if(EnableDebugMode)
                            Print("   Trailing BUY #", ticket, " SL: ", currentSL, " → ", newSL);
                    }
                }
                else if(type == POSITION_TYPE_SELL)
                {
                    ...
                    if(newSL < currentSL && newSL < openPrice)
                    {
                        trade.PositionModify(ticket, newSL, PositionGetDouble(POSITION_TP));
                        if(EnableDebugMode)
                            Print("   Trailing SELL #", ticket, " SL: ", currentSL, " → ", newSL);
                    }
                }
            }
        }
...
// เวอร์ชันสำหรับเทรนด์แรง
void ApplyTrailingStopForStrongTrend()
{
    ENUM_MARKET_REGIME_DETAILED regime = g_currentRegime;
    
    if(regime == MARKET_REGIME_TRENDING_STRONG_DETAILED)
    {
        double atrValue = GetCurrentATR(14);
        ...
        for(int i = PositionsTotal() - 1; i >= 0; i--)
        {
            if(PositionGetSymbol(i) == _Symbol)
            {
                ...
                // ใช้ trailing stop ที่ aggressive กว่าในเทรนด์แรง
                if(profit > 0) // มีกำไรแล้ว
                {
                    double newSL = 0;
                    if(type == POSITION_TYPE_BUY)
                        newSL = currentPrice - (atrValue * 1.0); // แคบกว่า
                    else if(type == POSITION_TYPE_SELL)
                        newSL = currentPrice + (atrValue * 1.0);
...
// ========== TIGHT STOP LOSS FUNCTIONS ==========

void ApplyTightStopLoss()
{
    // ใช้ tight stop loss สำหรับตลาดผันผวนหรือต้องการป้องกันความเสี่ยง
    double atrValue = GetCurrentATR(14);
    
    if(EnableDebugMode)
        Print("🔒 Applying Tight Stop Loss (ATR: ", atrValue, ")");
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
        if(PositionGetSymbol(i) == _Symbol)
        {
            ...
            // คำนวณ tight stop loss
            double newSL = 0;
            double tightDistance = atrValue * 0.8; // 0.8 x ATR (แคบมาก)
            
            // ตรวจสอบว่า position ยังไม่กำไรมาก
            double profit = PositionGetDouble(POSITION_PROFIT);
            double profitPips = MathAbs(currentPrice - openPrice) / SymbolInfoDouble(_Symbol, SYMBOL_POINT);
            
            if(profitPips < 20) // ยังไม่ได้กำไรมาก
            {
                if(type == POSITION_TYPE_BUY)
                {
                    newSL = openPrice - tightDistance;
                    // ตรวจสอบว่าไม่ทำให้ SL เกิน current price
                    if(newSL < currentPrice - (atrValue * 0.3))
                        newSL = currentPrice - (atrValue * 0.3);
                }
                else if(type == POSITION_TYPE_SELL)
                {
                    newSL = openPrice + tightDistance;
                ...
                if(newSL > 0)
                {
                    trade.PositionModify(ticket, newSL, PositionGetDouble(POSITION_TP));
                    if(EnableDebugMode)
                        Print("   Tight SL for #", ticket, ": ", currentSL, " → ", newSL);
                }
            }
        }
    }
...
// เวอร์ชันสำหรับ specific conditions
void ApplyTightStopLossForCondition(bool condition)
{
    if(condition) // เช่น ตลาดผันผวนสูง, ข่าวสำคัญ, etc.
    {
        double atrValue = GetCurrentATR(14);
        ...
        for(int i = PositionsTotal() - 1; i >= 0; i--)
        {
            if(PositionGetSymbol(i) == _Symbol)
            {
                ...
                double newSL = 0;
                if(type == POSITION_TYPE_BUY)
                    newSL = openPrice - (atrValue * 0.5); // แคบมาก
                else if(type == POSITION_TYPE_SELL)
                    newSL = openPrice + (atrValue * 0.5);
...
// 🔥 ฟังก์ชัน DisplayMarketDashboard
void DisplayMarketDashboard(ENUM_MARKET_REGIME_DETAILED regime, int trendDirection)
{
   // แปลง regime เป็น string
   string regimeStr = EnumToString(regime);
   
   // แปลง trend direction
   string trendStr;
   if(trendDirection == 1) trendStr = "UP";
   else if(trendDirection == -1) trendStr = "DOWN";
   else if(trendDirection == 0) trendStr = "NONE";
   else trendStr = "UNKNOWN (" + IntegerToString(trendDirection) + ")";
   
   // ดึงข้อมูล market เพิ่มเติม
   double volatility = CalculateVolatility();
   double atrValue = GetCurrentATR(14);
   double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double riskPercent = CalculateCurrentRiskPercent();
   ...
   string dashboard = 
      ...
      "║        MARKET DASHBOARD          ║\n" +
      "╠══════════════════════════════════╣\n" +
      "║ Regime:    " + StringFormat("%-20s", regimeStr) + "║\n" +
      "║ Trend:     " + StringFormat("%-20s", trendStr) + "║\n" +
      "║ Price:     " + StringFormat("%-20s", DoubleToString(currentPrice, _Digits)) + "║\n" +
      "║ Volatility:" + StringFormat("%-20s", DoubleToString(volatility, 1) + "%") + "║\n" +
      "║ ATR:       " + StringFormat("%-20s", DoubleToString(atrValue, _Digits)) + "║\n" +
      "║ Risk:      " + StringFormat("%-20s", DoubleToString(riskPercent, 1) + "%") + "║\n" +
      "║ Time:      " + StringFormat("%-20s", TimeToString(TimeCurrent(), TIME_SECONDS)) + "║\n" +
      "╚══════════════════════════════════╝";
   ...
   {
      Print("Dashboard Update - Regime: ", regimeStr, " | Trend: ", trendStr);
      lastUpdate = TimeCurrent();
   }
}

// หรือใช้ฟังก์ชัน helper สำหรับแปลง enum
string RegimeToString(ENUM_MARKET_REGIME_DETAILED regime)
{
   switch(regime)
   {
      case MARKET_REGIME_UNKNOWN_DETAILED: return "Unknown";
      case MARKET_REGIME_RANGING_CALM_DETAILED: return "Ranging Calm";
      case MARKET_REGIME_RANGING_VOLATILE_DETAILED: return "Ranging Volatile";
      case MARKET_REGIME_TRENDING_WEAK_DETAILED: return "Trending Weak";
      case MARKET_REGIME_TRENDING_STRONG_DETAILED: return "Trending Strong";
      case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED: return "Breakout Potential";
      case MARKET_REGIME_HIGH_VOLATILITY_DETAILED: return "High Volatility";
      case MARKET_REGIME_NEWS_EVENT_DETAILED: return "News Event";
      default: return "Unknown";
   }
}
//----------------ontick New system ---------------//
// 🔥 ฟังก์ชัน CalculateCurrentVolatility (ถ้ายังไม่มี)
double CalculateCurrentVolatility()
{
   ...
   double volatilityPercent = (atrHandle / currentPrice) * 10000;
   
   return NormalizeDouble(volatilityPercent, 2);
}
//+------------------------------------------------------------------+
//| Open Test Position                                              |
//+------------------------------------------------------------------+
void OpenTestPosition()
{
   ...
   if(CheckMargin(lot))
   {
      ...
      // Random direction หรือตามสัญญาณ
...
//+------------------------------------------------------------------+
void TryEmergencyCounter(ENUM_MARKET_REGIME_DETAILED regime = MARKET_REGIME_UNKNOWN_DETAILED)  // ✅ แก้ไข enum value
{
   // ตรวจจับ regime ถ้าไม่ได้ส่งมา
   if(regime == MARKET_REGIME_UNKNOWN_DETAILED)  // ✅ แก้ไขจาก MARKET_REGIME_NORMAL
   {
      regime = DetectMarketRegime();
   }
   ...
   {
      // 🔥 ปรับ counter multiplier ตาม regime
      // ✅ แก้ไข syntax error: ลบ ENUM_MARKET_REGIME_DETAILED ออก
      ...
      {
         // 🔥 แก้ไข: ส่ง regime ไปด้วย
         OpenCounterPosition(worstTicket, worstType, worstVolume, worstSymbol, worstProfit, regime);
      }
   }
}
// เพิ่มฟังก์ชัน helper สำหรับ emergency counter
double GetEmergencyCounterMultiplierForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 0.5; // ลด lot ในตลาดผันผวน
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 0.8; // ลด lot ในเทรนด์แรง
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return 0.6; // ลด lot ใน breakout
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            return 1.2; // อาจเพิ่ม lot ใน sideways เงียบ
...
{
    static int lastLossCount = 0;
    static datetime lastCheckTime = 0;
    
    // รีเฟรชทุก 1 นาที
    if(TimeCurrent() - lastCheckTime < 60)
        return lastLossCount;
    
    lastCheckTime = TimeCurrent();
    
    
    
    // ✅ วิธีที่ 1: ใช้ HistoryDealsTotal() (แนะนำ)
    int totalDeals = HistoryDealsTotal();
    
    // ตรวจสอบจากประวัติ (ล่าสุดไปหาเก่าสุด)
    for(int i = totalDeals - 1; i >= 0; i--)
    {
        ...
        if(dealTicket > 0)
        {
            ...
            // ตรวจสอบเฉพาะ position close deals
    ...
    lastLossCount = consecutiveLosses;
    return consecutiveLosses;
}
// ฟังก์ชันตรวจสอบเงื่อนไขฉุกเฉิน
bool IsEmergencyCondition()
{
    // ตรวจสอบ drawdown
    double drawdownPercent = CalculateCurrentDrawdownPercent();
    if(drawdownPercent > g_EmergencyDrawdownThreshold) {
        Print("⚠️ Emergency: Drawdown exceeded: ", drawdownPercent, "%");
        return true;
    }
    
    // ตรวจสอบ margin level
    double marginLevel = AccountInfoDouble(ACCOUNT_MARGIN_LEVEL);
    if(marginLevel < g_EmergencyMarginLevel) {
        Print("⚠️ Emergency: Margin level low: ", marginLevel, "%");
        return true;
    }
    
    // ตรวจสอบจำนวน loss positions ติดต่อกัน
    g_consecutiveLosses = GetConsecutiveLossCount();
    if(g_consecutiveLosses > MaxConsecutiveLosses) {
    Print("⚠️ Emergency: Too many consecutive losses: ", g_consecutiveLosses);
...
// ฟังก์ชัน ExecuteEmergencyCounterStrategy
void ExecuteEmergencyCounterStrategy(ENUM_MARKET_REGIME_DETAILED regime)
{
    Print("🚨 EXECUTING EMERGENCY COUNTER STRATEGY");
    Print("   Regime: ", RegimeToString(regime));
    Print("   Time: ", TimeToString(TimeCurrent()));
    
    // 1. ปิดบาง positions ที่ขาดทุนมากที่สุด
    //CloseWorstPerformingPositions();
    
    // 2. ปรับ lot size ตาม regime
    double emergencyLot = CalculateEmergencyLotSize(regime);
    
    // 3. Execute counter trade ตาม regime
    switch(regime)
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            ExecuteTrendCounterTrade(emergencyLot);
            break;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            ExecuteRangeCounterTrade(emergencyLot);
            break;
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            ExecuteBreakoutCounterTrade(emergencyLot);
            break;
            
        default:
            ExecuteDefaultCounterTrade(emergencyLot);
            break;
    }
    
    // 4. ตั้งค่า stop loss ที่กว้างขึ้น
    ApplyWiderStopLossForEmergency();
    
    // 5. ตั้งค่า take profit ที่เร็วขึ้น
    ApplyQuickTakeProfitForEmergency();
}
void ApplyWiderStopLossForEmergency()
{
    int totalPositions = PositionsTotal();
    if(totalPositions == 0)
    {
        Print("⚠️ No positions to adjust stop loss");
        return;
    }
    
    Print("🔧 Applying WIDER Stop Loss for Emergency");
    Print("   Positions to adjust: ", totalPositions);
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
        ...
        if(PositionSelectByTicket(ticket))
        {
            ...
            // คำนวณ new SL ที่กว้างขึ้น (3-5x ATR แทนที่จะเป็น 1-2x ปกติ)
            double newSL = 0;
            double widerMultiplier = 4.0; // 4 เท่าของ ATR (กว้างกว่าเดิม)
            
            if(type == POSITION_TYPE_BUY)
            {
                newSL = openPrice - (atrValue * widerMultiplier);
                
                // ตรวจสอบว่า newSL ไม่ต่ำกว่า currentPrice มากเกินไป
                double maxDistance = atrValue * 6.0;
                if(currentPrice - newSL > maxDistance)
                {
                    newSL = currentPrice - maxDistance;
                }
            }
            else if(type == POSITION_TYPE_SELL)
            {
                ...
                // ตรวจสอบว่า newSL ไม่สูงกว่า currentPrice มากเกินไป
                double maxDistance = atrValue * 6.0;
                if(newSL - currentPrice > maxDistance)
                {
                    newSL = currentPrice + maxDistance;
                }
            }
            ...
            if(newSL > 0 && newSL != currentSL)
            {
                ...
                {
                    adjustedCount++;
                    Print("   ✅ Adjusted #", ticket, 
                          " SL: ", currentSL, " → ", newSL,
                          " (ATR Multiplier: ", widerMultiplier, ")");
                }
                else
                {
                    Print("   ❌ Failed to adjust #", ticket, 
    ...
    Print("📊 Total positions adjusted: ", adjustedCount, "/", totalPositions);
}
void ApplyQuickTakeProfitForEmergency()
{
    int totalPositions = PositionsTotal();
    if(totalPositions == 0)
    {
        Print("⚠️ No positions to adjust take profit");
        return;
    }
    
    Print("🎯 Applying QUICK Take Profit for Emergency");
    Print("   Positions to adjust: ", totalPositions);
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
        ...
        if(PositionSelectByTicket(ticket))
        {
            ...
            // คำนวณ new TP ที่ใกล้ขึ้น (1-2x ATR แทนที่จะเป็น 3-4x ปกติ)
            double newTP = 0;
            double quickMultiplier = 1.5; // 1.5 เท่าของ ATR (ใกล้กว่าเดิม)
            
            // ตรวจสอบว่าต้องการ take profit หรือไม่
            bool shouldAdjustTP = true;
            
            // ถ้า position กำลังขาดทุนหนัก อาจไม่ปรับ TP
            double profit = PositionGetDouble(POSITION_PROFIT);
            if(profit < -50) // ขาดทุนมากกว่า $50
            {
                Print("   ⏸️ Skipping #", ticket, " - Heavy loss: $", profit);
            ...
            if(shouldAdjustTP)
            {
                if(type == POSITION_TYPE_BUY)
                {
                    newTP = openPrice + (atrValue * quickMultiplier);
                    
                    // ตรวจสอบว่า newTP ไม่สูงเกินไป
                    double minProfitDistance = atrValue * 0.5;
                    if(newTP - currentPrice < minProfitDistance)
                    {
                        newTP = currentPrice + minProfitDistance;
                    }
                }
                else if(type == POSITION_TYPE_SELL)
                {
                    ...
                    // ตรวจสอบว่า newTP ไม่ต่ำเกินไป
                    double minProfitDistance = atrValue * 0.5;
                    if(currentPrice - newTP < minProfitDistance)
                    {
                        newTP = currentPrice - minProfitDistance;
                    }
                }
                ...
                if(newTP > 0 && newTP != currentTP)
                {
                    if(trade.PositionModify(ticket, currentSL, newTP))
                    {
                        ...
                        Print("   ✅ Adjusted #", ticket, 
                              " TP: ", currentTP, " → ", newTP,
                              " (ATR Multiplier: ", quickMultiplier, ")");
                    }
                    else
                    {
                        Print("   ❌ Failed to adjust #", ticket, 
    ...
    Print("📊 Total TP adjusted: ", adjustedCount, "/", totalPositions);
}
void ApplySmartEmergencyStopLossTakeProfit(ENUM_MARKET_REGIME_DETAILED regime)
{
    Print("🧠 Applying SMART Emergency SL/TP for Regime: ", RegimeToString(regime));
    
    double atrValue = GetCurrentATR(14);
    
    // ตั้งค่า multipliers ตาม regime
    double slMultiplier = 3.0; // Default
    double tpMultiplier = 2.0; // Default
    
    switch(regime)
    {
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
            slMultiplier = 5.0; // กว้างมาก
            tpMultiplier = 3.0; // อยู่ห่างพอสมควร
            break;
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            slMultiplier = 4.0; // กว้าง
            tpMultiplier = 4.0; // ไกล (ให้กำไรเดินต่อ)
            break;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            slMultiplier = 3.0; // ปกติ
            tpMultiplier = 1.5; // ใกล้
            break;
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            slMultiplier = 3.5; // กว้างปานกลาง
            tpMultiplier = 3.0; // ไกลปานกลาง
            break;
            
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            slMultiplier = 6.0; // กว้างมาก
            tpMultiplier = 2.0; // ใกล้
            break;
    }
    
    Print("   SL Multiplier: ", slMultiplier, "x ATR");
    Print("   TP Multiplier: ", tpMultiplier, "x ATR");
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
        ...
        if(PositionSelectByTicket(ticket))
        {
            ...
            if(type == POSITION_TYPE_BUY)
            {
                newSL = openPrice - (atrValue * slMultiplier);
                newTP = openPrice + (atrValue * tpMultiplier);
            }
            else if(type == POSITION_TYPE_SELL)
            {
                newSL = openPrice + (atrValue * slMultiplier);
            ...
            if(EnableDebugMode)
            {
                Print("   Smart adjust #", ticket, 
                      " SL: ", newSL, " TP: ", newTP);
            }
        }
    }
}
void ApplyDynamicEmergencyStopLossTakeProfit()
{
    Print("📈 Applying DYNAMIC Emergency SL/TP based on P/L");
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
        ...
        if(PositionSelectByTicket(ticket))
        {
            ...
            // Dynamic multipliers based on profit/loss
            double slMultiplier, tpMultiplier;
            
            if(profit < -100) // ขาดทุนหนัก (> $100)
            {
                slMultiplier = 5.0; // กว้างมาก (ให้มีโอกาสคืนทุน)
                tpMultiplier = 2.5; // ใกล้ (ต้องการกำไรเร็ว)
            }
            else if(profit < 0) // ขาดทุนเล็กน้อย
            {
                slMultiplier = 3.5; // กว้าง
                tpMultiplier = 2.0; // ใกล้
            }
            else if(profit > 50) // กำไร (> $50)
            {
                slMultiplier = 2.0; // แคบ (ป้องกันกำไร)
                tpMultiplier = 1.5; // ใกล้ (เก็บกำไร)
            }
            else // กำไรเล็กน้อยหรือ break-even
            {
                slMultiplier = 3.0; // ปกติ
                tpMultiplier = 2.0; // ปกติ
            }
            ...
            if(type == POSITION_TYPE_BUY)
            {
                newSL = currentPrice - (atrValue * slMultiplier);
                newTP = currentPrice + (atrValue * tpMultiplier);
            }
            else if(type == POSITION_TYPE_SELL)
            {
                newSL = currentPrice + (atrValue * slMultiplier);
            ...
            // Move SL to break-even if in good profit
            if(profit > 20)
            {
                if(type == POSITION_TYPE_BUY)
            ...
            Print("   Dynamic #", ticket, 
                  " P/L: $", profit,
                  " SL: ", newSL, " (", slMultiplier, "x ATR)",
                  " TP: ", newTP, " (", tpMultiplier, "x ATR)");
        }
    }
}
double GetBaseEmergencyLot()
{
    Print("🔧 Calculating Base Emergency Lot");
    
    // วิธีที่ 1: ใช้ percentage ของ balance
    double balance = AccountInfoDouble(ACCOUNT_BALANCE);
    double equity = AccountInfoDouble(ACCOUNT_EQUITY);
    ...
    // วิธีที่ 2: ใช้ fixed risk amount
    double emergencyRiskAmount = 50.0; // $50 risk สำหรับ emergency trade
    double stopLossPips = 50; // 50 pips สำหรับ emergency SL
    double pipValue = 10; // สำหรับ forex standard (ประมาณ)
    
    // คำนวณ base lot
    double baseLot = emergencyRiskAmount / (stopLossPips * pipValue);
    
    // วิธีที่ 3: ปรับตาม drawdown
    double drawdownMultiplier = 1.0;
    if(drawdownPercent > 20)
        drawdownMultiplier = 0.3; // ลด lot มากถ้า drawdown สูง
    else if(drawdownPercent > 10)
        drawdownMultiplier = 0.5; // ลด lot ปานกลาง
    else if(drawdownPercent > 5)
        drawdownMultiplier = 0.7; // ลด lot น้อย
    
    baseLot *= drawdownMultiplier;
    
    // วิธีที่ 4: ใช้จาก input parameter ถ้ามี
    #ifdef InpEmergencyLotSize
        baseLot = InpEmergencyLotSize;
    #endif
    ...
    Print("   Equity: $", equity);
    Print("   Drawdown: ", drawdownPercent, "%");
    Print("   Drawdown Multiplier: ", drawdownMultiplier);
    Print("   Min Lot: ", minLot);
    Print("   Max Lot: ", maxLot);
    Print("   Final Emergency Lot: ", baseLot);
    
    return baseLot;
}
double CalculateEmergencyLotSize(ENUM_MARKET_REGIME_DETAILED regime)
{
    double baseLot = GetBaseEmergencyLot();
    double multiplier = 1.0;
    
    switch(regime)
    {
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
            multiplier = 0.3; // ลด lot มากใน high volatility
            break;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            multiplier = 0.5; // ลด lot ใน ranging volatile
            break;
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            multiplier = 0.7; // ลดนิดหน่อยใน trending strong
            break;
...
bool IsEmergencyCounterAllowedInRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            return false; // หยุดใช้ในตลาดเสี่ยงสูง
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return true; // อนุญาตใน trending strong
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return true; // อนุญาตใน ranging volatile
            
        default:
            return false; // ไม่อนุญาตในสภาวะอื่นโดย default
    }
}
//+------------------------------------------------------------------+
//| Open Counter Position                                           |
//+------------------------------------------------------------------+
bool OpenCounterPosition(ulong ticket, ENUM_POSITION_TYPE type, double volume, string symbol, double profit, ENUM_MARKET_REGIME_DETAILED regime)
{
    // ปรับ volume ตาม regime
    double adjustedVolume = AdjustCounterVolume(volume, regime);
    
    // จำกัด volume ขั้นต่ำ
    double minVolume = SymbolInfoDouble(symbol, SYMBOL_VOLUME_MIN);
    adjustedVolume = MathMax(adjustedVolume, minVolume);
    
    // จำกัด volume ขั้นสูง
    double maxVolume = SymbolInfoDouble(symbol, SYMBOL_VOLUME_MAX);
    adjustedVolume = MathMin(adjustedVolume, maxVolume);
    
    // ปรับ stop loss ตาม regime
    double slDistance = GetCounterStopLossDistance(regime);
    
    // ปรับ take profit ตาม regime
    ...
    if(counterType == ORDER_TYPE_BUY)
    {
        ...
        {
            Print("✅ Counter BUY opened successfully");
            return true;
        }
        else
        {
            Print("❌ Counter BUY failed: ", trade.ResultRetcodeDescription());
            return false;
        }
    }
    else // ORDER_TYPE_SELL
    {
        ...
        {
            Print("✅ Counter SELL opened successfully");
            return true;
        }
        else
        {
            Print("❌ Counter SELL failed: ", trade.ResultRetcodeDescription());
            return false;
        }
    }
}
double AdjustCounterVolume(double originalVolume, ENUM_MARKET_REGIME_DETAILED regime)
{
    double multiplier = 1.0;
    ...
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            multiplier = 0.5; // ลด volume ในตลาดผันผวน
            break;
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            multiplier = 0.7; // ลด volume ในเทรนด์แรง
            break;
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            multiplier = 0.6; // ลด volume ใน breakout
            break;
...
void ExecuteLowRiskOpportunity()
{
   ...
   double atrValue = CalculateATR(14, 0, _Symbol);
   bool strongBuy = (rates[0].close > rates[1].high + atrValue * 0.5) && 
                    (rates[0].close > rates[0].open);
   bool strongSell = (rates[0].close < rates[1].low - atrValue * 0.5) && 
                     (rates[0].close < rates[0].open);
...
//+------------------------------------------------------------------+
void ManageCounterPositions(ENUM_MARKET_REGIME_DETAILED regime) 
{
   if(!EnableCounterPosition) return;
   
   // ตรวจสอบว่า counter position ควรทำงานใน regime นี้ไหม
   ...
   // ปรับ counter trigger ตาม regime
   ...
   // ปรับ max counter positions ตาม regime
   ...
   // เปิด counter position
   OpenCounterPosition(worstTicket, worstType, worstVolume, worstSymbol, worstProfit, regime);
}

// ฟังก์ชันที่เรียกใช้งาน (ต้องส่ง regime ไปด้วย)
void CheckAndManageCounterPositions()
{
    // ตรวจจับ market regime
    ENUM_MARKET_REGIME_DETAILED currentRegime = DetectMarketRegime();
    
    // เรียกใช้ ManageCounterPositions
    ManageCounterPositions(currentRegime);
}
double GetCounterStopLossDistance(ENUM_MARKET_REGIME_DETAILED regime)
{
    double baseSL = localCounterSL; // ค่า SL พื้นฐาน (ควรประกาศใน input parameters)
    
    if(baseSL <= 0) 
    {
        // ถ้าไม่มีค่า SL พื้นฐาน ให้คำนวณจาก ATR
        double atrValue = GetATRValue(_Symbol, PERIOD_H1, 14, 0);
        baseSL = atrValue * 2; // 2 * ATR
    }
    ...
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            multiplier = 0.7; // SL แคบในตลาดผันผวน
            break;
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            multiplier = 1.3; // SL กว้างในเทรนด์แรง
            break;
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            multiplier = 1.2; // SL กว้างใน breakout
            break;
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            multiplier = 0.9; // SL แคบใน sideways เงียบ
            break;
...
double GetCounterTakeProfitDistance(ENUM_MARKET_REGIME_DETAILED regime)
{
    double baseTP = localCounterTP; // ค่า TP พื้นฐาน
    
    if(baseTP <= 0) 
    {
        // ถ้าไม่มีค่า TP พื้นฐาน ให้คำนวณจาก ATR
        double atrValue = GetATRValue(_Symbol, PERIOD_H1, 14, 0);
        baseTP = atrValue * 3; // 3 * ATR
    }
    ...
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            multiplier = 1.5; // TP ยาวในเทรนด์แรง
            break;
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            multiplier = 1.8; // TP ยาวใน breakout
            break;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            multiplier = 0.6; // TP สั้นในตลาดผันผวน
            break;
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            multiplier = 0.8; // TP สั้นใน sideways เงียบ
            break;
...
double GetCounterTriggerForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    ...
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // ในเทรนด์แรง ยอมรับ drawdown มากกว่านิดหน่อย
            return baseTrigger * 1.3;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // ในตลาดผันผวน ใช้ trigger แคบ
...
int GetMaxCounterPositionsForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    ...
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // ในตลาดผันผวน ลดจำนวน counter positions
            return MathMax(1, baseMax / 2);
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // ในเทรนด์แรง อาจเพิ่ม counter positions ได้
...
// ฟังก์ชัน Helper สำหรับ counter positions
bool ShouldUseCounterPositionsInRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // ในตลาดผันผวน ไม่ควรใช้ counter positions
            return false;
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            // ใน breakout ระวังการใช้ counter positions
            return (CalculateMarketStability() > 0.7);
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // ในเทรนด์แรง ใช้ counter positions เฉพาะกับ pullback
...
{
    // คำนวณ market stability จาก ATR และ volatility
    double atrValue = GetATRValue(_Symbol, PERIOD_H1, 14, 0);
    double atrAvg = GetATRAverage(_Symbol, PERIOD_H1, 14, 20);
    
    if(atrAvg == 0) return 1.0;
    
    double stability = 1.0 - MathMin(MathAbs(atrValue - atrAvg) / atrAvg, 1.0);
...
bool IsInPullbackPhase()
{
    ...
    // ✅ ถูกต้อง: เรียกใช้ GetEMAValue ด้วย 4 พารามิเตอร์
    double ema20 = GetEMAValue(_Symbol, PERIOD_H1, 20, 0);
    ...
    // ✅ ถูกต้อง: GetATRValue ใช้ 4 พารามิเตอร์เช่นกัน
    double atrValue = GetATRValue(_Symbol, PERIOD_H1, 14, 0);
    ...
    // คำนวณระยะห่างจาก EMA20
    double distance = MathAbs(currentPrice - ema20);
    
    // ถ้าราคาอยู่ใกล้ EMA20 (ภายใน 0.5 * ATR)
    bool isNearEMA = (distance < atrValue * 0.5);
...
//+------------------------------------------------------------------+
// ✅ ใช้ฟังก์ชั่นนี้ (อันแรก) - มี implementation เต็ม
void ExecuteUltimateFightingRecovery()
{
   ...
   // ✅ ตรวจสอบ Ultimate Fighting Mode
   CheckUltimateFightingMode();
   
   if(currentFightingPhase == 0) return;
   
   // ✅ AGGRESSIVE TARGET MANAGEMENT - Fight to break even first
   double phaseProgress = (currentEquity - phaseStartEquity) / MathAbs(phaseStartEquity) * 100.0;
   
   // ✅ ปิดทั้งหมดเฉพาะเมื่อกำไรจริงและผู้ใช้ยืนยัน
   if(phaseProgress >= 25.0 && totalProfit > 0) // 25% recovery + กำไรรวมเป็นบวก
   {
      Print("🎉 STRONG RECOVERY ACHIEVED: " + DoubleToString(phaseProgress, 1) + "% in Phase " + IntegerToString(currentFightingPhase));
      Print("💡 Consider manual closure if desired");
      // ✅ ไม่ปิดอัตโนมัติ ให้ผู้ตัดสินใจ
   }
   
   // ✅ ULTIMATE RECOVERY LOGIC - More aggressive and persistent
   if(newBar)
   {
   ...
   // ✅ EMERGENCY COUNTER-ATTACK SYSTEM
...
void ManageStepUpRecovery()
{
      ...
      newBar)
   {
      ...
      Print("🔄 STEP-UP RECOVERY | Loss: $" + DoubleToString(totalPortfolioLoss, 2) +
...
void CloseAllPortfolioPositions(string callerName = "Unknown", string reason = "Not Specified")
{
       ...
       // 🚨 ALERT: แจ้งเตือนผู้เรียก
    Alert("🛑 CLOSE ALL POSITIONS ACTIVATED");
    Alert("📞 CALLED BY: " + callerName);
    Alert("🎯 REASON: " + reason);
    Alert("💼 POSITIONS: " + IntegerToString(PositionsTotal()));
    Alert("💰 EQUITY: $" + DoubleToString(AccountInfoDouble(ACCOUNT_EQUITY), 2));
   ...
   Print("?? Total closed: " + IntegerToString(closedCount) + " portfolio positions");
    ...
    // 🚨 ALERT: สรุปผล
    Alert("✅ CLOSE ALL COMPLETED");
    Alert("📊 CLOSED: " + IntegerToString(closedCount) + "/" + IntegerToString(total));
    Alert("💰 TOTAL P/L: $" + DoubleToString(totalClosedProfit, 2));
    Alert("👤 CALLED BY: " + callerName);
    Alert("🎯 REASON: " + reason);
}
...
// ✅ สร้างฟังก์ชันคำนวณ Drawdown ที่ถูกต้อง (ใช้ Equity Peak)
double CalculateTrueDrawdownPercent()
{
    ...
    // รีเซ็ต Equity Peak ทุกวันหรือเมื่อ balance เปลี่ยน (เช่น ฝาก/ถอน)
    if(equityPeak == 0 || currentBalance != AccountInfoDouble(ACCOUNT_BALANCE) || 
       TimeCurrent() - lastPeakReset > 86400) // 86400 วินาที = 1 วัน
    {
        equityPeak = currentEquity;
        lastPeakReset = TimeCurrent();
        Print("🔄 Equity Peak Reset: $" + DoubleToString(equityPeak, 2));
    }
    
    // อัพเดท Equity Peak ถ้า equity สูงขึ้น
    if(currentEquity > equityPeak)
    {
        equityPeak = currentEquity;
        Print("📈 New Equity Peak: $" + DoubleToString(equityPeak, 2));
    }
    ...
    drawdownPercent = MathMax(0, drawdownPercent); // ไม่ให้ค่าติดลบ
    
    return NormalizeDouble(drawdownPercent, 2);
}
// ✅ ฟังก์ชันจัดการ Safety Systems หลัก
void ManageSafetySystems()
{
    if(!UseRecoverySafety) 
    {
        // ✅ เมื่อปิด UseRecoverySafety -> ไม่ทำอะไรเลย
        static datetime lastWarning = 0;
        if(TimeCurrent() - lastWarning > 300) // แจ้งเตือนทุก 5 นาที
        {
            ...
            {
                Print("⚠️ DRAWDOWN WARNING: " + DoubleToString(currentDrawdown, 1) + 
                      "% | UseRecoverySafety: OFF - No auto closure");
            }
            
            if(totalProfit <= -MaxTotalLoss)
            {
                Print("⚠️ LOSS WARNING: $" + DoubleToString(totalProfit, 2) + 
                      " | UseRecoverySafety: OFF - No auto closure");
            }
            lastWarning = TimeCurrent();
        }
        return; // ❌ ออกเลยถ้า safety ปิด
    }

    // ✅ เมื่อเปิด UseRecoverySafety -> ตรวจสอบเงื่อนไขทั้งหมด
    double currentDrawdown = CalculateTrueDrawdownPercent();
    double totalProfit = CalculateTotalProfit();
    double currentEquity = AccountInfoDouble(ACCOUNT_EQUITY);
    
    // ✅ เงื่อนไข Safety Stop
    ...
    // ✅ ตรวจสอบว่าต้องหยุด Recovery หรือปิดออเดอร์ทั้งหมด
    ...
    if(TimeCurrent() - lastSafetyWarning > 60) // แจ้งเตือนทุก 1 นาที
    {
        if(currentDrawdown >= MaxDrawdownPercent * 0.8) // 80% ของขีดจำกัด
        {
            Print("⚠️ SAFETY WARNING: Drawdown " + DoubleToString(currentDrawdown, 1) + 
                  "% (Limit: " + DoubleToString(MaxDrawdownPercent, 1) + "%)");
        }
        
        if(totalProfit <= -MaxTotalLoss * 0.8) // 80% ของขีดจำกัดขาดทุน
        {
            Print("⚠️ SAFETY WARNING: Total Loss $" + DoubleToString(totalProfit, 2) + 
...
// ✅ ฟังก์ชันดำเนินการ Safety Actions
void ExecuteSafetyActions(bool drawdownTrigger, bool lossTrigger, bool safetyStopTrigger, bool equityTrigger, double drawdown, double totalLoss)
{
    ...
    // ✅ ตัดสินใจว่าจะปิดออเดอร์ทั้งหมดหรือแค่หยุด Recovery
    ...
    Print("✅ SAFETY ACTIONS COMPLETED");
    Print("💡 Close Positions: " + (closeAllPositions ? "YES" : "NO"));
    Print("💡 Stop Recovery: " + ((stopRecoveryOnly || closeAllPositions) ? "YES" : "NO"));
}

// ✅ ฟังก์ชันปิดออเดอร์ทั้งหมดแบบปลอดภัย
void CloseAllPositionsSafety(string callerName = "Unknown", string reason = "Not Specified")
{
   ...
   // 🚨 ALERT: แจ้งเตือนผู้เรียก
    ...
    Print("✅ SAFETY CLOSURE: Closed " + IntegerToString(closedCount) + 
          "/" + IntegerToString(totalPositions) + " positions");
    Print("💰 Total P/L from closure: $" + DoubleToString(totalClosedProfit, 2));
...
// ✅ ฟังก์ชันตรวจสอบ Consecutive Losses
void CheckConsecutiveLosses()
{

   if(!UseRecoverySafety || !AutoStopRecovery) 
    {
        return; // ❌ ไม่ทำอะไรถ้า safety ปิดหรือ AutoStopRecovery ปิด
    }
    ...
    // ตรวจจับการขาดทุนต่อเนื่อง (logic แบบง่าย)
    if(totalProfit < lastTotalProfit)
    {
        // ขาดทุนเพิ่มขึ้น
        currentLossStreak++;
        Print("📉 Consecutive Loss Streak: " + IntegerToString(currentLossStreak) + 
              "/" + IntegerToString(MaxConsecutiveLosses));
        
        if(currentLossStreak >= MaxConsecutiveLosses && AutoStopRecovery)
        {
            Print("🛑 MAX CONSECUTIVE LOSSES REACHED: " + IntegerToString(currentLossStreak));
            Print("?? Stopping recovery operations...");
            recoveryStopped = true;
            currentLossStreak = 0; // รีเซ็ตหลังจากหยุด
        }
    }
    else if(totalProfit > lastTotalProfit)
    {
        // กำไรเพิ่มขึ้น -> รีเซ็ต streak
        if(currentLossStreak > 0)
        {
            Print("✅ Consecutive losses reset - Profit improved");
            currentLossStreak = 0;
        }
    }
...
{
    // 1. ตรวจสอบสภาวะตลาดก่อน
    ENUM_MARKET_REGIME_DETAILED marketRegime = DetectMarketRegime(); // ใช้ ENUM
    double recoveryPressure = CalculateRecoveryRiskPressure();
    
    // 2. ใช้ Emergency Stop ที่มีอยู่
    if(recoveryPressure >= 1.0) {
        EmergencyRecoveryStop(); // ✅ เรียกฟังก์ชันที่มีอยู่
        return;
    }
    
    // 3. ปรับ Recovery ตามสภาวะตลาด
    switch(marketRegime) {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // ตลาดเทรนด์ชัด → Recovery แ Aggressive
        ...
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            // ตลาดเงียบ → Recovery แบบ Passive
        ...
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            // โอกาส Breakout → Recovery แบบ Opportunistic
            if(IsBreakoutLikely()) {
        ...
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // ตลาดผันผวน → Recovery แบบระมัดระวัง
            ExecuteCautiousRecovery();
            break;
            
        default: // MARKET_REGIME_NORMAL, MARKET_REGIME_TRENDING_WEAK_DETAILED
...
{
    Print("   💪 Executing Aggressive Recovery");
...
{
    Print("   🎯 Executing Opportunistic Recovery");
    // รอจังหวะดีๆ แล้วค่อย Recovery
}

void ExecuteCautiousRecovery()
{
    Print("   🛡️ Executing Cautious Recovery");
...
{
    Print("   🔄 Executing Standard Recovery");
    // Recovery แบบมาตรฐาน
}

// ✅ ฟังก์ชันตรวจสอบสถานะ safety systems
void PrintCompleteSafetyStatus()
{
    ...
    // ✅ Configuration
    Print("--- CONFIGURATION ---");
    Print("UseRecoverySafety: " + (UseRecoverySafety ? "ENABLED 🟢" : "DISABLED 🔴"));
    ...
    Print("Drawdown Limit: " + DoubleToString(currentDrawdown, 1) + "% / " + 
          ...
          (currentDrawdown >= MaxDrawdownPercent ? "EXCEEDED 🔴" : "OK 🟢"));
    
    Print("Total Loss: $" + DoubleToString(totalProfit, 2) + " / $" + 
          DoubleToString(-MaxTotalLoss, 2) + " - " + 
          (totalProfit <= -MaxTotalLoss ? "EXCEEDED 🔴" : "OK 🟢"));
    
    Print("Safety Stop: $" + DoubleToString(totalProfit, 2) + " / $" + 
          DoubleToString(-SafetyStopThreshold, 2) + " - " + 
          (totalProfit <= -SafetyStopThreshold ? "EXCEEDED 🔴" : "OK 🟢"));
    
    Print("Margin Level: " + DoubleToString(marginLevel, 1) + "% - " + 
          (marginLevel < 100.0 ? "CRITICAL 🔴" : "OK 🟢"));
    
    // ✅ System Status
    Print("--- SYSTEM STATUS ---");
    if(!UseRecoverySafety)
    {
        Print("🚫 ALL SAFETY SYSTEMS: DISABLED");
        Print("💡 No automatic position closure will occur");
        Print("💡 Manual trading only - System shows warnings only");
    }
    else
    {
        Print("🛡️ RECOVERY SAFETY: FULLY OPERATIONAL");
        if(AutoStopRecovery)
        {
            Print("💡 Auto Stop Recovery: ENABLED");
        }
        Print("💡 Will close positions if:");
        Print("   • Drawdown exceeds " + DoubleToString(MaxDrawdownPercent, 1) + "%");
        Print("   • Total loss exceeds $" + DoubleToString(MaxTotalLoss, 2));
        Print("   • Margin level below 100%");
    }
}
void CheckAndPrintSafetyStatus()
{
    static datetime lastPrint = 0;
    if(TimeCurrent() - lastPrint > 600) // พิมพ์ทุก 10 นาที
    {
...
// ถ้าต้องการฟังก์ชันใหม่ ให้เปลี่ยนชื่อ
string DetectMarketRegimeForRecovery()
{
    // โค้ดใหม่เฉพาะสำหรับ Recovery
    double volatility = CalculateVolatility();
    double trendStrength = CalculateTrendStrength();
...
void CheckAndCloseAllPositions()
{
   ...
   // ✅ ตรวจสอบว่ากำไรรวมเป็นบวกก่อนปิดตำแหน่ง
   if(totalProfit >= RecoveryTarget && totalProfit > 0) 
   {
      Print("🎯 PORTFOLIO TARGET HIT - Closing ALL positions!");
      Print("💰 Total Portfolio Profit: $" + DoubleToString(totalProfit, 2));
   ...
   // ✅ ตรวจสอบกำไรของ recovery positions เป็นบวกก่อนปิด
   if(recoveryProfit >= RecoveryTarget * 0.6 && recoveryLayerCount >= 2 && recoveryProfit > 0) {
      Print("✅ RECOVERY PROFIT TARGET - Closing recovery positions");
   ...
   // ✅ ปิดที่จุดคุ้มทุนเมื่อกำไรรวมเป็นบวกหรือเป็นศูนย์
   if(totalProfit >= 0 && recoveryLayerCount >= 2) {
      Print("⚖️ PORTFOLIO BREAK-EVEN - Closing all positions");
      CloseAllPortfolioPositions("CheckAndCloseAllPositions", "PORTFOLIO BREAK-EVEN - Closing all positions");
      PrintRecoverySummary();
      ResetRecoverySystem();
   }
   
   // ✅ ป้องกันการปิดเมื่อขาดทุนในกรณีปรับเป้าหมาย
   if(recoveryLayerCount >= 3 && totalProfit < -RecoveryTarget * 0.5)
   {
      double newTarget = MathAbs(totalProfit) * 1.2; 
      Print("🎯 AUTO-TARGET ADJUST: New target: $" + DoubleToString(newTarget, 2));
      
      // ✅ ตรวจสอบว่ากำไรเป็นบวกก่อนปิด
      if(totalProfit >= 0) 
      {
         Print("✅ ADJUSTED TARGET HIT - Closing all portfolio positions");
         CloseAllPortfolioPositions("CheckAndCloseAllPositions", "totalProfit >= -newTarget");
         ResetRecoverySystem();
      }
   }
   
   // ✅ เพิ่มเงื่อนไขป้องกันการปิดขาดทุน
   if(totalProfit < 0) {
      Print("⏸️ Portfolio is in loss. Waiting for profit before closing positions.");
      Print("📉 Current Loss: $" + DoubleToString(totalProfit, 2));
      return;
   }
}
// ✅ ฟังก์ชันใหม่สำหรับตรวจสอบ Ultimate Fighting Mode
void CheckUltimateFightingMode()
{
   ...
   // ✅ ตรวจสอบการเปลี่ยน Phase โดยไม่ปิดออเดอร์
   int newPhase = 0;
   if(drawdownPercent >= Phase5Trigger) newPhase = 5;
   else if(drawdownPercent >= Phase4Trigger) newPhase = 4;
   else if(drawdownPercent >= Phase3Trigger) newPhase = 3;
   ...
   if(newPhase > currentFightingPhase)
   {
      ...
      // ✅ ไม่ปิดออเดอร์ที่มีอยู่ แต่เพิ่มการ Recovery
      ExecuteEnhancedRecoveryInFightingMode();
   }
   
   // ✅ ตรวจสอบความคืบหน้าในแต่ละ Phase
   CheckFightingPhaseProgress();
}
// ✅ ฟังก์ชันสำหรับ Recovery ในโหมด Fighting (ไม่ปิดออเดอร์)
void ExecuteEnhancedRecoveryInFightingMode()
{
   ...
   // ✅ เปิด Recovery เพิ่มเติมแทนการปิดออเดอร์
   if(totalLoss <= -(RecoveryTrigger * aggression) && recoveryLayerCount < localMaxSteps)
   {
      Print("🔄 ENHANCED RECOVERY IN FIGHTING MODE | Phase: " + IntegerToString(currentFightingPhase));
      OpenUltimateRecovery(aggression);
   }
}
// ✅ ตรวจสอบความคืบหน้าในแต่ละ Phase
void CheckFightingPhaseProgress()
{
   ...
   // ✅ ถ้าฟื้นตัวได้ดีใน Phase นี้
   if(phaseProgress >= 15.0)
   {
      Print("🎉 PHASE " + IntegerToString(currentFightingPhase) + " RECOVERY PROGRESS: " + DoubleToString(phaseProgress, 1) + "%");
      // ✅ ไม่ปิดออเดอร์อัตโนมัติ รอให้ถึงเป้าหมายจริง
   }
   
   // ✅ ถ้าฟื้นตัวกลับมาได้มากกว่า 50% ของ Phase Start
   if(phaseProgress >= 50.0)
   {
      Print("🚀 EXCELLENT RECOVERY IN PHASE " + IntegerToString(currentFightingPhase) + "!");
      // ✅ สามารถลด aggression ได้ แต่ไม่ปิดออเดอร์
      currentFightingPhase = MathMax(1, currentFightingPhase - 1);
      Print("📉 Downgrading to Phase " + IntegerToString(currentFightingPhase) + " due to excellent recovery");
   }
}
// ✅ ฟังก์ชันป้องกันการปิดออเดอร์โดยไม่ได้ตั้งใจ
void SafeCloseAllPositions(string callerName, string reason)
{
   double totalProfit = CalculateTotalProfit();
   
   // ✅ ตรวจสอบเงื่อนไขความปลอดภัยก่อนปิด
   if(totalProfit < 0)
   {
      Print("🚫 SAFETY BLOCK: Attempt to close positions while in loss prevented!");
      Print("💡 Caller: " + callerName + " | Reason: " + reason);
      Print("📉 Current Loss: $" + DoubleToString(totalProfit, 2));
      return;
   }
   
   // ✅ ตรวจสอบว่ากำไรถึงเกณฑ์ขั้นต่ำ
   double minProfitThreshold = RecoveryTarget * 0.8; // 80% ของ target
   if(totalProfit < minProfitThreshold)
   {
      Print("⚠️ PROFIT TOO LOW: $" + DoubleToString(totalProfit, 2) + " < $" + DoubleToString(minProfitThreshold, 2));
      Print("💡 Required minimum profit not reached for closure");
      return;
   }
   
   // ✅ ปิดออเดอร์เฉพาะเมื่อผ่านเงื่อนไขความปลอดภัยทั้งหมด
    ...
    for(int i = total-1; i >= 0; i--)
    {
        ...
        if(PositionSelectByTicket(ticket))
        {
            ...
            if(trade.PositionClose(ticket))
            {
                closedCount++;
                totalClosedProfit += profit;
                Print("🔻 CLOSED by " + callerName + ": " + symbol + " | P/L: $" + DoubleToString(profit, 2));
            }
        }
    }
...
void AdaptToMarketRegime()
{
    ...
    switch(regime) {
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            ExecuteRangingCalmPlan();      // ลดกิจกรรม รอสัญญาณ
            break;
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            ExecuteTrendingStrongPlan();   // ลงทุนเต็มที่ตามเทรนด์
            break;
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            ExecuteBreakoutPotentialPlan(); // เตรียมจับ Breakout
            break;
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            ExecuteRangingVolatilePlan();  // กลยุทธ์สำหรับตลาดผันผวน
            break;
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            ExecuteTrendingWeakPlan();     // กลยุทธ์สำหรับเทรนด์อ่อน
            break;
        default: // MARKET_REGIME_NORMAL
            ExecuteNormalPlan();           // กลยุทธ์ปกติ
    }
}

void ExecuteNormalPlan()
{
    Print("📊 EXECUTING NORMAL PLAN - Market Adaptive Strategy");
    
    // 1. ตั้งค่ามาตรฐาน
    Current_TrendScalp_TP_Points = Original_Trend_TP;
    Current_Scalp1_Lot_Multiplier = Original_Scalp1_Multiplier;
    
    // 2. ปรับตามโมเมนตัมเล็กน้อย
    double netMomentum = GetNetMomentum();
    if(netMomentum > 5) {
        // โมเมนตัมบวกเล็กน้อย → เน้น Buy นิดหน่อย
        AssignTeamWeights(35, 20, 25, 12, 8);
        Print("   🟢 Slightly favoring BUY teams");
    } else if(netMomentum < -5) {
        // โมเมนตัมลบเล็กน้อย → เน้น Sell นิดหน่อย
        AssignTeamWeights(20, 35, 25, 12, 8);
        Print("   🔴 Slightly favoring SELL teams");
    } else {
        // โมเมนตัมสมดุล → กระจายเท่าๆ กัน
        AssignTeamWeights(30, 25, 20, 15, 10);
        Print("   ⚖️ Balanced team distribution");
    }
    
    // 3. ใช้ recovery settings ปกติ
    ResetRecoveryModes(); // คืนค่าทั้งหมดเป็นปกติ
    
    Print("   📈 Standard trading parameters activated");
}
...
void ResetRecoveryModes()
{
    ...
    currentEnableCounterPosition = EnableCounterPosition; // ค่าจาก input
}

void ExecuteRangingVolatilePlan()
{
    Print("🎭 EXECUTING RANGING VOLATILE PLAN");
    
    // 1. เน้นทีม Scalp และ Counter
    Current_Scalp1_Lot_Multiplier = Original_Scalp1_Multiplier * 1.2; // +20%
    
    // 2. กระจายทีมให้ครอบคลุมทั้งสองฝั่ง
    AssignTeamWeights(15, 15, 40, 20, 10); // Scalp 70%
    
    // 3. เปิดใช้งาน Counter Team เต็มที่
    EnableCounterPositionMode(true);
    
    // 4. Recovery แบบรวดเร็ว
    EnableQuickRecovery(true);
    
    Print("   ⚡ Quick scalp & counter strategies activated");
}


void AdjustStrategyByRegime(ENUM_MARKET_REGIME_DETAILED regime) 
{
    switch(regime)  // ✅ ถูกต้อง: ใช้แค่ชื่อตัวแปร ไม่ต้องมี type
    {
        case MARKET_REGIME_RANGING_CALM_DETAILED:        // 1
            // Sideways เงียบ
            g_TP_Multiplier = 0.8;      // TP สั้นลง
            g_Lot_Multiplier = 1.2;     // อาจเพิ่ม lot นิดหน่อย
            g_Max_Orders = 5;           // จำนวนออเดอร์สูงสุด
            g_Use_Grid = true;          // ใช้ grid ได้
            g_Trend_Only = false;
            g_Strict_SL = false;
            Print("Strategy adjusted for RANGING_CALM market");
            break;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:    // 2
            // ผันผวนใน range
            g_TP_Multiplier = 0.7;      // TP สั้นมาก
            g_Lot_Multiplier = 0.7;     // ลด lot
            g_Max_Orders = 4;           // ปานกลาง
            g_Use_Grid = true;
            g_Trend_Only = false;
            g_Strict_SL = true;         // SL แคบ
            Print("Strategy adjusted for RANGING_VOLATILE market");
            break;
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:       // 3
            // เทรนด์อ่อน
            g_TP_Multiplier = 1.2;      // TP ปานกลาง
            g_Lot_Multiplier = 1.0;     // lot ปกติ
            g_Max_Orders = 4;           // ปานกลาง
            g_Use_Grid = false;
            g_Trend_Only = true;
            g_Strict_SL = false;
        ...
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:     // 4
            // เทรนด์แรง
            g_TP_Multiplier = 1.5;      // TP ยาวขึ้น
            g_Lot_Multiplier = 1.0;     // lot ปกติ
            g_Max_Orders = 3;           // น้อยแต่มีคุณภาพ
            g_Use_Grid = false;         // ไม่ใช้ grid
            g_Trend_Only = true;        // เทรดตามเทรนด์เท่านั้น
            g_Strict_SL = false;
            Print("Strategy adjusted for TRENDING_STRONG market");
            break;
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:  // 5
            // โอกาส breakout
            g_TP_Multiplier = 2.0;      // TP ยาวมาก
            g_Lot_Multiplier = 0.8;     // ลด lot เพราะเสี่ยง
            g_Max_Orders = 2;           // น้อยๆ
            g_Use_Grid = false;
            g_Trend_Only = false;
            g_Strict_SL = true;
        ...
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:     // 6
            // ตลาดผันผวนสูง
            g_TP_Multiplier = 1.0;      // ปกติ
            g_Lot_Multiplier = 0.5;     // ลด lot มาก
            g_Max_Orders = 1;           // น้อยที่สุด
            g_Use_Grid = false;
            g_Trend_Only = false;
            g_Strict_SL = true;
            Print("Strategy adjusted for HIGH_VOLATILITY market");
            break;
            
        case MARKET_REGIME_NEWS_EVENT_DETAILED:          // 7
            // ช่วงข่าว
            g_TP_Multiplier = 0.5;      // TP สั้นมาก
            g_Lot_Multiplier = 0.3;     // ลด lot มากที่สุด
            g_Max_Orders = 0;           // หยุดเปิดออเดอร์ใหม่
            g_Use_Grid = false;
            g_Trend_Only = false;
            g_Strict_SL = true;
            Print("Strategy adjusted for NEWS_EVENT market");
            break;
            
        case MARKET_REGIME_UNKNOWN:             // 0
        default:
            // สภาพปกติหรือไม่รู้จัก
            g_TP_Multiplier = 1.0;      // ปกติ
            g_Lot_Multiplier = 1.0;     // ปกติ
            g_Max_Orders = 5;           // ปกติ
            g_Use_Grid = true;
            g_Trend_Only = false;
            g_Strict_SL = false;
            Print("Strategy adjusted for NORMAL/UNKNOWN market");
            break;
    }
    
    // แสดงค่าที่ปรับแล้ว
    DisplayStrategySettings();
}
// ฟังก์ชันแสดงค่า strategy settings
void DisplayStrategySettings()
{
    if(EnableDebugMode) {
        Print("=== STRATEGY SETTINGS ===");
        Print("TP Multiplier: ", g_TP_Multiplier);
        Print("Lot Multiplier: ", g_Lot_Multiplier);
        Print("Max Orders: ", g_Max_Orders);
        Print("Use Grid: ", g_Use_Grid ? "Yes" : "No");
        Print("Trend Only: ", g_Trend_Only ? "Yes" : "No");
        Print("Strict SL: ", g_Strict_SL ? "Yes" : "No");
        Print("=========================");
    }
}
// ฟังก์ชันช่วยเหลือ
string GetRegimeName(ENUM_MARKET_REGIME_DETAILED regime)
{
   switch(regime) {
      case MARKET_REGIME_RANGING_CALM_DETAILED: return "RANGING_CALM";
      case MARKET_REGIME_TRENDING_STRONG_DETAILED: return "TRENDING_STRONG";
      case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED: return "BREAKOUT_POTENTIAL";
      case MARKET_REGIME_RANGING_VOLATILE_DETAILED: return "RANGING_VOLATILE";
      case MARKET_REGIME_TRENDING_WEAK_DETAILED: return "TRENDING_WEAK";
...
void ExecuteSmartRecovery()
{
    ...
    if(recoveryPressure >= 1.0) {
        EmergencyRecoveryStop(); // ภาวะฉุกเฉิน
        return;
    }
    ...
    switch(regime) {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // ตลาดเทรนด์ชัด → Recovery แ Aggressive
            if(g_netMomentum > 0) {
                OpenRecoveryPosition(POSITION_TYPE_BUY, 1.8); // Multiplier สูง
            } else {
        ...
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            // ตลาดเงียบ → Recovery แบบ Passive
            EnablePassiveRecovery(true);
            WaitForBetterConditions(); // รอจังหวะดีๆ
            break;
        ...
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            // เทรนด์อ่อน → Recovery แบบมาตรฐาน
            ExecuteStandardRecovery();
            break;
...
ENUM_TREND_DIRECTION AnalyzeTrendOnTimeframe(ENUM_TIMEFRAMES tf)
{
    ...
    // EMA เรียงตัวขึ้น (Fast > Medium > Slow)
    bool bullishAlignment = (emaFast > emaMedium && emaMedium > emaSlow);
    bool priceAboveEMA = (currentPrice > emaMedium); // ใช้ EMA medium เป็น reference
    
    // EMA เรียงตัวลง (Fast < Medium < Slow)
    bool bearishAlignment = (emaFast < emaMedium && emaMedium < emaSlow);
    bool priceBelowEMA = (currentPrice < emaMedium); // ใช้ EMA medium เป็น reference
    
    if(bullishAlignment && priceAboveEMA) return TREND_UP;
    if(bearishAlignment && priceBelowEMA) return TREND_DOWN;
    return TREND_SIDEWAYS;
}
// ===== ฟังก์ชั่นคำนวณความแข็งแกร่งของเทรนด์ =====
// เพิ่มฟังก์ชั่น iRSI นี้ในไฟล์ของคุณ
...
bool ShouldTradeBasedOnTrend(ENUM_TREND_DIRECTION trend, double strength)
{
    ...
    // 🔥 สำหรับ MQL5 - ถูกต้องแล้ว!
    double rsiValue = iRSI(_Symbol, PERIOD_M1, 14, PRICE_CLOSE, 0);
    
    if(trend == TREND_UP)
    {
        if(!(rsiValue > 30 && rsiValue < 85)) 
        {
            DebugPrint(3, "RSI not in BUY range: " + DoubleToString(rsiValue, 1));
            return false;
        }
    }
    else if(trend == TREND_DOWN)
    {
        if(!(rsiValue < 70 && rsiValue > 15)) 
        {
            DebugPrint(3, "RSI not in SELL range: " + DoubleToString(rsiValue, 1));
            return false;
        }
    }
...
// ===== การคำนวณล็อตตามความแข็งแกร่งของเทรนด์ =====
double CalculateTrendBasedLot(double trendStrength)
{
    double baseLot = BaseLot;
    
    // ปรับล็อตตามความแข็งแกร่งของเทรนด์
    if(trendStrength > 7.0) 
        baseLot *= 1.5; // เทรนด์แรง → ล็อตใหญ่ขึ้น
    else if(trendStrength > 5.0) 
        baseLot *= 1.2; // เทรนด์ปานกลาง
    else if(trendStrength < 3.0)
        baseLot *= 0.8; // เทรนด์อ่อน → ล็อตเล็กลง
    
    baseLot = MathMin(baseLot, MaxLot);
    baseLot = MathMax(baseLot, BaseLot);  // ใช้ BaseLot แทน MinLot
...
{
    double tpPoints = Current_TrendScalp_TP_Points; // ใช้พารามิเตอร์ของ Trend Scalp
    double slPoints = TrendScalp_SL_Points; // ใช้พารามิเตอร์ของ Trend Scalp
    
    double tpPrice = GetTPPriceUniversal(true, tpPoints);
    double slPrice = GetSLPriceUniversal(true, slPoints);
    
    // ปรับ TP/SL ตามความแข็งแกร่งของเทรนด์
    double trendStrength = CalculateTrendStrength();
    if(trendStrength > 7.0) 
    {
        // เทรนด์แรง → TP ไกลขึ้น
        tpPrice = GetTPPriceUniversal(true, tpPoints * 1.3);
        DebugPrint(2, "Strong trend detected - Increasing TP by 30%");
    }
    else if(trendStrength < 3.0)
    {
        // เทรนด์อ่อน → TP สั้นลง
    ...
    if(trade.Buy(lot, _Symbol, slPrice, 0, tpPrice, "TrendScalp-Buy"))
     {
    ulong ticket = trade.ResultOrder();
    Print("🎯 TREND SCALP BUY | Trend: " + GetTrendDirectionName(TREND_UP) + 
          " | Strength: " + DoubleToString(trendStrength, 1) + 
          " | Lot: " + DoubleToString(lot, 2) + 
          " | TP: " + DoubleToString(tpPoints, 1) + "pips" +  // เปลี่ยน IntegerToString เป็น DoubleToString
          " | Ticket: #" + TicketToStr(ticket));
      }
    else
    {
        Print("❌ Trend Scalp BUY Error: " + trade.ResultRetcodeDescription());
    }
}
...
void ExecuteTrendSell(double lot)
{
    ...
    if(trade.Sell(lot, _Symbol, slPrice, 0, tpPrice, "TrendScalp-Sell"))
      {
    ulong ticket = trade.ResultOrder();
    Print("🎯 TREND SCALP SELL | Trend: " + GetTrendDirectionName(TREND_DOWN) + 
          " | Strength: " + DoubleToString(trendStrength, 1) + 
          " | Lot: " + DoubleToString(lot, 2) + 
          " | TP: " + DoubleToString(tpPoints, 1) + "pips" +  // เปลี่ยน IntegerToString เป็น DoubleToString
          " | Ticket: #" + TicketToStr(ticket));
    ...
    {
        Print("❌ Trend Scalp SELL Error: " + trade.ResultRetcodeDescription());
    }
}
// ===== นับออเดอร์ในทิศทางเทรนด์ =====
int CountPositionsInTrendDirection(ENUM_TREND_DIRECTION trend)
{
    ...
    for(int i = 0; i < PositionsTotal(); i++)
    {
        ...
        if(PositionSelectByTicket(ticket))
        {
            if(PositionGetInteger(POSITION_MAGIC) == MAGIC_TREND_SCALP) // ใช้ Magic ของ Trend Scalp
            {
                ENUM_POSITION_TYPE type = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
...
void BroadcastToAllSystems(string message) {
   Print("📢 SYSTEM BROADCAST: " + message);
   
   // บันทึกข้อความล่าสุดสำหรับระบบอื่นๆ
   LastSystemMessage = message;
   MessageTime = TimeCurrent();
}

// ฟังก์ชั่นตรวจสอบข้อความใหม่
bool HasNewSystemMessage() {
   return (TimeCurrent() - MessageTime) < 5; // 5 วินาที
}

//+------------------------------------------------------------------+
//| ExecuteScalp1 - Scalp1 Strategy Deepseek                         |
//+------------------------------------------------------------------+
void ExecuteScalp1(ENUM_MARKET_REGIME_DETAILED regime = MARKET_REGIME_UNKNOWN)
{
    // ตรวจจับ regime ถ้าไม่ได้ส่งมา
    if(regime == MARKET_REGIME_UNKNOWN) 
    {
        regime = DetectMarketRegime();
    }
    
    // ตรวจสอบโหมดระบบก่อนเทรด
    if(SystemMode == MODE_RECOVERY) 
    {
        DebugPrint(2, "Scalp1: Reducing activity in Recovery mode");
        // ตรวจสอบว่า regime นี้ปลอดภัยสำหรับ recovery หรือไม่
    ...
    // ตรวจสอบว่า Scalp1 ควรทำงานใน regime นี้หรือไม่
    if(!IsScalp1AllowedInRegime(regime)) 
    {
        DebugPrint(2, "Scalp1 not allowed in current market regime: " + RegimeToString(regime));
        return;
    }
    
    // 🔍 DEBUG START - วางก่อนโค้ดเดิม
    if(EnableDebugMode) 
    {
        Print("=== 🎯 SCALP1 DEBUG START ===");
        Print("Market Regime: " + RegimeToString(regime));
    ...
    // ใช้ตัวแปรเดิมที่มีอยู่แล้ว
    double ema9 = iMA(_Symbol, PERIOD_M1, 9, 0, MODE_EMA, PRICE_CLOSE);
    double ema21 = iMA(_Symbol, PERIOD_M1, 21, 0, MODE_EMA, PRICE_CLOSE);
    double rsiValue = iRSI(_Symbol, PERIOD_M1, 14, PRICE_CLOSE);
    
    if(EnableDebugMode) 
    {
        Print("6. EMA9: ", ema9, " | EMA21: ", ema21, " | RSI: ", rsiValue);
    }
    
    // 🔥 ปรับเงื่อนไข entry ตาม market regime
    bool buyCondition = GetScalp1BuyCondition(ema9, ema21, rsiValue, regime);
    bool sellCondition = GetScalp1SellCondition(ema9, ema21, rsiValue, regime);
    
    if(EnableDebugMode) 
    {
        Print("7. Buy Condition: ", buyCondition);
        Print("8. Sell Condition: ", sellCondition);
        Print("=== 🎯 SCALP1 DEBUG END ===");
    }
    // 🔍 DEBUG END
    ...
    {
        DebugPrint(3, "Max Scalp1 positions reached for " + RegimeToString(regime) + 
                  ": " + IntegerToString(scalp1Count) + "/" + IntegerToString(allowedPositions));
        return;
    }

    DebugPrint(3, "Starting Scalp1 analysis for regime: " + RegimeToString(regime));
    
    DebugPrint(3, "Indicators - EMA9: " + DoubleToString(ema9, 5) + 
                ", EMA21: " + DoubleToString(ema21, 5) + 
                ", RSI: " + DoubleToString(rsiValue, 1));
    
    // เพิ่ม Price Action confirmation
    MqlRates rates[];
    ...
    {
        DebugPrint(3, "No valid Scalp1 signal found for " + RegimeToString(regime));
        return;
    }
    
    // 🔥 ปรับ lot size ตาม regime
    ...
    if(buyCondition)
    {
        // 🔥 ใช้ TP/SL ที่ปรับตาม regime
        double tpPoints = MathMax(Scalp1_TP_Points * tpMultiplier, 5); // อย่างน้อย 5 points
        double slPoints = MathMax(Scalp1_SL_Points * slMultiplier, 10); // อย่างน้อย 10 points
        
        double tpPrice = GetTPPriceUniversal(true, (int)tpPoints);
        double slPrice = GetSLPriceUniversal(true, (int)slPoints);
        
        DebugPrint(2, "Scalp1 BUY Signal - Regime: " + RegimeToString(regime) + 
                   ", Lot: " + DoubleToString(lot, 2) + 
                   ", TP: " + DoubleToString(tpPrice, GetDigits()) + 
                   " (" + DoubleToString(tpMultiplier, 2) + "x)");
                   
        if(trade.Buy(lot, _Symbol, slPrice, 0, tpPrice, "Scalp1-Buy-" + RegimeToString(regime)))
        {
            ulong ticket = trade.ResultOrder();
            Print("✅ SCALP1 BUY | Regime: " + RegimeToString(regime) + 
                  " | Ticket: #" + TicketToStr(ticket) + 
                  " | Lot: " + DoubleToString(lot, 2) + 
        ...
        {
            Print("❌ Scalp1 BUY Error: " + trade.ResultRetcodeDescription());
        }
    }
    else if(sellCondition)
    {
        ...
        DebugPrint(2, "Scalp1 SELL Signal - Regime: " + RegimeToString(regime) + 
                   ", Lot: " + DoubleToString(lot, 2) + 
                   ", TP: " + DoubleToString(tpPrice, GetDigits()) + 
                   " (" + DoubleToString(tpMultiplier, 2) + "x)");
                   
        if(trade.Sell(lot, _Symbol, slPrice, 0, tpPrice, "Scalp1-Sell-" + RegimeToString(regime)))
        {
            ulong ticket = trade.ResultOrder();
            Print("✅ SCALP1 SELL | Regime: " + RegimeToString(regime) + 
                  " | Ticket: #" + TicketToStr(ticket) + 
                  " | Lot: " + DoubleToString(lot, 2) + 
                  " | TP: " + IntegerToString((int)tpPoints) + "pips" +
                  " | Multiplier: " + DoubleToString(lotMultiplier, 2) + "x");
        ...
        {
            Print("❌ Scalp1 SELL Error: " + trade.ResultRetcodeDescription());
        }
    }
}
// 🔥 ฟังก์ชัน Helper สำหรับ Scalp1 ตาม Market Regime
bool IsScalp1AllowedInRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // ในตลาดผันผวน อาจปิด Scalp1 หรือใช้ strict rules
            return EnableScalp1InVolatileMarket;
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            // ใน breakout รอ confirmation
            return EnableScalp1InBreakout;
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // ในเทรนด์แรง ใช้เฉพาะกับ trend direction
...
bool GetScalp1BuyCondition(double ema9, double ema21, double rsiValue, ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // ในเทรนด์แรง ใช้ RSI range แคบกว่า
            return (ema9 > ema21 && rsiValue > 55 && rsiValue < 75);
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // ในตลาดผันผวน ใช้ RSI range แคบและต้องการ divergence
            return (ema9 > ema21 && rsiValue> 60 && rsiValue < 70);
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            // ใน breakout ต้องการ confirmation มากขึ้น
            return (ema9 > ema21 && rsiValue > 65 && rsiValue < 80);
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            // ใน sideways เงียบ ใช้ RSI range กว้างกว่า
            return (ema9 > ema21 && rsiValue > 40 && rsiValue < 80);
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            // ในเทรนด์อ่อน ใช้ RSI range ปานกลาง
            return (ema9 > ema21 && rsiValue > 50 && rsiValue < 75);
            
        case MARKET_REGIME_NORMAL:
        default:
            // เงื่อนไขเดิม
            return (ema9 > ema21 && rsiValue > 35 && rsiValue < 80);
    }
}

bool GetScalp1SellCondition(double ema9, double ema21, double rsiValue, ENUM_MARKET_REGIME_DETAILED regime)
// ✅ เปลี่ยนจาก 'rsi' เป็น 'rsiValue'
{
    switch(regime)
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return (ema9 < ema21 && rsiValue < 45 && rsiValue > 25);
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return (ema9 < ema21 && rsiValue < 40 && rsiValue > 30);
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return (ema9 < ema21 && rsiValue < 35 && rsiValue > 20);
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            return (ema9 < ema21 && rsiValue < 60 && rsiValue > 20);
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            return (ema9 < ema21 && rsiValue < 50 && rsiValue > 25);
            
        case MARKET_REGIME_NORMAL:
        default:
            return (ema9 < ema21 && rsiValue < 65 && rsiValue > 20);
    }
}
int GetScalp1MaxPositionsForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    int baseMax = Scalp1_MaxPositions;
    ...
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return MathMax(1, baseMax / 2); // ลดครึ่งหนึ่ง
        
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return MathMax(2, (int)(baseMax * 0.7)); // ลด 30%
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return MathMin(baseMax + 1, 10); // อาจเพิ่มได้ 1 ตำแหน่ง
...
double GetScalp1LotMultiplierForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 0.5; // ลด lot 50%
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return 0.7; // ลด lot 30%
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 1.2; // เพิ่ม lot 20%
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            return 1.0; // ปกติ
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            return 0.9; // ลดเล็กน้อย
...
double GetScalp1TPMultiplierForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 1.5; // TP ยาวขึ้นในเทรนด์แรง
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 0.7; // TP สั้นลงในตลาดผันผวน
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return 2.0; // TP ยาวมากใน breakout
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            return 0.8; // TP สั้นลงใน sideways เงียบ
...
double GetScalp1SLMultiplierForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 0.8; // SL แคบลงในตลาดผันผวน
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 1.3; // SL กว้างขึ้นในเทรนด์แรง
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return 1.5; // SL กว้างใน breakout
...
bool ShouldUsePriceActionForBuy(ENUM_MARKET_REGIME_DETAILED regime, bool bullishCandle)
{
    switch(regime)
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return bullishCandle; // ต้องการ bullish candle ในตลาดผันผวน
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return true; // ใน breakout อาจไม่ต้องการ candle confirmation เสมอไป
            
        default:
            return true; // ใช้ตามปกติ
    }
}

bool ShouldUsePriceActionForSell(ENUM_MARKET_REGIME_DETAILED regime, bool bearishCandle)
{
    switch(regime)
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
...
bool IsRegimeSafeForRecovery(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return false; // ไม่ปลอดภัยในตลาดผันผวน
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return false; // ไม่ปลอดภัยใน breakout
            
        default:
            return true;
    }
}
bool IsBreakoutStrategySuitableForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return true; // เหมาะสมที่สุด
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return true; // ใช้ breakout ใน trending ได้
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return true; // ใช้ได้ใน ranging volatile
            
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            return false; // ไม่ควรใช้ในตลาดเสี่ยงสูง
            
        default:
            return false; // ไม่ใช้ในสภาวะอื่น
    }
}

double GetBreakoutTPMultiplierForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return 1.5; // TP ยาวขึ้นใน breakout
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 1.3; // TP ยาวใน trending strong
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 0.8; // TP สั้นลงใน ranging volatile
            
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
            return 2.0; // TP ยาวมากใน high volatility
...
double GetBreakoutSLMultiplierForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 0.8; // SL แคบในตลาดผันผวน
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return 1.3; // SL กว้างใน breakout
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 1.2; // SL กว้างในเทรนด์แรง
...
//+------------------------------------------------------------------+
void ExecuteAdvancedBreakout(ENUM_MARKET_REGIME_DETAILED regime = MARKET_REGIME_UNKNOWN)
{
    // ตรวจจับ regime ถ้าไม่ได้ส่งมา
    if(regime == MARKET_REGIME_UNKNOWN) 
    {
    ...
    // ตรวจสอบว่า breakout strategy เหมาะสมกับ market regime นี้ไหม
    if(!IsBreakoutStrategySuitableForRegime(regime))
    {
        DebugPrint(2, "Breakout strategy not suitable for " + RegimeToString(regime) + " regime");
        return;
    }
    
    DebugPrint(3, "Starting Breakout analysis for regime: " + RegimeToString(regime));
    
    // ตรวจสอบว่ามี breakout position อยู่แล้วหรือไม่
    ...
    // ปรับ parameters ของ breakout detection ตาม regime
    bool buySignal = DetectAdvancedBreakoutBUY(regime);
    bool sellSignal = DetectAdvancedBreakoutSELL(regime);

    DebugPrint(3, "Breakout Signals for " + RegimeToString(regime) + 
    ...
    if(buySignal)
    {
        ...
        DebugPrint(2, "Breakout BUY Signal - Regime: " + RegimeToString(regime) + 
                   ", Lot: " + DoubleToString(lot, 2) + 
                   ", TP: " + DoubleToString(tpPrice, GetDigits()) +
                   " (" + DoubleToString(tpMultiplier, 2) + "x)");
        
        if(trade.Buy(lot, _Symbol, slPrice, 0, tpPrice, "Adv-Breakout-Buy-" + RegimeToString(regime)))
        {
            Print("✅ Advanced Breakout BUY | Regime: " + RegimeToString(regime) + 
                  ", Lot: " + DoubleToString(lot, 2) + 
        ...
        else
        {
            ...
            Print("❌ Breakout BUY Error: " + IntegerToString(trade.ResultRetcode()));
        }
    }
    else if(sellSignal)
    {
        ...
        DebugPrint(2, "Breakout SELL Signal - Regime: " + RegimeToString(regime) + 
                   ", Lot: " + DoubleToString(lot, 2) + 
                   ", TP: " + DoubleToString(tpPrice, GetDigits()) +
                   " (" + DoubleToString(tpMultiplier, 2) + "x)");
        
        if(trade.Sell(lot, _Symbol, slPrice, 0, tpPrice, "Adv-Breakout-Sell-" + RegimeToString(regime)))
        {
            Print("✅ Advanced Breakout SELL | Regime: " + RegimeToString(regime) + 
                  ", Lot: " + DoubleToString(lot, 2) + 
                  ", TP: " + IntegerToString(tpPoints) + "pips");
        }
        else
        {
            ...
            Print("❌ Breakout SELL Error: " + IntegerToString(trade.ResultRetcode()));
        }
    }
    else
    {
        DebugPrint(3, "No Breakout signals detected for " + RegimeToString(regime));
    }
}

//---//
double GetBreakoutLotMultiplierForRegime(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    switch(currentRegime)
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 1.5; // ใช้ lot มากกว่าใน trending strong
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            return 1.2;
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return 1.8; // ใช้ lot มากที่สุดใน breakout potential
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
            return 0.5; // ใช้ lot น้อยใน high volatility
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 0.8;
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            return 1.0;
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            return 0.3; // ใช้ lot น้อยที่สุดในข่าว
        default:
            return 1.0;
    }
}
...
{
    // สำหรับเทรนด์แรง breakout อาจต้องการ confirmation น้อยลง
    // แต่ต้องแน่ใจว่าไม่ใช่ pullback
    // ใช้ GetEMAValue ที่อัปเดตแล้ว
    double ema20 = GetEMAValue(_Symbol, PERIOD_H1, 20, 0);
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
...
bool DetectBreakoutSellInVolatileMarket(double volatilityThresholdHigh = 0.008)
{
    ...
    double atrValue = CalculateATR(20, 0, Symbol());  // ใช้ shift = 0
...
bool DetectBreakoutBuyOriginal()
{
    ...
    // 1. ตรวจสอบว่าราคาอยู่เหนือ resistance หรือไม่
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    ...
    // 4. ตรวจสอบ volatility
    double atrValue = GetATRValue(_Symbol, PERIOD_H1, 14, 0);
    double atrAvg = GetATRAverage(_Symbol, PERIOD_H1, 14, 20);
    
    if(atrValue > atrAvg * 2.0) // volatility สูงเกินไป
    {
        if(EnableDebugMode) Print("High volatility for breakout");
        return false;
    }
    
    // 5. ตรวจสอบ candle confirmation
...
bool HasBreakoutVolumeConfirmation(bool isBuy)
{
    ...
    // แปลง volume จาก long เป็น double
    ...
    // ตรวจสอบว่า volume ปัจจุบันสูงกว่าเฉลี่ย 2 แท่งก่อนหน้าหรือไม่
    if(avgPrevVolume == 0) return true; // ไม่มีข้อมูล volume ก่อนหน้า
...
bool HasBullishBreakoutCandle()
{
    ...
    // แท่งปัจจุบันเป็น bullish
    bool currentBullish = (rates[0].close > rates[0].open);
    
    // แท่งก่อนหน้าเป็น bearish หรือ doji
...
bool DetectBreakoutSellOriginal()
{
    ...
    // 1. ตรวจสอบว่าราคาอยู่ใต้ support หรือไม่
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
...
bool HasBearishBreakoutCandle()
{
    ...
    // แท่งปัจจุบันเป็น bearish
    bool currentBearish = (rates[0].close < rates[0].open);
    
    // แท่งก่อนหน้าเป็น bullish หรือ doji
...
{
    // ในตลาดผันผวน ต้องการ confirmation มากขึ้น
    // และต้องแน่ใจว่าไม่ใช่ volatility spike
    
    // 1. ตรวจสอบ volatility ไม่สูงเกินไป
    double atrvalue = GetATRValue(_Symbol, PERIOD_H1, 14, 0);
    double atrAvg = GetATRAverage(_Symbol, PERIOD_H1, 14, 20);
...
{
    // ใน sideways เงียบ breakout อาจเป็น false signal
    // ต้องการ confirmation มากขึ้น
    
    // ตรวจสอบว่า market ไม่อยู่ใน tight range นานเกินไป
    if(IsMarketInTightRange(20, 0.002)) // range แคบกว่า 20 pips
    {
...
//+------------------------------------------------------------------+
bool DetectAdvancedBreakoutBUY(ENUM_MARKET_REGIME_DETAILED regime)
{
    // ตรรกะตรวจสอบ breakout buy
    int atrValue = iATR(_Symbol, _Period, 14);
    double currentHigh = iHigh(_Symbol, _Period, 0);
    double previousHigh = iHigh(_Symbol, _Period, 1);
    
    // ปรับเงื่อนไขตาม regime
    double breakoutThreshold = 0;
    
    switch(regime)
    {
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            breakoutThreshold = atrValue * 0.3; // threshold ต่ำใน breakout potential
            break;
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            breakoutThreshold = atrValue * 0.5; // threshold สูงขึ้นใน trending
            break;
            
        default:
            breakoutThreshold = atrValue * 0.4; // ปกติ
            break;
    }
    
    return (currentHigh > previousHigh + breakoutThreshold);
}
...
bool MomentumConfirmation(int method = 1)
{
    switch(method)
    {
        ...
        default:
            return true; // ถ้าไม่ต้องการ momentum confirmation
    }
}
...
//+------------------------------------------------------------------+
bool DetectAdvancedBreakoutSELL(ENUM_MARKET_REGIME_DETAILED regime)
{
    // ตรรกะตรวจสอบ breakout sell
    int atrValue = iATR(_Symbol, _Period, 14);
    ...
    return (currentLow < previousLow - breakoutThreshold);
}
//+------------------------------------------------------------------+
...
bool CheckBreakoutStrengthBUY()
{
   ...
   return (breakoutSize >= atrValue * MinBreakoutStrength);
}
...
double CalculateScalp2HelpLot(double baseVolume, double lossAmount)
{
   ...
   if(UseFixedLot) {
      ...
      helpLot = baseVolume * Current_Scalp2_LotMultiplier;  // ✅ ใช้ตัวแปรใหม่
   }
...
{
    double atrvalue = CalculateATR(14, 0, symbol);
    double currentPrice = SymbolInfoDouble(symbol, SYMBOL_BID);
...
bool IsStrongReversalSignal(string symbol = NULL)
{
    ...
    double rsivalue = iRSI(sym, PERIOD_M1, 14, PRICE_CLOSE);
    MqlRates rates[];
    ...
    if(CopyRates(sym, PERIOD_M1, 0, 3, rates) == 3)
    {
        ...
        if((bullishReversal && rsivalue < 30) || (bearishReversal && rsivalue > 70))
            return true;
    }
...
{
   // --- Fallback thresholds (หน่วยเป็นสกุลเงินบัญชี) ---
   // หากคุณมีตัวแปร global inputs ชื่อ Scalp3_PairCloseProfit / Scalp3_PairCloseLoss
   // ให้ลบสองบรรทัดนี้และใช้ตัวแปร global ของคุณแทน
   double pairCloseProfit = 10.0; // ตัวอย่าง: ปิดเมื่อรวมกำไร >= $10
   double pairCloseLoss   = 50.0; // ตัวอย่าง: ปิดเมื่อรวมขาดทุน <= -$50
...
void ExecuteScalp3Integrated()
{
 if(!Scalp3_Enabled) return;
    ...
    // ---------------🔍 DEBUG START---------------//
    Print("=== 🎯 SCALP3 DEBUG START ===");
    Print("1. Scalp3_Enabled: ", Scalp3_Enabled);
    
    ulong helpTicket = FindHelpNeededPosition();  // ใช้ตัวแปรเดิม
...
int CountPositionsByMagic(long magicNumber)
{
   ...
   for(int i = 0; i < PositionsTotal(); i++)
   {
      ...
      if(PositionSelectByTicket(ticket))
      {
         // ✅ ฟังก์ชั่นนี้ใช้ได้แล้วเพราะรับ parameter
...
{
    // รูปแบบ Comment: "Recovery-123456" หรือ "Individual-Recov-123456"
    int startPos = -1;
...
void ManageScalp3PairClosing()
{
   for(int i = PositionsTotal()-1; i >= 0; i--)
   {
      ...
      // 1. กรองเฉพาะออเดอร์ Scalp3
      if(PositionGetInteger(POSITION_MAGIC) == MAGIC_SCALP3)
      {
         // 2. อ่าน Comment เพื่อหา Main Ticket
         string comment = PositionGetString(POSITION_COMMENT);
         ulong mainTicket = ExtractMainTicketFromComment(comment);
         
         // 3. ตรวจสอบว่า Main ยังมีอยู่
         if(mainTicket > 0 && PositionSelectByTicket(mainTicket))
         {
            // 4. คำนวณกำไรรวมของคู่
            double helpProfit = PositionGetDouble(POSITION_PROFIT);
            double mainProfit = PositionGetDouble(POSITION_PROFIT);
            double pairProfit = helpProfit + mainProfit;
            
            // 5. ตรวจสอบเงื่อนไขปิด
...
bool GetScalp3BuySignal()
{
   ...
   return (ema5 > ema10 && rsiValue > 30 && rsiValue < 70);
}
...
double CalculateEnhancedRiskPressure() {
    double basePressure = CalculateRiskPressure(); // ใช้ของเดิม
    
    // คำนวณ Volatility Pressure
    double atrValue = iATR(_Symbol, PERIOD_H1, 14);
    double atrPercent = (atrValue / SymbolInfoDouble(_Symbol, SYMBOL_BID)) * 100;
    double volatilityPressure = MathMin(atrPercent / 0.2, 1.0);
    
    // คำนวณ Position Pressure
    int totalPositions = PositionsTotal();
...
void ExecuteRangeCalmStrategy(ENUM_MARKET_REGIME_DETAILED regime = MARKET_REGIME_UNKNOWN)
{
    // ตรวจจับ regime ถ้าไม่ได้ส่งมา
    if(regime == MARKET_REGIME_UNKNOWN) 
    {
    ...
    if(regime == MARKET_REGIME_RANGING_CALM_DETAILED)
    {
        // Execute calm range strategy
        ExecuteCalmRangeTrading(regime);
    }
    else if(regime == MARKET_REGIME_RANGING_VOLATILE_DETAILED)
    {
        // Execute volatile range strategy
        ExecuteVolatileRangeTrading(regime);
    }
    else
    {
        Print("❌ ExecuteRangeCalmStrategy called with wrong regime: ", RegimeToString(regime));
    }
}
// ฟังก์ชันใหม่ที่รวมทุกอย่าง
void ExecuteStrategy(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_RANGING_CALM_DETAILED:
        ...
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
        default:
            ExecuteAdvancedBreakout(regime); // เรียกฟังก์ชันที่ต้องการพารามิเตอร์
            break;
    }
}
// ==================== ฟังก์ชันช่วยเหลือสำหรับ Range Trading ==============//
// 1. ตรวจสอบจำนวน position ที่เป็น range
int CountRangePositions()
{
    int count = 0;
    int total = PositionsTotal();
    ...
    for(int i = total-1; i >= 0; i--)
    {
        ...
        {
            long magic = PositionGetInteger(POSITION_MAGIC);
            // ตรวจสอบว่าเป็น position ของ range trading หรือไม่
            if(magic == MAGIC_RANGE || magic == MAGIC_RANGE_CALM || magic == MAGIC_RANGE_VOLATILE)
            {
                count++;
            }
        }
...
// 2. รับจำนวน position สูงสุดตาม regime
int GetMaxRangePositionsForRegime(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    switch(currentRegime)
    {
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            return 2; // เปิดได้ 2 positions ใน calm range
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 1; // เปิดได้ 1 position ใน volatile range
        default:
            return 0; // ไม่เปิดใน regime อื่นๆ
    }
}
bool ShouldBuyInRange(ENUM_MARKET_REGIME_DETAILED currentRegime, double currentPrice, double rangeLow)
{
    // ตรวจสอบว่าเป็น regime ที่เหมาะสม
    if(currentRegime != MARKET_REGIME_RANGING_CALM_DETAILED && 
       currentRegime != MARKET_REGIME_RANGING_VOLATILE_DETAILED)
    {
        return false;
    }
    
    // ตรวจสอบว่าราคาอยู่ใกล้ support หรือไม่
    double distanceToLow = (currentPrice - rangeLow) / SymbolInfoDouble(_Symbol, SYMBOL_POINT);
    double rangeHeight = (GetRangeHigh() - rangeLow) / SymbolInfoDouble(_Symbol, SYMBOL_POINT);
    
    // ซื้อเมื่อราคาอยู่ใน 30% ล่างของ range
    if(distanceToLow < (rangeHeight * 0.3))
    {
        // ตรวจสอบ RSI สำหรับ oversold
...
bool ShouldSellInRange(ENUM_MARKET_REGIME_DETAILED currentRegime, double currentPrice, double rangeHigh)
{
    // ตรวจสอบว่าเป็น regime ที่เหมาะสม
    if(currentRegime != MARKET_REGIME_RANGING_CALM_DETAILED && 
       currentRegime != MARKET_REGIME_RANGING_VOLATILE_DETAILED)
    ...
    // ตรวจสอบว่าราคาอยู่ใกล้ resistance หรือไม่
    double distanceToHigh = (rangeHigh - currentPrice) / SymbolInfoDouble(_Symbol, SYMBOL_POINT);
    double rangeHeight = (rangeHigh - GetRangeLow()) / SymbolInfoDouble(_Symbol, SYMBOL_POINT);
    
    // ขายเมื่อราคาอยู่ใน 30% บนของ range
    if(distanceToHigh < (rangeHeight * 0.3))
    {
        // ตรวจสอบ RSI สำหรับ overbought
        double rsiValue = iRSI(_Symbol, PERIOD_H1, 14, PRICE_CLOSE, 0);
...
void ExecuteRangeBuy(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    double lotSize = CalculateRangeLotSize(currentRegime);
    double slPrice = GetRangeStopLoss(true, currentRegime);
    double tpPrice = GetRangeTakeProfit(true, currentRegime);
    ...
    request.sl = slPrice;
    request.tp = tpPrice;
    request.magic = (currentRegime == MARKET_REGIME_RANGING_CALM_DETAILED) ? MAGIC_RANGE_CALM : MAGIC_RANGE_VOLATILE;
    request.comment = "Range Buy - " + RegimeToString(currentRegime);
    
    if(OrderSend(request, result))
    {
        Print("✅ Range BUY executed | Regime: " + RegimeToString(currentRegime) + 
              " | Lot: " + DoubleToString(lotSize, 2));
    }
    else
    {
        Print("❌ Range BUY failed: Error ", result.retcode);
    }
}
void ExecuteRangeSell(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    double lotSize = CalculateRangeLotSize(currentRegime);
    double slPrice = GetRangeStopLoss(false, currentRegime);
    double tpPrice = GetRangeTakeProfit(false, currentRegime);
    ...
    request.tp = tpPrice;
    request.magic = (currentRegime == MARKET_REGIME_RANGING_CALM_DETAILED) ? MAGIC_RANGE_CALM : MAGIC_RANGE_VOLATILE;
    request.comment = "Range Sell - " + RegimeToString(currentRegime);
    
    if(OrderSend(request, result))
    {
        Print("✅ Range SELL executed | Regime: " + RegimeToString(currentRegime) + 
              " | Lot: " + DoubleToString(lotSize, 2));
    }
    else
    {
        Print("❌ Range SELL failed: Error ", result.retcode);
    }
}
// 9. คำนวณ lot size สำหรับ range trading
double CalculateRangeLotSize(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    double baseLot = BaseLot;
    ...
    {
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            return NormalizeDouble(baseLot * 0.5, 2); // ใช้ lot น้อยลงใน calm range
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return NormalizeDouble(baseLot * 0.3, 2); // ใช้ lot น้อยที่สุดใน volatile range
        default:
            return NormalizeDouble(baseLot * 0.5, 2);
    }
}
// 10. หา Stop Loss สำหรับ range trading
double GetRangeStopLoss(bool isBuy, ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    ...
    if(isBuy)
    {
        // สำหรับ Buy: SL อยู่ใต้ range low
        double slDistance = (currentRegime == MARKET_REGIME_RANGING_VOLATILE_DETAILED) ? atrValue * 2 : atrValue;
        return rangeLow - slDistance;
    }
    else
    {
        // สำหรับ Sell: SL อยู่เหนือ range high
        double slDistance = (currentRegime == MARKET_REGIME_RANGING_VOLATILE_DETAILED) ? atrValue * 2 : atrValue;
        return rangeHigh + slDistance;
    }
}

// 11. หา Take Profit สำหรับ range trading
double GetRangeTakeProfit(bool isBuy, ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    double rangeHigh = GetRangeHigh();
    double rangeLow = GetRangeLow();
    ...
    if(isBuy)
    {
        // สำหรับ Buy: TP อยู่ที่กลาง range หรือด้านบน
        return rangeLow + (rangeHeight * 0.7); // 70% ของ range height
    }
    else
    {
        // สำหรับ Sell: TP อยู่ที่กลาง range หรือด้านล่าง
        return rangeHigh - (rangeHeight * 0.7); // 70% ของ range height
    }
}
// ==================== ฟังก์ชันช่วยเหลือสำหรับ Range Trading ===============//
// ===== BREAKOUT STRATEGY EXECUTION =====
bool IsRangeTradingSuitableForRegime(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    // Range trading เหมาะกับ ranging markets เท่านั้น
    switch(currentRegime)
    {
        case MARKET_REGIME_RANGING_CALM_DETAILED:
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
...
void ExecuteCalmRangeTrading(ENUM_MARKET_REGIME_DETAILED regime)
{

 // ตรวจสอบว่าเป็น regime ที่เหมาะกับ range trading หรือไม่
    if(regime != MARKET_REGIME_RANGING_CALM_DETAILED && 
       regime != MARKET_REGIME_RANGING_VOLATILE_DETAILED)
    {
        Print("Range trading not suitable for current regime: " + RegimeToString(regime));
        return;
    }
    
    if(!IsRangeTradingEnabled) 
    {
        Print("Range trading disabled");
        return;
    }
    
    Print("=== EXECUTING CALM RANGE STRATEGY ===");
    
    // 1. ตรวจสอบจำนวน positions ปัจจุบัน
    int rangePositions = CountRangePositions();
    int maxRangePositions = GetMaxRangePositionsForRegime(regime);
    
    if(rangePositions >= maxRangePositions) 
    {
        Print("Max range positions reached: ", rangePositions, "/", maxRangePositions);
        return;
    }
    
    // 2. หาค่า range boundaries
    double rangeHigh = GetRangeHigh();
    double rangeLow = GetRangeLow();
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    
    // 3. ตรวจสอบเงื่อนไข entry
    bool shouldBuy = ShouldBuyInRange(regime, currentPrice, rangeLow);
    bool shouldSell = ShouldSellInRange(regime, currentPrice, rangeHigh);
    
    // 4. Execute trades
    if(shouldBuy) 
    {
        ExecuteRangeBuy(regime);
    }
    else if(shouldSell) 
    {
        ExecuteRangeSell(regime);
    }
    else 
    {
        Print("No range trading signals detected");
    }
}
double GetStopLossMultiplierForVolatileRange()
{
    // ในช่วง volatile range ใช้ tighter stop loss (คูณด้วยค่าที่น้อยกว่า 1)
    return 0.7; // ใช้ SL แค่ 70% ของปกติ
}

// หรือฟังก์ชันที่ปรับตาม volatility:
double GetStopLossMultiplierForRegime(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    switch(currentRegime)
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 0.7; // 70% ใน volatile range
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
            return 0.5; // 50% ใน high volatility
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            return 0.3; // 30% ในช่วงข่าว
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            return 1.0; // 100% ใน calm range
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 1.5; // 150% ใน strong trend (ให้ SL ใหญ่ขึ้น)
        default:
            return 1.0; // ปกติ
    }
}
bool IsBullishReversalPattern(int shift = 0)
{
    // ตรวจสอบ bullish reversal pattern ง่ายๆ (pin bar, hammer, etc.)
    ...
    // Hammer pattern: lower wick ยาว, body สั้น, upper wick สั้น
    if(bodySize < totalRange * 0.3 && lowerWick > bodySize * 2 && upperWick < bodySize)
    {
        return true;
    }
    
    // Bullish pin bar
    if(lowerWick > bodySize * 3 && upperWick < bodySize * 0.5)
    {
        return true;
    }
...
bool ShouldBuyInVolatileRange(ENUM_MARKET_REGIME_DETAILED currentRegime, double currentPrice, double rangeLow, double volatility)
{
    // ตรวจสอบว่าเป็น volatile range regime หรือไม่
    if(currentRegime != MARKET_REGIME_RANGING_VOLATILE_DETAILED)
        return false;
    
    // คำนวณ distance จาก range low
    double distanceFromLow = (currentPrice - rangeLow) / SymbolInfoDouble(_Symbol, SYMBOL_POINT);
    double rangeHeight = (GetRangeHigh() - rangeLow) / SymbolInfoDouble(_Symbol, SYMBOL_POINT);
    
    // ใน volatile range ใช้ entry threshold ที่ต่ำกว่า (20% แทน 30%)
    double entryThreshold = 0.2; // 20%
    
    if(distanceFromLow < (rangeHeight * entryThreshold))
    {
        // ตรวจสอบ oversold condition ด้วย RSI
        double rsiValue = iRSI(_Symbol, PERIOD_M15, 14, PRICE_CLOSE, 0);
        if(rsiValue < 35) // ใช้ระดับ oversold ที่สูงกว่าใน volatile range
        {
            // ตรวจสอบ price action: pin bar หรือ bullish reversal pattern
            if(IsBullishReversalPattern(0))
            {
                return true;
            }
        }
...
bool IsBearishReversalPattern(int shift = 0)
{
    // ตรวจสอบ bearish reversal pattern ง่ายๆ (shooting star, bearish pin bar, etc.)
    double open = iOpen(_Symbol, _Period, shift);
    double close = iClose(_Symbol, _Period, shift);
    double high = iHigh(_Symbol, _Period, shift);
    double low = iLow(_Symbol, _Period, shift);
    ...
    // Shooting star pattern: upper wick ยาว, body สั้น, lower wick สั้น
    if(bodySize < totalRange * 0.3 && upperWick > bodySize * 2 && lowerWick < bodySize)
    {
        return true;
    }
    
    // Bearish pin bar
    if(upperWick > bodySize * 3 && lowerWick < bodySize * 0.5)
    {
        return true;
    }
...
bool ShouldSellInVolatileRange(ENUM_MARKET_REGIME_DETAILED currentRegime, double currentPrice, double rangeHigh, double volatility)
{
    // ตรวจสอบว่าเป็น volatile range regime หรือไม่
    if(currentRegime != MARKET_REGIME_RANGING_VOLATILE_DETAILED)
        return false;
    
    // คำนวณ distance จาก range high
    double distanceFromHigh = (rangeHigh - currentPrice) / SymbolInfoDouble(_Symbol, SYMBOL_POINT);
    double rangeHeight = (rangeHigh - GetRangeLow()) / SymbolInfoDouble(_Symbol, SYMBOL_POINT);
    
    // ใน volatile range ใช้ entry threshold ที่ต่ำกว่า (20% แทน 30%)
    double entryThreshold = 0.2; // 20%
    
    if(distanceFromHigh < (rangeHeight * entryThreshold))
    {
        // ตรวจสอบ overbought condition ด้วย RSI
        double rsiValue = iRSI(_Symbol, PERIOD_M15, 14, PRICE_CLOSE, 0);
        if(rsiValue > 65) // ใช้ระดับ overbought ที่ต่ำกว่าใน volatile range
        {
            // ตรวจสอบ price action: pin bar หรือ bearish reversal pattern
            if(IsBearishReversalPattern(0))
            {
                return true;
            }
        }
...
void ExecuteVolatileRangeBuy(ENUM_MARKET_REGIME_DETAILED currentRegime, double slMultiplier)
{
    double baseLot = BaseLot;
    double lotSize = NormalizeDouble(baseLot * 0.5, 2); // ใช้ lot น้อยลงใน volatile range
    
    // คำนวณ SL และ TP
    int slPips = (int)(RangeTradingStopLossPips * slMultiplier);
    int tpPips = (int)(RangeTradingTakeProfitPips * 0.8); // TP น้อยลงใน volatile range
    
    double slPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID) - (slPips * SymbolInfoDouble(_Symbol, SYMBOL_POINT));
    double tpPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID) + (tpPips * SymbolInfoDouble(_Symbol, SYMBOL_POINT));
    ...
    request.sl = slPrice;
    request.tp = tpPrice;
    request.magic = MAGIC_RANGE_VOLATILE;
    request.comment = "Volatile Range Buy";
    
    if(OrderSend(request, result))
    {
        Print("✅ Volatile Range BUY executed | Lot: " + DoubleToString(lotSize, 2) + 
              " | SL: " + IntegerToString(slPips) + "pips | TP: " + IntegerToString(tpPips) + "pips");
    }
    else
    {
        Print("❌ Volatile Range BUY failed: Error ", result.retcode);
    }
}

void ExecuteVolatileRangeSell(ENUM_MARKET_REGIME_DETAILED currentRegime, double slMultiplier)
{
    double baseLot = BaseLot;
    double lotSize = NormalizeDouble(baseLot * 0.5, 2); // ใช้ lot น้อยลงใน volatile range
    ...
    double slPrice = SymbolInfoDouble(_Symbol, SYMBOL_ASK) + (slPips * SymbolInfoDouble(_Symbol, SYMBOL_POINT));
    double tpPrice = SymbolInfoDouble(_Symbol, SYMBOL_ASK) - (tpPips * SymbolInfoDouble(_Symbol, SYMBOL_POINT));
    ...
    request.tp = tpPrice;
    request.magic = MAGIC_RANGE_VOLATILE;
    request.comment = "Volatile Range Sell";
    
    if(OrderSend(request, result))
    {
        Print("✅ Volatile Range SELL executed | Lot: " + DoubleToString(lotSize, 2) + 
              " | SL: " + IntegerToString(slPips) + "pips | TP: " + IntegerToString(tpPips) + "pips");
    }
    else
    {
        Print("❌ Volatile Range SELL failed: Error ", result.retcode);
    }
}
void ExecuteVolatileRangeTrading(ENUM_MARKET_REGIME_DETAILED regime)
{
    if(!IsRangeTradingEnabled) 
    {
        Print("Range trading disabled");
    ...
    Print("=== EXECUTING VOLATILE RANGE STRATEGY ===");
    
    // 1. ตรวจสอบ volatility
    double volatility = CalculateCurrentVolatility();
    
    if(volatility > MaxVolatilityForRange) 
    {
        Print("Volatility too high for range trading: ", volatility);
        return;
    }
    
    // 2. ตรวจสอบจำนวน positions
    int rangePositions = CountRangePositions();
    int maxRangePositions = GetMaxRangePositionsForRegime(regime);
    ...
    // 3. ใช้ tighter stop loss ใน volatile range
    double stopLossMultiplier = GetStopLossMultiplierForRegime(regime);
    
    // 4. หา range boundaries
    double rangeHigh = GetRangeHigh();
    double rangeLow = GetRangeLow();
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    
    // 5. ตรวจสอบเงื่อนไข entry
    bool shouldBuy = ShouldBuyInVolatileRange(regime, currentPrice, rangeLow, volatility);
    bool shouldSell = ShouldSellInVolatileRange(regime, currentPrice, rangeHigh, volatility);
    
    // 6. Execute trades
    if(shouldBuy) 
    {
        ExecuteVolatileRangeBuy(regime, stopLossMultiplier);
    }
    else if(shouldSell) 
    {
        ExecuteVolatileRangeSell(regime, stopLossMultiplier);
    }
    else 
    {
        Print("No volatile range trading signals detected");
    }
}
...
void ExecuteBreakoutStrategyByRegime(ENUM_MARKET_REGIME_DETAILED currentregime)
{
    ...
    switch(currentregime)
    {
        ...
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            // Usually avoid breakout in ranging markets or high volatility
            shouldExecute = false;
            break;
    ...
    {
        Print("Breakout strategy not suitable for regime: " + RegimeToString(currentregime));
    }
}
// ===== VOLATILE RANGE STRATEGY =====
void ExecuteRangeVolatileStrategy(ENUM_MARKET_REGIME_DETAILED regime = MARKET_REGIME_RANGING_VOLATILE_DETAILED)
{
    // Only execute if regime is volatile range
    if(regime != MARKET_REGIME_RANGING_VOLATILE_DETAILED)
        return;
    ...
    // bool buySignal = DetectBreakoutBuyInVolatileMarket(); // ถ้ามี
...
// ฟังก์ชันดึงค่า Bollinger Bands
...
void ExecuteTrendWeakStrategy(ENUM_MARKET_REGIME_DETAILED regime)
{
    // 1. ประกาศตัวแปรทั้งหมดที่ต้องการใช้
...
// ประกาศฟังก์ชันเหล่านี้ที่ส่วนบนของไฟล์
...
void ExecuteAdaptiveStrategy(ENUM_MARKET_REGIME_DETAILED regime = MARKET_REGIME_UNKNOWN_DETAILED) 
{
    // ตรวจจับ regime ถ้าไม่ได้ส่งมา
    if(regime == MARKET_REGIME_UNKNOWN_DETAILED) 
    {
    ...
    // ตรวจสอบว่า adaptive strategy เปิดใช้งาน
    if(!AdaptiveStrategyEnabled) 
    {
        Print("Adaptive strategy disabled");
        return;
    }
    
    Print("=== EXECUTING ADAPTIVE STRATEGY ===");
    Print("Market Regime: ", RegimeToString(regime));
    
    // ปรับ strategy ตาม market regime
    AdjustStrategyParameters(regime);
    
    // ตรวจสอบเงื่อนไขการเทรดตาม regime
    if(!IsTradingAllowedInRegime(regime)) 
    {
        Print("Trading not allowed in ", RegimeToString(regime), " regime");
        return;
    }
    ...
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            ExecuteTrendFollowingStrategy(regime);
            break;
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            ExecuteMildTrendStrategy(regime);
            break;
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            ExecuteRangeTradingStrategy(regime);
            break;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            ExecuteVolatileRangeStrategy(regime);
            break;
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            ExecuteAdvancedBreakout(regime);
            break;
            
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
            ExecuteSafeModeStrategy(regime);
            break;
        ...
        default:
            ExecuteDefaultStrategy(regime);
            break;
    }
    
    // บันทึกการดำเนินการ
    RecordStrategyExecution(regime);
}
void AdjustStrategyParameters(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    ...
    switch(currentRegime)
    {
        ...
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            CurrentTakeProfitPips = DefaultTakeProfitPips * 1.2;
            CurrentStopLossPips = DefaultStopLossPips * 1.0;
            CurrentLotMultiplier = 0.8;
            break;
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            CurrentTakeProfitPips = DefaultTakeProfitPips * 0.7;
            CurrentStopLossPips = DefaultStopLossPips * 0.8;
            CurrentLotMultiplier = 0.6;
            break;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            CurrentTakeProfitPips = DefaultTakeProfitPips * 0.5;
            CurrentStopLossPips = DefaultStopLossPips * 0.6;
            CurrentLotMultiplier = 0.4;
            break;
            
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
            CurrentTakeProfitPips = DefaultTakeProfitPips * 0.3;
            CurrentStopLossPips = DefaultStopLossPips * 0.5;
            CurrentLotMultiplier = 0.3;
            break;
            
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            CurrentTakeProfitPips = DefaultTakeProfitPips * 0.4;
            CurrentStopLossPips = DefaultStopLossPips * 0.4;
...
bool IsTradingAllowedInRegime(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    switch(currentRegime)
    {
        case MARKET_REGIME_HIGH_VOLATILITY_DETAILED:
        case MARKET_REGIME_NEWS_EVENT_DETAILED:
            Print("Trading not recommended in " + RegimeToString(currentRegime));
            return false;
...
void ExecuteTrendFollowingStrategy(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    Print("Executing Trend Following Strategy");
    // ใช้ moving average crossover, momentum indicators
    // ...
}

// 4. Mild Trend Strategy
void ExecuteMildTrendStrategy(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    Print("Executing Mild Trend Strategy");
    // ใช้ trend strategy แต่ conservative กว่า
    // ...
}

// 5. Range Trading Strategy
void ExecuteRangeTradingStrategy(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    Print("Executing Range Trading Strategy");
    // เรียกใช้ฟังก์ชันที่มีอยู่
    ExecuteCalmRangeTrading(currentRegime);
}

// 6. Volatile Range Strategy
void ExecuteVolatileRangeStrategy(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    Print("Executing Volatile Range Strategy");
    // เรียกใช้ฟังก์ชันที่มีอยู่
    ExecuteVolatileRangeTrading(currentRegime);
}

// 7. Safe Mode Strategy (สำหรับ high volatility)
...
// 8. News Event Strategy
void ExecuteNewsEventStrategy(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
...
{
    Print("Executing Default Strategy for regime: " + RegimeToString(currentRegime));
    // ใช้ strategy พื้นฐาน
...
// 11. ฟังก์ชันพื้นฐาน
void ExecuteBasicStrategy()
{
    // Strategy พื้นฐานที่ใช้ก่อนมี adaptive
    Print("Executing Basic Strategy");
    // ... logic พื้นฐาน ...
}

// 12. บันทึกการ execute
void RecordStrategyExecution(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    // บันทึกลงไฟล์หรือตัวแปร
    string record = TimeToString(TimeCurrent()) + " - Executed " + 
...
void ExecuteWeakTrendBuy(double lotSize, double takeProfitPips, double stopLossPips, ENUM_MARKET_REGIME_DETAILED regime)
{
    ...
    {
        double price = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
        double tpPrice = price + (takeProfitPips * Point * 10);
        double slPrice = price - (stopLossPips * Point * 10);
...
bool IsWeakUptrend()
{
    ...
    double maFast = iMA(_Symbol, PERIOD_H1, 10, 0, MODE_SMA, PRICE_CLOSE, 0);
    double maMedium = iMA(_Symbol, PERIOD_H1, 20, 0, MODE_SMA, PRICE_CLOSE, 0);
    double maSlow = iMA(_Symbol, PERIOD_H1, 50, 0, MODE_SMA, PRICE_CLOSE, 0);
    ...
    bool rsiCondition = (rsi > 50 && rsi < 65);  // bullish แต่ไม่ร้อนแรงเกิน
...
bool IsWeakDowntrend()
{
    ...
    bool maCondition = (maFast < maMedium && maMedium < maSlow);
    bool rsiCondition = (rsi < 50 && rsi > 35);  // bearish แต่ไม่ oversold เกิน
...
bool ShouldTradeBreakoutInHighVol()
{
    ...
    double atr = GetATRValue(_Symbol, PERIOD_M15, 14, 0);
    double normalATR = GetATRValue(_Symbol, PERIOD_H1, 14, 0);
    
    // ถ้า volatility สูงกว่า 200% ของปกติ
    bool highVolCondition = (atr > normalATR * 2.0);
...
bool DetectVolatileRangeOpportunity()
{
    ...
    if(nearSupport)
    {
        ...
        rsiConfirmation = (rsi < 35);  // oversold
    }
    else if(nearResistance)
    {
        double rsi = iRSI(_Symbol, PERIOD_M15, 14, PRICE_CLOSE, 0);
        rsiConfirmation = (rsi > 65);  // overbought
    }
...
bool DetectWeakTrendRangeOpportunity()
{
    ...
    double maFast = iMA(_Symbol, PERIOD_H1, 20, 0, MODE_SMA, PRICE_CLOSE, 0);
    double maSlow = iMA(_Symbol, PERIOD_H1, 50, 0, MODE_SMA, PRICE_CLOSE, 0);
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    
    // ตรวจสอบว่า price เคลื่อนที่ใน sideways channel
    bool inChannel = IsPriceInChannel(currentPrice, maFast, maSlow);
...
void ExecuteRangeTrade(double riskMultiplier)
{
...
{
    Print("Range Buy - Buying at support");
    // ส่งคำสั่งซื้อ...
}

void ExecuteRangeSell(double lotSize, double tpPips, double slPips)
{
    Print("Range Sell - Selling at resistance");
    // ส่งคำสั่งขาย...
}
bool ShouldTradeMeanReversionInHighVol()
{
    ...
    double rsi = iRSI(_Symbol, PERIOD_M15, 14, PRICE_CLOSE, 0);
    double stochastic = iStochastic(_Symbol, PERIOD_M15, 5, 3, 3, MODE_SMA, STO_LOWHIGH, MODE_MAIN, 0);
    
    // Overbought/oversold ในช่วง volatile
    bool oversold = (rsi < 30 && stochastic < 20);
    bool overbought = (rsi > 70 && stochastic > 80);
...
void ExecuteNormalStrategy(ENUM_MARKET_REGIME_DETAILED regime = MARKET_REGIME_NORMAL)
{
    ...
    {
        ExecuteRangeTrade(0.7); // 70% of normal risk for range trading
    }
}
void ResetRecoveryToDefault()
{
    ...
    Print("   🔄 Recovery settings reset to default");
}
void AdjustStrategyByMomentum(double momentum)
{
    if(momentum > 10) {
        // โมเมนตัมบวกชัด → เน้นทีม Buy
        AssignTeamWeights(40, 20, 25, 10, 5);
        Print("   🟢 Momentum positive - Focusing on BUY teams");
    } else if(momentum < -10) {
        // โมเมนตัมลบชัด → เน้นทีม Sell
        AssignTeamWeights(20, 40, 25, 10, 5);
        Print("   🔴 Momentum negative - Focusing on SELL teams");
    } else {
        // โมเมนตัมสมดุล → กระจายเท่าๆ กัน
        AssignTeamWeights(30, 25, 20, 15, 10);
        Print("   ⚖️ Balanced momentum - Equal team distribution");
    }
}
//---------------- Performance Feedback -----------//
// ใช้ข้อมูลจาก CalculateTotalProfit() และฟังก์ชันอื่นๆ ที่มีอยู่
void AdaptiveParameterAdjustment(ENUM_MARKET_REGIME_DETAILED regime = MARKET_REGIME_NORMAL) 
{
    double totalProfit = CalculateTotalProfit();
    double winRate = CalculateWinRate(); // ต้องสร้างฟังก์ชันนี้
    
    // ปรับพารามิเตอร์ตาม Performance และ Market Regime
    switch(regime) 
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // ในเทรนด์แรง สามารถเพิ่ม aggression ได้
            if(totalProfit > 0 && winRate > 0.55) 
            {
                Current_Scalp1_Lot_Multiplier = MathMin(Current_Scalp1_Lot_Multiplier * 1.15, 2.5);
                Print("📈 Strong trend - Increasing aggression: ", Current_Scalp1_Lot_Multiplier);
            }
            break;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // ในตลาดผันผวน ลด aggression
            Current_Scalp1_Lot_Multiplier = MathMax(Current_Scalp1_Lot_Multiplier * 0.7, 0.5);
            Print("⚠️ Volatile market - Reducing aggression: ", Current_Scalp1_Lot_Multiplier);
            break;
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            // ใน sideways เงียบ ใช้การตั้งค่าปกติ
            Current_Scalp1_Lot_Multiplier = MathMax(Current_Scalp1_Lot_Multiplier, 0.8);
            Print("😐 Calm market - Normal aggression: ", Current_Scalp1_Lot_Multiplier);
            break;
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            // ระวังใน breakout ลด lot
            Current_Scalp1_Lot_Multiplier = MathMax(Current_Scalp1_Lot_Multiplier * 0.6, 0.4);
            Print("🎯 Breakout potential - Conservative aggression: ", Current_Scalp1_Lot_Multiplier);
            break;
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            // ในเทรนด์อ่อน ใช้การตั้งค่าปานกลาง
            Current_Scalp1_Lot_Multiplier = MathMin(MathMax(Current_Scalp1_Lot_Multiplier, 0.7), 1.3);
            Print("📊 Weak trend - Moderate aggression: ", Current_Scalp1_Lot_Multiplier);
            break;
        ...
        default:
            ...
            if(winRate < 0.4) 
            {
                ...
                Print("📉 Reducing aggression due to low win rate: ", winRate);
            }
            
            if(totalProfit > 0 && winRate > 0.6) 
            {
                // เพิ่ม Aggression เล็กน้อย
                Current_Scalp1_Lot_Multiplier = MathMin(Current_Scalp1_Lot_Multiplier * 1.1, 2.0);
                Print("📈 Slightly increasing aggression due to good performance");
            }
            break;
    }
...
// สร้างฟังก์ชันคำนวณ Win Rate จากข้อมูลที่มี
double CalculateWinRate() {
    ...
    for(int i = PositionsTotal()-1; i >= 0; i--) {
        ...
        if(PositionSelectByTicket(ticket)) {
            double profit = PositionGetDouble(POSITION_PROFIT);
            if(profit != 0) { // ไม่นับออเดอร์ที่ยังเปิดอยู่
                totalTrades++;
...
void OpenRecoveryPosition(ENUM_POSITION_TYPE type, double multiplier)
{
    ...
    Print("   📈 Opening Recovery " + EnumToString(type) + 
          " Lot: " + DoubleToString(lotSize, 2) +
...
double CalculateRecoveryLotSize()
{
    ...
    double baseLot = 0.01; // หรือคำนวณจาก balance
    int recoveryStep = GetCurrentRecoveryStep();
...
{
   return recoveryLayerCount; // หรือ logic การนับขั้น recovery
}
...
//-----------------------------------------**--//
// ✅ ฟังก์ชันตรวจสอบสถานะ safety system
void PrintRecoverySafetyStatus()
{
    ...
    if(!UseRecoverySafety)
    {
        Print("🚫 RECOVERY SAFETY: DISABLED");
        Print("💡 No automatic position closure will occur");
        Print("💡 System will only show warnings");
    }
    else
    {
        Print("🛡️ RECOVERY SAFETY: ENABLED");
        Print("💡 Will take action if conditions are met");
    }
}
// ✅ ตรวจสอบ UseRecoverySafety
void ExecuteMarginProtectionOnly()
{
    // ✅ ตรวจสอบอีกครั้งก่อนดำเนินการ
    if(!UseRecoverySafety) 
    {
        Print("⚠️ MARGIN PROTECTION: UseRecoverySafety = FALSE - No action taken");
        return;
    }
    
    Print("🛡️ MARGIN PROTECTION: Managing positions carefully");
    
    int closedCount = 0;
    
    // ✅ ปิดเฉพาะออเดอร์ที่กำไรเล็กน้อย
    ...
    if(closedCount > 0)
    {
        ...
        Print("✅ MARGIN PROTECTION: Closed " + IntegerToString(closedCount) + 
...
void RunSafetyCheck()
{
    // ✅ เรียกใช้ Safety Systems Manager
    ManageSafetySystems();
    
    // ✅ ระบบเดิมสำหรับ Margin Protection (แยกส่วน)
    double marginLevel = (AccountInfoDouble(ACCOUNT_EQUITY) / AccountInfoDouble(ACCOUNT_MARGIN)) * 100.0;
...
bool CloseHedgedPair(ulong ticket1, ulong ticket2, double profit)
{
    ...
    {
        Print("❌ FAILED to close hedged pair: " +
              "T1: " + (closed1 ? "OK" : "FAIL") + 
              " | T2: " + (closed2 ? "OK" : "FAIL"));
        return false;
    }
}
// เพิ่มฟังก์ชันนี้ก่อนฟังก์ชัน CloseHedgedPair
...
{
    // ปิดคู่ที่ขัดแย้งกันแบบ selective
    double totalClosedProfit = 0;
    int pairsClosed = 0;
    ...
    for(int i = PositionsTotal()-1; i >= 0; i--)
    {
        ...
        for(int j = PositionsTotal()-1; j >= 0; j--)
        {
            ...
            // ถ้าเป็นสกุลเงินเดียวกันและทิศทางตรงข้าม
            if(symbol1 == symbol2 && type1 != type2)
            {
                ...
                // ปิดเมื่อได้กำไรรวม หรือ ขาดทุนน้อยมาก
                if(totalProfit >= 1.0 || (totalProfit >= -2.0 && MathAbs(profit1 - profit2) < 5.0))
                {
                    if(trade.PositionClose(ticket1) && trade.PositionClose(ticket2))
                    {
                        ...
                        pairsClosed++;
                        Print("🎯 SMART HEDGE CLOSED | Profit: $" + DoubleToString(totalProfit, 2));
                        break;
                    }
                }
            }
    ...
    if(pairsClosed > 0)
    {
        Print("🎯 HEDGE SUMMARY | Pairs: " + IntegerToString(pairsClosed) + 
...
double CalculateBreakEvenPrice(ulong buyTicket, ulong sellTicket)
{
    ...
    // ตรวจสอบว่าเป็นตำแหน่งตรงข้ามกันจริงๆ
    ...
    // คำนวณราคาคุมทุน (Weighted Average)
...
void UpdateBreakevenLine()
{
    ...
    // ถ้าไม่มีตำแหน่งขัดแย้งกัน → ลบเส้นออก
...
double CalculateGlobalBreakevenPrice()
{
    ...
    for(int i = 0; i < PositionsTotal(); i++)
    {
        ...
        string symbol = PositionGetString(POSITION_SYMBOL);
        if(symbol != _Symbol) continue; // เฉพาะสัญลักษณ์ปัจจุบัน
        ...
        if(type == POSITION_TYPE_BUY)
        {
            totalBuyLots += (int)(volume / 0.01); // แปลงเป็น lot
            totalBuyCost += volume * openPrice;
        }
        else if(type == POSITION_TYPE_SELL)
        {
            totalSellLots += (int)(volume / 0.01); // แปลงเป็น lot
            totalSellCost += volume * openPrice;
        }
    }
    
    // ถ้าไม่มีตำแหน่งขัดแย้งกัน
    ...
    Print("💰 BREAKEVEN CALC | Buy: " + IntegerToString(totalBuyLots) + 
...
void CreateBreakevenLine(double price)
{
    ...
    ObjectSetInteger(0, beLineName, OBJPROP_COLOR, clrLime); // สีเขียว
    ...
    Print("📊 BREAKEVEN LINE CREATED | Price: " + DoubleToString(price, _Digits));
}
...
void CheckStuckHedges()
{
    for(int i = 0; i < PositionsTotal(); i++)
    {
        ...
        // ตรวจสอบว่า "ค้าง" และต้องการแก้ไข
        bool isStuck = (hoursOpen > 1.0) && (MathAbs(profit) > 10.0);
        
        if(isStuck)
        {
            Print("⚠️ ANALYZING STUCK HEDGE | #" + TicketToStr(ticket) + 
                  " | P/L: $" + DoubleToString(profit, 2) + 
                  " | Hours: " + DoubleToString(hoursOpen, 1));
            
            // ✅ ไม่ปิด! แต่วิเคราะห์และแก้ไข
...
// +------------------------------------------------------------------+
//| Manual Hedge Management (เรียกจาก OnChartEvent)                 |
//+------------------------------------------------------------------+
...
void CloseAllHedges()
{
    ...
    for(int i = PositionsTotal()-1; i >= 0; i--)
    {
        ...
        if(trade.PositionClose(ticket))
        {
            closedCount++;
            totalProfit += profit;
            Print("✅ HEDGE CLOSED | #" + TicketToStr(ticket) + 
                  " | P/L: $" + DoubleToString(profit, 2));
        }
    }
    
    Print("🎯 HEDGE CLOSE SUMMARY | Closed: " + IntegerToString(closedCount) + 
          " | Total P/L: $" + DoubleToString(totalProfit, 2));
}
// +------------------------------------------------------------------+
//| Chart Event (สำหรับ manual control)                            |
//+------------------------------------------------------------------+
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
    if(id == CHARTEVENT_KEYDOWN)
    {
        // กด 'H' เพื่อจัดการ hedge manual
        if(lparam == 'H')
        {
                       ...
                       double hedgeVolume, double hedgeProfit)
{
    ...
    Print("🔧 FIXING STANDALONE HEDGE | #" + TicketToStr(hedgeTicket) + 
          " | " + (hedgeType == POSITION_TYPE_BUY ? "BUY" : "SELL") +
          " | Lot: " + DoubleToString(hedgeVolume, 2) + 
          " | P/L: $" + DoubleToString(hedgeProfit, 2));
    
    // ✅ กลยุทธ์แก้ไข Hedge แบบ standalone
    
    if(hedgeProfit <= -20.0) 
    {
        // กรณี 1: Hedge ติดลบหนัก → เปิดช่วยเหลือ
        OpenHelpForStandaloneHedge(hedgeTicket, hedgeType, hedgeVolume, symbol, hedgeProfit);
    }
    else if(hedgeProfit >= 15.0) 
    {
        // กรณี 2: Hedge กำไรมาก → ปิดบางส่วน lock profit
        ClosePartialPosition(hedgeTicket, hedgeVolume * 0.4, "Standalone hedge profit take");
    }
    else 
    {
        // กรณี 3: Hedge ค้างแต่ไม่หนัก → เปิดตำแหน่งช่วยแบบ conservative
...
{
    double helpVolume = hedgeVolume * 0.6; // 60% ของ volume เดิม
    ...
    {
        Print("❌ Margin insufficient for standalone hedge help");
        return;
    }
    ...
    // เปิดตำแหน่งช่วยในทิศทางเดียวกัน (เสริมกำลัง)
    ...
    if(success)
    {
        Print("🆘 STANDALONE HEDGE HELP | For: #" + TicketToStr(hedgeTicket) + 
              " | " + (hedgeType == POSITION_TYPE_BUY ? "BUY" : "SELL") +
...
void OpenConservativeHelp(ulong hedgeTicket, ENUM_POSITION_TYPE hedgeType,
                         double hedgeVolume, string symbol, double hedgeProfit)
{
    double helpVolume = BaseLot; // ใช้ lot ขนาดเล็ก
    helpVolume = NormalizeDouble(helpVolume, 2);
    
    if(!CheckMargin(helpVolume, symbol))
    {
        Print("❌ Margin insufficient for conservative help");
        return;
    }
    ...
    // เปิดตำแหน่งตรงข้ามเพื่อสร้าง balance
    if(hedgeType == POSITION_TYPE_BUY)
    {
        success = trade.Sell(helpVolume, symbol, 0, 0, 0, 
    ...
    if(success)
    {
        Print("🎯 CONSERVATIVE HEDGE BALANCE | For: #" + TicketToStr(hedgeTicket) + 
                         ...
                         double hedgeVolume, double hedgeProfit)
{
    ...
    Print("💰 HEDGE PAIR ANALYSIS | Main: #" + TicketToStr(mainTicket) + 
    ...
    // ✅ กลยุทธ์แก้ไข: เปิด recovery เพิ่มเติม
    if(totalPairProfit <= -15.0) // ขาดทุนรวมมาก
    {
        OpenHedgeRecovery(hedgeTicket, mainTicket, symbol, mainType, mainVolume, hedgeVolume);
    }
    else if(MathAbs(hedgeProfit) > 25.0) // Hedge ติดลบหรือกำไรมาก
    {
                      ...
                      ENUM_POSITION_TYPE mainType, double mainVolume, double hedgeVolume)
{
    ...
    double recoveryVolume = totalVolume * 0.3; // 30% ของ volume รวม
    ...
    {
        Print("❌ Insufficient margin for hedge recovery: " + DoubleToString(recoveryVolume, 2));
        return;
    }
    ...
    // เปิดตรงข้ามกับตำแหน่งหลัก (ช่วย hedge)
    if(mainType == POSITION_TYPE_BUY)
    {
        // หลักเป็น BUY, Hedge เป็น SELL → เปิด SELL เพิ่ม
        success = trade.Sell(recoveryVolume, symbol, 0, 0, 0, 
                           "Hedge-Recov-Main" + TicketToStr(mainTicket));
    }
    else
    {
        // หลักเป็น SELL, Hedge เป็น BUY → เปิด BUY เพิ่ม  
    ...
    if(success)
    {
        ...
        Print("🎯 HEDGE RECOVERY OPENED | Main: #" + TicketToStr(mainTicket) + 
              " | Hedge: #" + TicketToStr(hedgeTicket) +
              " | Recovery: #" + TicketToStr(recoveryTicket) +
...
{
    if(hedgeProfit <= -20.0) // Hedge ติดลบหนัก
    {
        // ✅ เปิดตำแหน่งช่วย Hedge
        ...
        if(hedgeType == POSITION_TYPE_BUY)
        {
            // Hedge BUY ติดลบ → เปิด BUY ช่วย
            success = trade.Buy(helpVolume, symbol, 0, 0, 0,
                              "Hedge-Help-" + TicketToStr(hedgeTicket));
        }
        else
        {
            // Hedge SELL ติดลบ → เปิด SELL ช่วย  
            success = trade.Sell(helpVolume, symbol, 0, 0, 0,
                               "Hedge-Help-" + TicketToStr(hedgeTicket));
        }
        ...
        if(success)
        {
            Print("🔄 HEDGE HELP OPENED | For: #" + TicketToStr(hedgeTicket) + 
                  " | Lot: " + DoubleToString(helpVolume, 2) +
                  " | Reason: Hedge losing $" + DoubleToString(hedgeProfit, 2));
        }
    }
    else if(hedgeProfit >= 25.0) // Hedge กำไรมาก
    {
        // ✅ ปิดบางส่วนเพื่อ lock profit
        double closeVolume = hedgeVolume * 0.3; // ปิด 30%
...
bool ClosePartialPosition(ulong ticket, double closeVolume, string reason)
{
    ...
    if(closeVolume >= currentVolume) return false; // ไม่ปิดทั้งหมด
    
    string symbol = PositionGetString(POSITION_SYMBOL);
    ENUM_POSITION_TYPE type = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
    
    // ปิดบางส่วนโดยเปิดตำแหน่งตรงข้าม
    trade.SetExpertMagicNumber(MAGIC_HEDGE);
    bool success = false;
    ...
    if(success)
    {
        Print("📉 PARTIAL CLOSE | #" + TicketToStr(ticket) + 
...
ulong FindMainTicketForHedge(ulong hedgeTicket)
{
    ...
    // หาตำแหน่งหลักที่ตรงข้ามกับ hedge
    for(int i = 0; i < PositionsTotal(); i++)
    {
        ulong ticket = PositionGetTicket(i);
        if(!PositionSelectByTicket(ticket)) continue;
        ...
        // เป็นตำแหน่งหลัก + สกุลเงินเดียวกัน + ตรงข้ามกับ hedge
...
// +------------------------------------------------------------------+
//| Orphan Universal - ผู้จัดการงาน                                  |
//+------------------------------------------------------------------+
void ManageOrphanUniversal()
{
    if(!EnableUniversalOrphan) return;
    
    // 1. เช็คว่ามี job ว่างไหม (ตาม MaxOrphansPerCycle)
    ...
    // 2. หา Orphan Main Ticket ที่ยังไม่มี job
    ...
    {
        Print("🏭 ORPHAN UNIVERSAL: New Job Created | Job ID: " + IntegerToString(nextJobId-1) + 
...
{
    // 1. ตรวจสอบจาก Recovery Sets ที่ Main หาย
    for(int i = 0; i < totalRecoverySets; i++)
    {
        if(recoverySets[i].isCompleted) continue;
        ...
        if(!PositionSelectByTicket(recoverySets[i].mainTicket))
        {
            // พบ Main ที่หาย → ตรวจสอบว่าไม่มี job นี้อยู่แล้ว
    ...
    // 2. ตรวจสอบจากตำแหน่งอื่นๆ ที่อาจเป็น Orphan
    for(int i = 0; i < PositionsTotal(); i++)
    {
        ulong ticket = PositionGetTicket(i);
        if(!PositionSelectByTicket(ticket)) continue;
...
bool CreateOrphanJob(ulong mainTicket)
{
    ...
    {
        Print("❌ ORPHAN UNIVERSAL: Maximum jobs capacity reached (10)");
        return false;
    }
    ...
    // หา Orphan tickets ที่เกี่ยวข้องกับ main นี้
    ...
    Print("✅ ORPHAN JOB CREATED | Job ID: " + IntegerToString(newJob.jobId) + 
          " | Main: #" + TicketToStr(mainTicket) +
...
// +------------------------------------------------------------------+
//| Orphan Worker - พนักงานจัดการ job                               |
//+------------------------------------------------------------------+
void ManageOrphanWorkers()
{
    ...
    for(int i = 0; i < totalActiveJobs; i++)
    {
        ...
        // ตรวจสอบว่า job นี้เสร็จแล้วหรือยัง
        ...
        {
            // ยังไม่เสร็จ → จัดการ orphan tickets
            ManageOrphanTicketsInJob(i);
        }
    }
    
    // ลบ job ที่เสร็จแล้วออกจาก array
    if(completedJobs > 0)
    {
        CompactOrphanJobsArray();
        Print("🧹 ORPHAN WORKER: Completed " + IntegerToString(completedJobs) + " jobs | Remaining: " + IntegerToString(totalActiveJobs));
        
        // แจ้ง Universal ให้หางานใหม่
        if(totalActiveJobs < MaxOrphansPerCycle)
        {
            ManageOrphanUniversal();
        }
...
bool CheckOrphanJobCompletion(int jobIndex)
{
    ...
    // เงื่อนไข job เสร็จ: ถึง TP หรือไม่มี orphan เหลือ
    ...
    {
        Print("🎯 ORPHAN JOB READY FOR COMPLETION | Job ID: " + IntegerToString(activeOrphanJobs[jobIndex].jobId) + 
...
{
    // ปิด orphan tickets ทั้งหมดใน job นี้
    int closedCount = 0;
    double totalProfit = 0;
    ...
    Print("✅ ORPHAN JOB COMPLETED | Job ID: " + IntegerToString(activeOrphanJobs[jobIndex].jobId) + 
...
// +------------------------------------------------------------------+
//| Manage Orphan Tickets In Job (แบบใช้ฟังก์ชันใหม่)              |
//+------------------------------------------------------------------+
void ManageOrphanTicketsInJob(int jobIndex)
{
    // จัดการ orphan tickets ใน job (ช่วยเหลือเพิ่มเติมหากจำเป็น)
    for(int i = 0; i < activeOrphanJobs[jobIndex].orphanCount; i++)
    {
        ulong ticket = activeOrphanJobs[jobIndex].orphanTickets[i];
        if(PositionSelectByTicket(ticket))
        {
            ...
            // หาก orphan ติดลบมาก → พิจารณาช่วยเหลือ
            if(profit <= -localOrphanTrigger)
            {
                int helpCount = CountHelpForOrphan(ticket);
                if(helpCount < 3) // จำกัด help สูงสุด 3 ครั้ง
                {
...
// +------------------------------------------------------------------+
//| Count Help For Orphan (นับจำนวนครั้งที่ช่วยเหลือ Orphan)      |
//+------------------------------------------------------------------+
int CountHelpForOrphan(ulong orphanTicket)
{
    ...
    for(int i = 0; i < PositionsTotal(); i++)
    {
        ...
        {
            string comment = PositionGetString(POSITION_COMMENT);
            // นับตำแหน่งช่วยเหลือที่มี ticket นี้อยู่ใน comment
...
void OpenOrphanHelpWithinJob(ulong orphanTicket, int jobIndex)
{
    ...
    // จำกัดจำนวน help ภายใน job เดียว
    int helpCountInJob = CountHelpInJob(jobIndex);
    if(helpCountInJob >= 3) // จำกัด help 3 ครั้งต่อ job
    {
    ...
    if(!CheckMargin(helpLot, symbol)) 
    {
        Print("❌ Insufficient margin for orphan help: " + DoubleToString(helpLot, 2));
        return;
    }
    ...
    if(success)
    {
        Print("🆘 ORPHAN JOB HELP | Job ID: " + IntegerToString(activeOrphanJobs[jobIndex].jobId) + 
    ...
    {
        Print("❌ Failed to open orphan help: " + trade.ResultRetcodeDescription());
    }
}
// +------------------------------------------------------------------+
...
void FindOrphanTicketsForMain(ulong mainTicket, OrphanJob &job)
{
    ...
    if(setIndex != -1)
    {
        for(int i = 0; i < recoverySets[setIndex].subCount; i++)
        {
            if(job.orphanCount < 20) // จำกัดจำนวน
            {
    ...
    for(int i = 0; i < PositionsTotal(); i++)
    {
        ...
        // ตรวจสอบว่าเป็น orphan ของ main นี้
...
void RegisterOrphan(ulong ticket, long magic, string comment)
{
    ...
    if(PositionSelectByTicket(ticket))
    {
        ...
        Print("🆘 ORPHAN REGISTERED: #" + TicketToStr(ticket) + 
            ...
            for(int j = i + 1; j < PositionsTotal(); j++)
            {
                ...
                if(PositionSelectByTicket(ticket2))
                {
                    ...
                    if(symbol1 == symbol2 && type1 != type2)
                    {
                        ...
                        // ✅ หา Target Profit ที่ถูกต้องสำหรับกลยุทธ์นี้
                        ...
                        if(totalProfit >= correctTargetProfit) 
                        {
                            if(trade.PositionClose(ticket1) && trade.PositionClose(ticket2))
                            {
                                Print("🎯 HEDGE PAIR CLOSED | " +
                                      "Strategies: " + strategyPair + " | " +
                                      "Profit: $" + DoubleToString(totalProfit, 2) + " | " +
                                      "Target: $" + DoubleToString(correctTargetProfit, 2));
                                
                                Print("🎯 HEDGE TARGET REACHED | " + strategyPair + " | Profit: $" + DoubleToString(totalProfit, 2));
                               
                            }
                        }
                        else if(totalProfit > 0)
                        {
                            // Debug: แสดงว่ากำลังติดตามคู่นี้
                            Print("📊 Monitoring: " + strategyPair + 
...
void PrioritizeAndExecuteHelp()
{
    ...
    {
        Print("🎯 PRIORITY HELP: Ticket #" + TicketToStr(mostNeedyTicket) + " | Score: " + DoubleToString(highestPriority, 1));
...
void ExecuteSmartBatchClosing()
{
    ...
    if(totalProfit >= SmartCloseProfitTarget)
    {
        Print("🎉 BATCH CLOSING: Total profit target reached: $" + DoubleToString(totalProfit, 2));
        CloseAllPositions("ExecuteSmartBatchClosing", "SmartCloseProfitTarget");
    }
}
*/
//+------------------------------------------------------------------+
//| IsMarketGoodForTrading - ตรวจสอบสภาพตลาดที่เหมาะสม             |
//+------------------------------------------------------------------+
bool IsMarketGoodForTrading(string symbol)
{
    ...
    if(atrPercent < 0.03) // ATR ต่ำเกินไป (ตลาดเงียบ)
    {
        DebugPrint(2, "❌ Market too quiet - ATR: " + DoubleToString(atrPercent, 3) + "%");
        return false;
    }
    
    if(atrPercent > 0.5) // ATR สูงเกินไป (ตลาดผันผวน)
    {
        DebugPrint(2, "❌ Market too volatile - ATR: " + DoubleToString(atrPercent, 3) + "%");
        return false;
    }
    
    // 2. ✅ ตรวจสอบ Session Time
    MqlDateTime timeStruct;
    ...
    {
        DebugPrint(2, "❌ Weekend - no trading");
        return false;
    }
    
    // ตรวจสอบตามประเภทสัญลักษณ์
    if(StringFind(sym, "XAU") != -1) // Gold
    {
        // Gold - เวลาเทรดที่ดี 08:00-22:00 (Session London/NY)
        if(hour < 8 || hour >= 22)
        {
            DebugPrint(2, "❌ Gold: Outside trading hours (" + IntegerToString(hour) + ":00)");
            return false;
        }
    }
    ...
    {
        // Forex - เวลาเทรดที่ดี 02:00-20:00 (Session Overlap)
        if(hour < 2 || hour >= 20)
        {
            DebugPrint(2, "❌ Forex: Outside trading hours (" + IntegerToString(hour) + ":00)");
            return false;
        }
    }
    else // สัญลักษณ์อื่นๆ
    {
        // เทรดเฉพาะวันจันทร์-ศุกร์ 08:00-18:00
        if(hour < 8 || hour >= 18)
        {
            DebugPrint(2, "❌ Other: Outside trading hours (" + IntegerToString(hour) + ":00)");
            return false;
        }
    }
    ...
    if(spreadPips > 3.0)
    {
        DebugPrint(2, "❌ Spread too high: " + DoubleToString(spreadPips, 1) + " pips");
        return false;
    }
    
    DebugPrint(2, "✅ Market conditions good for " + symbol);
    return true;
}
//+------------------------------------------------------------------+
//| GetTradingSession - ตรวจสอบ Session ปัจจุบัน                   |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| IsSessionGoodForTrading - ตรวจสอบว่า Session นี้เหมาะสำหรับเทรด |
//+------------------------------------------------------------------+
bool IsSessionGoodForTrading(string symbol)
{
    ...
    if(StringFind(sym, "XAU") != -1) // Gold
    {
        // Gold ดีที่สุดในช่วง London และ NY Overlap
        return (session == "LONDON" || session == "LONDON_NY_OVERLAP" || session == "NY");
    }
    else // Forex
    {
        // Forex ดีที่สุดในช่วง Session Overlap
...
void InitializeCollaborativeSystem()
{
    ...
    Print("=== 🤝 COOPERATIVE SYSTEM INITIALIZED ===");
    for(int i = 0; i < 5; i++) {
...
void BroadcastRecoveryMode(bool isRecoveryActive) {
   if(isRecoveryActive) {
      Print("🆘 BROADCAST: Entering Recovery Mode - All strategies cooperate!");
      BroadcastToAllSystems("RECOVERY_MODE_ACTIVATED");
      
      // ตั้งค่าสถานะ Recovery ในระบบต่างๆ
      recoveryModeActive = true;
      SystemMode = MODE_RECOVERY;
      
   } else {
      Print("✅ BROADCAST: Exiting Recovery Mode - Returning to normal!");
      BroadcastToAllSystems("RECOVERY_MODE_ENDED");
      
      // ตั้งค่าสถานะปกติในระบบต่างๆ
      recoveryModeActive = false;
      SystemMode = MODE_NORMAL;
   }
}
...
{
    // กลยุทธ์สนับสนุนลดกิจกรรมเพื่อให้ผู้นำ Recovery ทำงานได้ดีขึ้น
    Print("   🤝 " + strategies[supporterIndex].name + " supporting " + strategies[leaderIndex].name);
    
    // สามารถเพิ่ม logic การสนับสนุนเฉพาะที่นี่
    // เช่น ลดขนาดลอต, ลดความถี่การเทรด, ปิดออร์เดอร์บางส่วน
}

// 📍 เพิ่มฟังก์ชั่นใหม่
void AdjustStrategyWeightsForRecovery()
{
    // ในโหมด Recovery ให้เพิ่มน้ำหนักกลยุทธ์ที่ทำผลงานดี
    int bestPerformer = FindBestPerformingStrategy();
    
    if(bestPerformer != -1) {
        // เพิ่มน้ำหนักให้กลยุทธ์ที่ดีที่สุดชั่วคราว
        double tempWeight = StrategyWeight_Scalp1;
        
        // สามารถปรับน้ำหนักตามสถานะได้ที่นี่
        Print("📊 Temporary weight adjustment for recovery mode");
    }
}
//+------------------------------------------------------------------+
...
{
    // 🔥 หากลยุทธ์ที่ทำผลงานดีที่สุดมาเป็นผู้นำ Recovery
    int bestPerformer = FindBestPerformingStrategy();
    
    if(bestPerformer != -1) {
        Print("🏆 RECOVERY LEADER: " + strategies[bestPerformer].name + 
              " leading recovery operations");
        
        // กลยุทธ์อื่นๆ ช่วยสนับสนุน
        for(int i = 0; i < 5; i++) {
    ...
    // 🔥 ปรับน้ำหนักกลยุทธ์ชั่วคราวในโหมด Recovery
...
{
    // ใช้ if-else chain เพื่อหลีกเลี่ยงปัญหา type conversion
    ...
    return -1; // ไม่พบ strategy
}
//+------------------------------------------------------------------+
//| Manage Cooperative Strategies                                   |
//+------------------------------------------------------------------+
void ManageCooperativeStrategies(ENUM_MARKET_REGIME_DETAILED regime = MARKET_REGIME_NORMAL) 
{
    ...
    for(int i = 0; i < 5; i++) 
    {
        ...
        {
            string status = (strategies[i].progress >= 80) ? "🎯 NEAR TARGET" : "📊 WORKING";
            if(strategies[i].progress >= 100) status = "✅ TARGET ACHIEVED";
    ...
    for(int i = 0; i < 5; i++) 
    {
        if(strategies[i].isActive) 
        {
            switch(regime)
            {
                ...
                case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
                    // ปิดบาง strategies ในตลาดผันผวน
...
void SetupMissionTeam() {
    // 🎯 TRADING TEAMS (5 ทีม)
    strategyMissionTeam[0] = "QUICK_STRIKE_TEAM";     // Scalp1 - ทีมจู่โจมเร็ว
    strategyMissionTeam[1] = "INCOME_SUPPORT_TEAM";   // Scalp2 - ทีมสนับสนุนรายได้  
    strategyMissionTeam[2] = "SPECIAL_OPS_TEAM";      // Scalp3 - ทีมปฏิบัติการพิเศษ
    strategyMissionTeam[3] = "TREND_CONTROL_TEAM";    // Trend - ทีมควบคุมเทรนด์
    strategyMissionTeam[4] = "BREAKOUT_SQUAD_TEAM";   // Breakout - ทีมจับจุดแตกหัก
    
    // 🛡️ RECOVERY TEAMS (6 ทีม)
    ...
    // ✅ ทุกทีมใช้ลอตปกติ 100%
    Mission.lotSize = 1.0; // 100%
    
    Print("✅ MISSION TEAMS SETUP: 5 Trading + 6 Recovery Teams Ready!");
}
// ✅ ฟังก์ชั่นใหม่: ติดตามความคืบหน้าทีม
void UpdateTeamProgress() {
    double totalMissionProgress = 0;
    int activeTeams = 0;
    int totalTeams = 11; // 5 Trading + 6 Recovery
    
    // 🔄 อัพเดท progress ทั้ง 11 ทีม
    for(int i = 0; i < totalTeams; i++) {
        ...
        // แจ้งเตือนเมื่อทีมใกล้บรรลุเป้าหมาย
        if(teamProgress >= 80) {
            Print("🎯 " + strategyMissionTeam[i] + " nearing target: " + 
    ...
    if(averageProgress >= 70 && activeTeams >= 6) { // 70% โดยเฉลี่ย และ 6 ทีมทำงาน
        ...
        // 🎯 แจ้งเตือนไปยังทุกระบบ
        BroadcastToAllSystems("MISSION_COMPLETED");
    }
}
// ✅ ฟังก์ชั่นใหม่: แสดงแดชบอร์ดทีม
void DisplayTeamDashboard() {
    string teamDashboard = "=== 🤝 FULL TEAM COOPERATION DASHBOARD ===\n";
    ...
    for(int i = 0; i < 5; i++) {
        string statusIcon = (strategyMissionProgress[i] > 0) ? "✅" : "⏸️";
    ...
    for(int i = 5; i < totalTeams; i++) {
        string statusIcon = (IsRecoveryTeamActive(i)) ? "🛡️" : "💤";
        teamDashboard += StringFormat("%s %-20s: %5.1f%%\n", 
                          statusIcon, 
                          strategyMissionTeam[i], 
                          strategyMissionProgress[i]);
    ...
    teamDashboard += "----------------------------------------\n";
    teamDashboard += StringFormat("🎯 TOTAL MISSION PROGRESS: %.1f%%\n", totalProgress / totalTeams);
    teamDashboard += StringFormat("📊 ACTIVE TEAMS: %d/%d\n", CountActiveTeams(), totalTeams);
    teamDashboard += StringFormat("💰 DAILY TARGET: $%.2f\n", CalculateDailyMissionTarget());
    
    Comment(teamDashboard);
}

bool IsRecoveryTeamActive(int teamIndex) {
    // ตรวจสอบว่า Recovery Team กำลังทำงานอยู่ไหม
    switch(teamIndex) {
...
double CalculateTeamProgress(string teamName) {
    // TRADING TEAMS (ใช้ profit-based progress)
...
bool IsIndividualRecoveryActive() {
    // ตรวจสอบว่า Individual Recovery กำลังทำงาน (ใช้วิธีง่ายๆ)
    return EnableIndividualRecovery; // แค่เปิดใช้งานก็นับว่าทำงาน
}

bool IsStepUpRecoveryActive() {
    // ตรวจสอบว่า Step-Up Recovery กำลังทำงาน
    return StepUpRecoveryEnabled;
}

bool IsCounterPositionActive() {
    // ตรวจสอบว่า Counter Position กำลังทำงาน  
    return EnableCounterPosition;
}

bool IsHedgeActive() {
    // ตรวจสอบว่า Hedge กำลังทำงาน
    return UseEmergencyHedge;
}

bool IsOrphanManagementActive() {
    // ตรวจสอบว่า Orphan Management กำลังทำงาน
    return EnableUniversalOrphan;
}

bool IsUltimateFightingActive() {
    // ตรวจสอบว่า Ultimate Fighting กำลังทำงาน
...
double CalculateDailyMissionTarget() {
    // ใช้ existing collaborative target
...
// ✅ ฟังก์ชั่นตรวจสอบสภาพตลาด (ต้องเพิ่ม)

bool IsMarketRanging() {
    // ใช้ iATR แบบ MQL5 ที่ถูกต้อง
    double atrArray[];
...
bool IsMarketTrending() {
    // ใช้ ADX เพื่อตรวจสอบความแรงของเทรนด์
    double adxArray[];
    ...
    return (adx > 25.0); // ADX > 25 = เทรนด์แรง
}
...
string GetDetailedMarketStatus()
{
    ...
    // 4. 📉 Trend Analysis (Multiple Timeframes)
...
string GetMultiTimeframeTrendStatus()
{
    ...
    for(int i = 0; i < 5; i++) {
        ...
        if(strength > 0.7) status += "💪";
        else if(strength > 0.4) status += "👍";
        else status += "👎";
...
{
    string status = "--- 📊 SUPPORT & RESISTANCE ---\n";
...
string GetVolumeMomentumStatus()
{
    ...
    string volumeStatus = volumeRatio > 1.5 ? "📈 HIGH" : volumeRatio > 0.8 ? "📊 NORMAL" : "📉 LOW";
...
{
    string status = "--- ⚠️ RISK ASSESSMENT ---\n";
    ...
    if(riskScore >= 6) {
        status += "⚠️ RECOMMENDATION: Reduce position sizes, avoid trading\n";
    } else if(riskScore >= 3) {
        status += "⚠️ RECOMMENDATION: Trade with caution, use smaller lots\n";
    } else {
        status += "✅ RECOMMENDATION: Normal trading conditions\n";
    }
...
string GetTradingSessionStatus()
{
    ...
    if(session == "ASIA") {
        status += "💡 Focus: JPY pairs, Gold, Conservative trading\n";
    } else if(session == "LONDON") {
        status += "💡 Focus: EUR, GBP pairs, High volatility expected\n";
    } else if(session == "LONDON_NY_OVERLAP") {
        status += "💡 Focus: All major pairs, Highest volatility\n";
    } else if(session == "NY") {
        status += "💡 Focus: USD pairs, Good momentum opportunities\n";
    } else {
        status += "💡 Focus: Light trading, Wait for next session\n";
    }
...
//+------------------------------------------------------------------+
string GetTradingRecommendations(ENUM_MARKET_REGIME_DETAILED regime)
{
    string status = "--- 🎯 TRADING RECOMMENDATIONS ---\n";
    
    switch(regime) {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            status += "✅ STRONG TREND DETECTED\n";
            status += "• Use Trend-following strategies\n";
            status += "• Trade in direction of trend\n";
            status += "• Consider larger position sizes\n";
            status += "• Look for pullback entries\n";
            break;
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            status += "✅ WEAK TREND DETECTED\n";
            status += "• Use conservative trend strategies\n";
            status += "• Smaller position sizes\n";
            status += "• Wait for clear entry signals\n";
            status += "• Consider range strategies as well\n";
            break;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            status += "✅ VOLATILE RANGE DETECTED\n";
            status += "• Use Scalping strategies\n";
            status += "• Trade range boundaries\n";
            status += "• Quick entries and exits\n";
            status += "• Tight stop losses\n";
            break;
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            status += "✅ CALM RANGE DETECTED\n";
            status += "• Reduced trading activity\n";
            status += "• Wait for breakout signals\n";
            status += "• Smaller position sizes\n";
            status += "• Focus on high-probability setups\n";
            break;
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            status += "✅ BREAKOUT POTENTIAL DETECTED\n";
            status += "• Prepare for breakout trades\n";
            status += "• Monitor key support/resistance\n";
            status += "• Have entry orders ready\n";
            status += "• Wait for confirmation\n";
            break;
            
        default: // MARKET_REGIME_NORMAL
            status += "📊 NORMAL MARKET CONDITIONS\n";
            status += "• Use balanced strategies\n";
            status += "• Monitor trend development\n";
            status += "• Be ready for regime changes\n";
            status += "• Use standard position sizing\n";
            break;
    }
...
string GetRecommendedStrategies(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime) {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
...
int CalculateRegimeConfidence(ENUM_MARKET_REGIME_DETAILED regime)
{
...
bool HasMultipleBreakoutConfirmations(bool isBuy)
{
    ...
    // ต้องการอย่างน้อย 3 confirmations
    return (confirmations >= 3);
}

bool IsBreakoutWithTrend(bool isBuy)
{
    // ตรวจสอบว่า breakout ไปในทิศทางเดียวกับ trend หรือไม่
    ...
    if(isBuy)
    {
        // สำหรับ buy breakout ควรอยู่ใน uptrend
        return (ema9 > ema21);
    }
    else
    {
        // สำหรับ sell breakout ควรอยู่ใน downtrend
        return (ema9 < ema21);
    }
}
...
void ExecuteTeamByMarketCondition()
{
   ...
   if(IsMarketRanging() && ConfirmMarketCondition("RANGING", 3)) {
      ...
      Print("🚀 MARKET CONFIRMED BREAKOUT - Breakout Teams Lead");
      if(ShouldTradeInThisRegime("BREAKOUT")) {
         ExecuteTradingTeamsInBreakout();
      }
   }
   else {
      Print("🔄 MARKET UNCLEAR - Conservative Approach");
      ExecuteConservativeTradingApproach();
   }
}
bool ShouldTradeInThisRegime(string regime)
{
   // ตรวจสอบเงื่อนไขเพิ่มเติมก่อนเทรด
   double riskPressure = CalculateRiskPressure();
   int totalPositions = PositionsTotal();
   
   if(riskPressure > 0.8) {
      Print("⚠️ Risk pressure too high for " + regime + " trading");
      return false;
   }
   
   if(totalPositions >= MaxTotalPositions * 0.8) {
      Print("⚠️ Position limit nearly reached for " + regime + " trading");
      return false;
   }
   
   if(!IsMarketGoodForTrading(_Symbol)) {
      Print("⚠️ Market conditions not suitable for " + regime + " trading");
      return false;
   }
   
   return true;
}
// 📍 ฟังก์ชันยืนยันสภาพตลาด (ป้องกัน false signal)
bool ConfirmMarketCondition(string condition, int confirmationCount)
{
   ...
   for(int i = 0; i < confirmationCount; i++) {
      ...
      if(i < confirmationCount - 1) Sleep(1000); // รอ 1 วินาทีระหว่างการตรวจสอบ
   }
...
void ExecuteConservativeTradingApproach()
{
   ...
   if(hasClearSignal && marketScore > 70) {
      ...
      if(!hasClearSignal) {
         Print("⏸️ No clear signals detected");
      } else {
         Print("⏸️ Market score too low: " + DoubleToString(marketScore, 1) + "/100");
      }
...
bool HasClearSupportResistance() {
    ...
    // หาแนวรับ-แนวต้านจาก High/Low ที่เด่นชัด
    int supportLevels = CountSupportLevels(rates, 50);
    int resistanceLevels = CountResistanceLevels(rates, 50);
    
    return (supportLevels >= 2 && resistanceLevels >= 2); // มีอย่างน้อย 2 แนวรับและแนวต้าน
}

int CountSupportLevels(MqlRates &rates[], int count) {
    int supportCount = 0;
    
    // หา Low ที่มีการทดสอบหลายครั้ง (Support)
...
int CountResistanceLevels(MqlRates &rates[], int count) {
    ...
    // หา High ที่มีการทดสอบหลายครั้ง (Resistance)
    for(int i = 2; i < count - 2; i++) {
...
bool IsSupportLevel(MqlRates &rates[], int index, int totalBars) {
    ...
    // ตรวจสอบว่ามีการทดสอบระดับนี้หลายครั้งหรือไม่
    int touchCount = 0;
    ...
    for(int i = 0; i < totalBars; i++) {
        ...
        // ถ้าราคาเคลื่อนที่ต่ำกว่าระดับนี้มากเกินไป → ไม่ใช่ Support
        if(rates[i].low < currentLow - tolerance * 3) {
            return false;
        }
    }
    
    return (touchCount >= 3); // ถูกทดสอบอย่างน้อย 3 ครั้ง
}
...
bool IsResistanceLevel(MqlRates &rates[], int index, int totalBars) {
    ...
    for(int i = 0; i < totalBars; i++) {
        ...
        // ถ้าราคาเคลื่อนที่สูงกว่าระดับนี้มากเกินไป → ไม่ใช่ Resistance
...
bool IsRangeContraction() {
    ...
    return (recentRange < previousRange * 0.7); // Range หดตัวมากกว่า 30%
}
...
bool IsRangeExpansion() {
    ...
    return (recentRange > previousRange * 1.3); // Range ขยายตัวมากกว่า 30%
}
...
bool IsRangeAboutToBreak() {
    ...
    // ตรวจสอบว่าราคาอยู่ที่ขอบ Range และมี Volume เพิ่มขึ้น
    double highest = rates[0].high;
    double lowest = rates[0].low;
    for(int i = 1; i < 30; i++) {
        if(rates[i].high > highest) highest = rates[i].high;
...
bool HasBreakoutPotential() {
    ...
    // ตรวจสอบว่า Range แคบลง (Coiling) → มีโอกาส Breakout สูง
    double recentRanges[5];
    for(int i = 0; i < 5; i++) {
        recentRanges[i] = rates[i].high - rates[i].low;
    }
    
    // เรียง Range จากเก่าที่สุดไปใหม่ที่สุด
    bool isCoiling = true;
    for(int i = 1; i < 5; i++) {
...
void ExecuteTradingTeamsInRange() {
    // 📊 ตลาดไซด์เวย์ → ทีม Scalp ทำงานเต็มที่
    Print("📊 MARKET RANGING - Scalp Teams Taking Lead");
    
    // 🎯 QUICK_STRIKE_TEAM (Scalp1) - จู่โจมเร็วในตลาดไซด์เวย์
    if(Scalp1Enabled && ShouldOpenPosition("Scalp1")) {
        if(IsLowVolatility() && IsInRangeZone()) {
            ExecuteScalp1();  // ✅ ทำงานเต็มที่
            Print("⚡ QUICK_STRIKE: Scalp1 executed in ranging market");
        }
    }
    
    // 🎯 INCOME_SUPPORT_TEAM (Scalp2) - สร้างรายได้สม่ำเสมอ
    if(Scalp2Enabled && ShouldOpenPosition("Scalp2")) {
        if(IsStableRange() && HasClearSupportResistance()) {
            ExecuteScalp2();  // ✅ ทำงานเต็มที่
            Print("💰 INCOME_SUPPORT: Scalp2 adding consistent income");
        }
    }
    
    // 🎯 SPECIAL_OPS_TEAM (Scalp3) - จัดการสถานการณ์พิเศษ
    if(Scalp3_Enabled && ShouldOpenPosition("Scalp3")) {
        if(IsRangeContraction() || IsRangeExpansion()) {
            ExecuteScalp3Integrated();  // ✅ ทำงานเต็มที่
            Print("🎯 SPECIAL_OPS: Scalp3 handling special range conditions");
        }
    }
    
    // 🎯 TREND_CONTROL_TEAM (Trend) - รอเทรนด์ชัด
    if(SimpleTrendScalpEnabled && ShouldOpenPosition("Trend")) {
        if(IsRangeAboutToBreak() && HasTrendSignal()) {
            ExecuteSimpleTrendScalp();  // ✅ ทำงานแต่รอสัญญาณชัด
            Print("📈 TREND_CONTROL: Trend team waiting for clear trend");
        }
    }
    
    // 🎯 BREAKOUT_SQUAD_TEAM (Breakout) - เตรียมพร้อมจับ Breakout
    if(BreakoutStrategy && ShouldOpenPosition("Breakout")) {
        if(IsAtRangeBoundary() && HasBreakoutPotential()) {
            // ✅ อาจเปิดออร์เดอร์เล็กน้อยเพื่อจับ Breakout เบื้องต้น
            ExecuteAdvancedBreakout();
            Print("🚀 BREAKOUT_SQUAD: Monitoring range boundaries for breakout");
        }
    }
}

void ExecuteTradingTeamsInTrend() {
    // 📈 ตลาดเทรนด์ → ทีม Trend ทำงานเต็มที่
    Print("📈 MARKET TRENDING - Trend Teams Taking Lead");
    
    // 🎯 TREND_CONTROL_TEAM (Trend) - ควบคุมเทรนด์หลัก
    if(SimpleTrendScalpEnabled && ShouldOpenPosition("Trend")) {
        ExecuteSimpleTrendScalp();  // ✅ ทำงานเต็มที่
        Print("📈 TREND_CONTROL: Riding the strong trend");
    }
    
    // 🎯 BREAKOUT_SQUAD_TEAM (Breakout) - จับ Breakout ในเทรนด์
    if(BreakoutStrategy && ShouldOpenPosition("Breakout")) {
        if(IsTrendContinuationBreakout()) {
            ExecuteAdvancedBreakout();  // ✅ ทำงานเต็มที่
            Print("🚀 BREAKOUT_SQUAD: Capturing trend continuation breakouts");
        }
    }
    
    // 🎯 QUICK_STRIKE_TEAM (Scalp1) - จับ Pullback
    if(Scalp1Enabled && ShouldOpenPosition("Scalp1")) {
        if(IsTrendPullback() && IsShallowRetracement()) {
            ExecuteScalp1();  // ✅ ทำงาน (จับ Pullback)
            Print("⚡ QUICK_STRIKE: Scalping trend pullbacks");
        }
    }
    
    // 🎯 INCOME_SUPPORT_TEAM (Scalp2) - สนับสนุนในเทรนด์
    if(Scalp2Enabled && ShouldOpenPosition("Scalp2")) {
        if(IsTrendConsolidation()) {
            ExecuteScalp2();  // ✅ ทำงาน (จับ Consolidation)
            Print("💰 INCOME_SUPPORT: Trading trend consolidations");
        }
    }
    
    // 🎯 SPECIAL_OPS_TEAM (Scalp3) - จัดการเทรนด์พิเศษ
    if(Scalp3_Enabled && ShouldOpenPosition("Scalp3")) {
        if(IsTrendExhaustion() || IsTrendReversalSignal()) {
            ExecuteScalp3Integrated();  // ✅ ทำงาน (จับ Reversal)
            Print("🎯 SPECIAL_OPS: Monitoring trend exhaustion signals");
        }
    }
}
//--//
...
bool HasTrendSignal()
{
    ...
    // เทรนด์ขึ้น: EMA เรียงตัวกัน
    bool uptrend = (ema9 > ema21 && ema21 > ema50);
    // เทรนด์ลง: EMA เรียงตัวกัน
...
bool IsShallowRetracement() {
    ...
    // ตรวจสอบว่า Retracement ไม่ลึกเกิน 38.2%
    if(IsUptrend()) {
...
bool IsTrendConsolidation() {
    ...
    // ตรวจสอบการ Consolidation ในเทรนด์
    double totalRange = 0;
    for(int i = 0; i < 10; i++) {
...
bool IsTrendReversalSignal() {
    // ตรวจสอบสัญญาณกลับเทรนด์จาก Multiple Timeframes
...
bool CheckReversalOnTimeframe(ENUM_TIMEFRAMES tf) {
    ...
    switch(tf) {
        ...
        case PERIOD_M5:
            ...
            break;
        default:
            // สำหรับ timeframe อื่นๆ สร้าง handle ชั่วคราว
    ...
    // ถ้าเป็น timeframe ที่ไม่ใช่ predefined ให้ release handles
...
bool CheckMomentumReversal(ENUM_TIMEFRAMES tf) {
    // สร้าง handles ชั่วคราวสำหรับ timeframe นี้
...
// 🔧 ฟังก์ชั่น GetATRValue (ถ้ายังไม่มี)
...
bool IsNewTrendConfirmed() {
    ...
    // ตรวจสอบว่า Breakout นำไปสู่เทรนด์ใหม่
...
bool IsBreakoutMomentum() {
    ...
    // ตรวจสอบโมเมนตัมหลัง Breakout
    double volume1 = GetVolume(_Symbol, PERIOD_M5, 0);
    double volume2 = GetVolume(_Symbol, PERIOD_M5, 1);
    
    bool strongMomentum = (volume1 > volume2 * 1.3) && // Volume เพิ่มขึ้น
                         (MathAbs(rates[0].close - rates[1].close) > 
                          MathAbs(rates[1].close - rates[2].close) * 1.5); // Price movement เพิ่มขึ้น
    
    return strongMomentum;
}

bool IsLowRiskEntry() {
    // ตรวจสอบว่าเป็นจุดเข้าเทรดความเสี่ยงต่ำ
    double spread = SymbolInfoInteger(_Symbol, SYMBOL_SPREAD) * _Point;
...
bool IsBreakoutRetest() {
    ...
    // ตรวจสอบการ Retest หลัง Breakout
    bool resistanceBreakRetest = (rates[4].high < rates[3].high &&  // ก่อน Breakout
...
bool IsFalseBreakout() {
    ...
    // ตรวจสอบ False Breakout (Breakout แล้วกลับเข้ามาใน Range)
    bool falseBreakoutUp = (rates[2].close > rates[3].high &&  // Breakout ขึ้น
                           rates[1].close < rates[3].high &&   // กลับเข้า Range
...
bool IsBreakoutFailure() {
    ...
    // ตรวจสอบ Breakout ล้มเหลว (ไม่มีโมเมนตัมตาม)
    bool failureUp = (rates[3].close > rates[2].high &&  // Breakout
                     rates[2].close < rates[3].close &&  // ไม่มี follow-through
                     rates[1].close < rates[2].close &&
                     rates[0].close < rates[1].close);
...
bool IsTrendContinuationBreakout() {
    ...
    // ตรวจสอบ Breakout ที่ต่อเนื่องกับเทรนด์เดิม
...
// 🔧 ฟังก์ชั่นตรวจสอบ Breakout Signal
bool IsCleanBreakoutSignal() {
    MqlRates rates[];
    if(CopyRates(_Symbol, PERIOD_H1, 0, 3, rates) < 3) return false;
    
    // ตรวจสอบว่าเป็น Breakout ที่ clean
...
void ExecuteTradingTeamsInBreakout() {
    // 🚀 ตลาดเบรกเอ้าท์ → ทีม Breakout ทำงานเต็มที่
    Print("🚀 MARKET BREAKOUT - Breakout Teams Taking Lead");
    
    // 🎯 BREAKOUT_SQUAD_TEAM (Breakout) - จับ Breakout หลัก
    if(BreakoutStrategy && ShouldOpenPosition("Breakout")) {
        if(IsCleanBreakoutSignal()) {
            ExecuteAdvancedBreakout();  // ✅ ทำงานเต็มที่
            Print("🚀 BREAKOUT_SQUAD: Executing breakout strategy");
        }
    }
    
    // 🎯 TREND_CONTROL_TEAM (Trend) - ตามเทรนด์ใหม่
    if(SimpleTrendScalpEnabled && ShouldOpenPosition("Trend")) {
        if(IsNewTrendConfirmed()) {
            ExecuteSimpleTrendScalp();  // ✅ ทำงานเต็มที่
            Print("📈 TREND_CONTROL: Following new trend after breakout");
        }
    }
    
    // 🎯 QUICK_STRIKE_TEAM (Scalp1) - จับโมเมนตัมหลัง Breakout
    if(Scalp1Enabled && ShouldOpenPosition("Scalp1")) {
        if(IsBreakoutMomentum() && IsLowRiskEntry()) {
            ExecuteScalp1();  // ✅ ทำงาน (จับโมเมนตัม)
            Print("⚡ QUICK_STRIKE: Scalping breakout momentum");
        }
    }
    
    // 🎯 INCOME_SUPPORT_TEAM (Scalp2) - สนับสนุนหลัง Breakout
    if(Scalp2Enabled && ShouldOpenPosition("Scalp2")) {
        if(IsBreakoutRetest()) {
            ExecuteScalp2();  // ✅ ทำงาน (จับ Retest)
            Print("💰 INCOME_SUPPORT: Trading breakout retests");
        }
    }
    
    // 🎯 SPECIAL_OPS_TEAM (Scalp3) - จัดการ False Breakout
    if(Scalp3_Enabled && ShouldOpenPosition("Scalp3")) {
        if(IsFalseBreakout() || IsBreakoutFailure()) {
            ExecuteScalp3Integrated();  // ✅ ทำงาน (จัดการ False Breakout)
            Print("🎯 SPECIAL_OPS: Handling false breakout scenarios");
        }
    }
}

void ExecuteAllTradingTeams() {
    // 🔄 ตลาดไม่ชัดเจน → ทุกทีมทำงานตามสัญญาณปกติ
    Print("🔄 MARKET UNCLEAR - All Teams Working Based on Signals");
    
    // ✅ ทุกกลยุทธ์ทำงานตามสัญญาณของตัวเอง
    if(Scalp1Enabled && ShouldOpenPosition("Scalp1")) {
        ExecuteScalp1();
    }
...
bool IsLowVolatility() {
    ...
    return (rangePercent < 0.05); // Volatility ต่ำ
}

bool IsInRangeZone() {
    // ตรวจสอบว่าอยู่ในโซนกลางของ Range
    MqlRates rates[];
    if(CopyRates(_Symbol, PERIOD_H1, 0, 20, rates) < 20) return false;
    ...
    return (deviation < 30); // อยู่ภายใน 30% จากกลาง Range
}

bool IsStableRange() {
    // ตรวจสอบว่า Range มีความเสถียร
    MqlRates rates[];
    if(CopyRates(_Symbol, PERIOD_H1, 0, 10, rates) < 10) return false;
    ...
    // คำนวณ Standard Deviation ของ Ranges
    double mean = 0;
    ...
    return (cv < 30); // Range มีความเสถียร (CV < 30%)
}
...
bool IsAtRangeBoundary() {
    ...
    // ตรวจสอบว่าใกล้ขอบ range หรือไม่
    ...
    if(orderType == ORDER_TYPE_BUY)  //
    {
        // ซื้อที่ขอบล่างของ range
        return (distanceFromLow < rangeSize * 0.1);
    }
    else if(orderType == ORDER_TYPE_SELL) 
    {
        // ขายที่ขอบบนของ range
...
void ExecuteRecoveryTeams() {
    // 🛡️ ทุก Recovery Teams ทำงานตามระบบของตัวเอง
    ...
    ManageCounterPositions(ENUM_MARKET_REGIME_DETAILED currentRegime);         // COUNTER_ATTACK_TEAM
    }
...
void ExecuteCoordinatedStrategies()
{
    ...
    // 🔥 ตรวจสอบโหมด Recovery
    if(SystemMode == MODE_RECOVERY) {
        ExecuteFullTeamCooperation();  // ✅ ใหม่: ใช้ระบบทีมเต็มรูปแบบ
        return;
    }
    
    // 🔥 โหมดปกติ: ทำงานตามเดิม
    int mostNeedyStrategy = FindMostNeedyStrategy();
    
    if(mostNeedyStrategy == -1) return;
    
    Print("🎯 COORDINATOR: Executing " + strategies[mostNeedyStrategy].name + " (Most needy)");
    
    // ✅ ใช้ฟังก์ชั่นที่มีอยู่จริงทั้งหมด
    ...
    // ✅ เรียกใช้ Recovery Systems แยกต่างหาก
    ExecuteRecoverySystems();
    
    // ✅ เรียกใช้ AI Systems ถ้าเปิดใช้งาน
    ExecuteAISystems();
    
    // ✅ เรียกใช้ Low Risk Opportunities
    ExecuteLowRiskOpportunities();
}

// ✅ ฟังก์ชั่นใหม่: ระบบทีมเต็มรูปแบบ
void ExecuteFullTeamCooperation() {
    Print("🤝 FULL TEAM COOPERATION ACTIVATED");
    
    // ✅ เรียกประชุมทีมครั้งแรก
    ...
    // ✅ อัพเดทความคืบหน้าทีม
    UpdateTeamProgress();
    
    // ✅ แสดงแดชบอร์ด
    DisplayTeamDashboard();
    
    // ✅ เรียก Recovery Systems ตามปกติ
    ExecuteRecoverySystems();
}

// ✅ ฟังก์ชั่นใหม่: ประชุมทีม
void TeamBriefing() {
    Print("=== 🎯 MISSION BRIEFING ===");
    
    // 📊 แจ้งเป้าหมายร่วมกัน
    double dailyTarget = CalculateDailyMissionTarget();
    Print("🎯 DAILY MISSION: $" + DoubleToString(dailyTarget, 2));
    
    // 👥 แจงบทบาทแต่ละทีม (แยกประเภท)
    Print("--- TRADING TEAMS ---");
    for(int i = 0; i < 5; i++) {
        string teamTask = GetTeamTask(strategyMissionTeam[i]);
        Print("👤 " + strategies[i].name + " → " + strategyMissionTeam[i] + ": " + teamTask);
    }
    
    Print("--- RECOVERY TEAMS ---");
    for(int i = 5; i < 11; i++) {
        string teamTask = GetTeamTask(strategyMissionTeam[i]);
        Print("🛡️ " + strategyMissionTeam[i] + ": " + teamTask);
    }
    
    // 🤝 กฎการทำงานร่วมกัน
    Print("📜 COOPERATION RULES: All teams work together for portfolio recovery");
}

// 🔥 เพิ่มฟังก์ชั่นใหม่สำหรับการทำงานในโหมด Recovery
void ExecuteEssentialSystemsInRecovery()
{

    Print("🔄 RECOVERY MODE: Running essential systems only");
    
    // 1. ระบบ Low Risk เท่านั้น
    ExecuteLowRiskOpportunities();
    
    // 2. กลยุทธ์ Reduced (เรียงตามความเสี่ยงจากน้อยไปมาก)
...
void ExecuteScalp2WithCustomLot(double customLot)
{
    ...
    // เปิดออร์เดอร์ (ตัวอย่างแบบ Buy)
    if(trade.Buy(customLot, _Symbol, slPrice, tpPrice, 0, "Scalp2-CustomLot")) {
        ulong ticket = trade.ResultOrder();
        Print("✅ SCALP2 CUSTOM LOT BUY | Lot: " + DoubleToString(customLot, 2) + 
              " | Ticket: #" + IntegerToString(ticket));
    } else {
        Print("❌ Scalp2 BUY Error: " + trade.ResultRetcodeDescription());
    }
}
void ExecuteScalp3WithReducedLot(double reducedLot)
{
    ...
    int fixedTPPoints = 20;  // ตั้งค่า fixed
    ...
    if(buyCondition) {
        ...
        if(trade.Buy(reducedLot, _Symbol, slPrice, tpPrice, 0, "Scalp3-Reduced")) {
            ulong ticket = trade.ResultOrder();
            Print("✅ SCALP3 REDUCED BUY | Lot: " + DoubleToString(reducedLot, 2) + 
        ...
        if(trade.Sell(reducedLot, _Symbol, slPrice, tpPrice, 0, "Scalp3-Reduced")) {
            ulong ticket = trade.ResultOrder();
            Print("✅ SCALP3 REDUCED SELL | Lot: " + DoubleToString(reducedLot, 2) + 
                  " | SL: " + IntegerToString(fixedSLPoints) + "pips" +
                  " | TP: " + IntegerToString(fixedTPPoints) + "pips");
        }
    }
...
void ExecuteSimpleTrendScalpWithCustomLot(double customLot)
{
    ...
    if(buyCondition) {
        ...
        if(trade.Buy(customLot, _Symbol, 0, 0, 0, "Trend-Reduced")) {
            ulong ticket = trade.ResultOrder();
            Print("✅ TREND REDUCED BUY | Lot: " + DoubleToString(customLot, 2) + 
                  " | Ticket: #" + IntegerToString(ticket));
        } else {
            Print("❌ Trend Reduced BUY Error: " + trade.ResultRetcodeDescription());
        }
    }
    else if(sellCondition) {
        // 🔥 ไม่ใช้ SL/TP Points - เปิดออร์เดอร์แบบไม่มี SL/TP
        if(trade.Sell(customLot, _Symbol, 0, 0, 0, "Trend-Reduced")) {
            ulong ticket = trade.ResultOrder();
            Print("✅ TREND REDUCED SELL | Lot: " + DoubleToString(customLot, 2) + 
                  " | Ticket: #" + IntegerToString(ticket));
        } else {
            Print("❌ Trend Reduced SELL Error: " + trade.ResultRetcodeDescription());
        }
    }
}
void ExecuteAdvancedBreakoutWithCustomLot(double customLot)
{
    ...
    // ใช้ logic จาก ExecuteAdvancedBreakout() แต่ใช้ custom lot
    MqlRates rates[];
    if(CopyRates(_Symbol, PERIOD_M1, 0, 2, rates) == 2) {
        ...
        if(bullishBreakout) {
            ...
            if(trade.Buy(customLot, _Symbol, slPrice, tpPrice, 0, "Breakout-Reduced")) {
                ulong ticket = trade.ResultOrder();
                Print("✅ BREAKOUT REDUCED BUY | Lot: " + DoubleToString(customLot, 2) + 
                      " | Ticket: #" + IntegerToString(ticket));
            }
        }
            ...
            if(trade.Sell(customLot, _Symbol, slPrice, tpPrice, 0, "Breakout-Reduced")) {
                ulong ticket = trade.ResultOrder();
                Print("✅ BREAKOUT REDUCED SELL | Lot: " + DoubleToString(customLot, 2) + 
                      " | Ticket: #" + IntegerToString(ticket));
            }
        }
    }
...
void ExecuteScalp1WithCustomLot(double customLot)
{
    ...
    // ใช้ logic จาก ExecuteScalp1() แต่ใช้ custom lot
    double ema9 = iMA(_Symbol, PERIOD_M1, 9, 0, MODE_EMA, PRICE_CLOSE);
    double ema21 = iMA(_Symbol, PERIOD_M1, 21, 0, MODE_EMA, PRICE_CLOSE);
    double rsi = iRSI(_Symbol, PERIOD_M1, 14, PRICE_CLOSE, 0);
    ...
    if(buyCondition) {
        ...
        if(trade.Buy(customLot, _Symbol, slPrice, tpPrice, 0, "Scalp1-CustomLot")) {
            ulong ticket = trade.ResultOrder();
            Print("✅ SCALP1 CUSTOM LOT BUY | Lot: " + DoubleToString(customLot, 2) + 
                  " | Ticket: #" + IntegerToString(ticket));
        }
    }
    else if(sellCondition) {
        ...
        if(trade.Sell(customLot, _Symbol, slPrice, tpPrice, 0, "Scalp1-CustomLot")) {
            ulong ticket = trade.ResultOrder();
            Print("✅ SCALP1 CUSTOM LOT SELL | Lot: " + DoubleToString(customLot, 2) + 
                  " | Ticket: #" + IntegerToString(ticket));
        }
    }
}
...
// 🔥 ฟังก์ชั่นสำหรับ Scalp1 ในโหมด Recovery
void ExecuteScalp1WithReducedRisk()
{
    Print("🔰 Scalp1 (Reduced Risk Mode)");
    
    // คำนวณลอตที่ลดลง
    double originalLot = CalculateSafeScalpLot();
    double reducedLot = originalLot * 0.3; // ลดเหลือ 30%
    
    // ใช้เงื่อนไขที่เข้มงวดขึ้น
    double ema9 = iMA(_Symbol, PERIOD_M1, 9, 0, MODE_EMA, PRICE_CLOSE);
    double ema21 = iMA(_Symbol, PERIOD_M1, 21, 0, MODE_EMA, PRICE_CLOSE);
    double rsi = iRSI(_Symbol, PERIOD_M1, 14, PRICE_CLOSE, 0);
    
    // 🔥 เงื่อนไขที่เข้มงวดกว่าเดิม
    bool buyCondition = (ema9 > ema21 && rsi > 40 && rsi < 75); // Range แคบลง
    bool sellCondition = (ema9 < ema21 && rsi < 60 && rsi > 25);
...
void ExecuteScalp1Reduced()
{
    ...
    if(buyCondition) {
        ...
        if(trade.Buy(reducedLot, _Symbol, sl, tp, 0, "Scalp1-Reduced")) {
            Print("✅ SCALP1 REDUCED BUY | Lot: " + DoubleToString(reducedLot, 2));
        }
    }
    else if(sellCondition) {
        ...
        if(trade.Sell(reducedLot, _Symbol, sl, tp, 0, "Scalp1-Reduced")) {
            Print("✅ SCALP1 REDUCED SELL | Lot: " + DoubleToString(reducedLot, 2));
        }
    }
}
void ExecuteScalp2Reduced()
{
    ...
    Print("🔰 Scalp2 (Reduced Risk Mode)");
    
    // ลดลอตเหลือ 40%
    double originalLot = CalculateSafeScalpLot();
    double reducedLot = originalLot * 0.4;
    
    // เงื่อนไขเข้มงวดขึ้น + Risk Pressure ต่ำ
    if(CalculateRecoveryRiskPressure() > 0.2) {
        DebugPrint(2, "Scalp2Reduced: Recovery risk too high, skipping");
        return;
    }
    
    // ใช้เงื่อนไขเดิมแต่เพิ่ม filter
    if(ShouldOpenPosition("Scalp2")) {
        // เรียกฟังก์ชั่นเดิมแต่ส่งพารามิเตอร์ reduced lot
        ExecuteScalp2WithCustomLot(reducedLot);
        Print("✅ SCALP2 REDUCED | Lot: " + DoubleToString(reducedLot, 2));
    }
}
void ExecuteScalp3Reduced()
{
    ...
    Print("🔰 Scalp3 (Reduced Risk Mode)");
    
    // ลดลอตเหลือ 35%
    double reducedLot = CalculateSafeScalpLot() * 0.35;
    
    // เงื่อนไข Risk Pressure ที่เข้มงวด
    if(CalculateRecoveryRiskPressure() > 0.25) {
        DebugPrint(2, "Scalp3Reduced: Recovery risk too high, skipping");
        return;
    }
    
    // เงื่อนไขเพิ่มเติมสำหรับ Reduced mode
    if(ShouldOpenPosition("Scalp3")) {
        // เรียกฟังก์ชั่นเดิมแต่ลดขนาด
        ExecuteScalp3WithReducedLot(reducedLot);
        Print("✅ SCALP3 REDUCED | Lot: " + DoubleToString(reducedLot, 2));
    }
}
...
{
    // ในโหมด Recovery ให้ทุกกลยุทธ์ทำงานร่วมกัน
    int bestPerformer = FindBestPerformingStrategy();
    
    if(bestPerformer != -1) {
        Print("🏆 RECOVERY COORDINATOR: " + strategies[bestPerformer].name + " leading recovery team");
        
        // 🔥 1. ผู้นำทำงานเต็มที่
        ExecuteRecoveryLeader(bestPerformer);
        
        // 🔥 2. ผู้สนับสนุนทำงานแต่ลดระดับ
        ExecuteRecoverySupporters(bestPerformer);
    }
    
    // เรียก Recovery Systems เพื่อช่วยแก้ไข
    ExecuteRecoverySystems();
}

// 🔥 ฟังก์ชั่นใหม่: ผู้นำทำงานเต็มที่
void ExecuteRecoveryLeader(int leaderIndex)
{
    Print("   🎯 LEADER: " + strategies[leaderIndex].name + " - Full power");
...
// 🔥 ฟังก์ชั่นใหม่: ผู้สนับสนุนทำงานลดระดับ
void ExecuteRecoverySupporters(int leaderIndex)
{
    for(int i = 0; i < 5; i++) {
        if(i != leaderIndex && strategies[i].isActive) {
            Print("   🤝 SUPPORTER: " + strategies[i].name + " - Reduced activity");
            
            // เรียกฟังก์ชั่น Reduced version ของแต่ละกลยุทธ์
            // (ต้องสร้างฟังก์ชั่นเหล่านี้เพิ่ม)
        }
    }
}
//+------------------------------------------------------------------+
//| Execute Recovery Systems (แยกจากกลยุทธ์เทรด)                  |
//+------------------------------------------------------------------+
void ExecuteRecoverySystems()
{
    // ✅ Individual Recovery System
    if(EnableIndividualRecovery) {
        ManageIndividualRecoverySets();
    }
    
    // ✅ Step-Up Recovery System  
    if(StepUpRecoveryEnabled) {
        ManageStepUpRecovery();
    }
    
    // ✅ Counter Position System
    if(EnableCounterPosition) {
    ...
    // ✅ Emergency Hedge System
    if(UseEmergencyHedge) {
        CheckEmergencyHedge();
    }
    
    // ✅ Ultimate Fighting Recovery System
    if(UltimateFightingMode && newBar) {
        ExecuteUltimateFightingRecovery(); // ✅ เรียกอันที่ถูกต้อง
    }
}
//+------------------------------------------------------------------+
//| Execute AI Systems                                             |
...
void ExecuteAISystems()
{
    if(AI_NewsTrading_Enabled && newBarAI) {
        ...
        if(ShouldOpenPosition("AI-News")) {
            // เรียกฟังก์ชั่น AI News Trading
        }
    }
    ...
    if(AI_TrendTrading_Enabled && newBarAI) {
        ...
        if(ShouldOpenPosition("AI-Trend")) {
            // เรียกฟังก์ชั่น AI Trend Trading
        }
    }
}
...
bool IsStrategyOverPerforming(int index)
{
    ...
    return (strategies[index].progress > 120); // ทำได้เกิน 120% ของ target
}
...
void LimitStrategyOperations(int index)
{
    ...
    Print("🛑 Limiting " + strategies[index].name + " operations (Over-performing: " + 
          DoubleToString(strategies[index].progress, 1) + "%)");
    
    // สามารถเพิ่ม logic การจำกัดการทำงานที่นี่
    // เช่น ลดจำนวนออเดอร์สูงสุด, เพิ่มระยะห่างระหว่างออเดอร์ ฯลฯ
}
bool CheckRecoveryScalp2Conditions(ENUM_MARKET_REGIME_DETAILED regime, string symbol = NULL)
{
...
bool CheckRecoveryScalp3Conditions(ENUM_MARKET_REGIME_DETAILED regime, string symbol = NULL)
{
    if(symbol == NULL) symbol = Symbol();
    ...
    // 2. Bollinger Bands squeeze (แก้ไขให้ถูกต้อง)
    double bbUpper = 0, bbMiddle = 0, bbLower = 0;
    
    // ดึงค่า Bollinger Bands
...
//+------------------------------------------------------------------+
bool ShouldOpenPosition(string strategyName, double value = 0, void* params = NULL, ENUM_MARKET_REGIME_DETAILED regime = MARKET_REGIME_NORMAL)
{
    // ตรวจสอบพื้นฐาน
    ...
    {
        return CheckScalp1Conditions(ENUM_MARKET_REGIME_DETAILED regime);
    }
...
// ฟังก์ชันตรวจสอบเงื่อนไขตาม Regime
bool CheckScalp1Conditions(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // ในตลาดผันผวน range-based
            return IsAtRangeBoundary();
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // ในเทรนด์แรง ต้องเป็น pullback entry
            return IsPullbackEntry();
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            // รอ breakout confirmation
...
{
    // ✅ หา pullback entry สำหรับ scalp
    ...
    // Pullback condition: ราคาอยู่ใกล้ EMA (0.1%-0.5%)
...
{
    // ✅ หา entry ใน range market
    
    // ดึงค่า Bollinger Bands แบบถูกต้อง
    int bbHandle = iBands(Symbol(), PERIOD_M5, 20, 0, 2.0, PRICE_CLOSE);
...
{
    // ✅ ตรวจสอบ momentum และ volume
    
    // 1. Volume ต้องสูงกว่า average (แก้ type conversion)
    ...
    if(currentVolume < avgVolume * 0.8) // ต่ำกว่า 80% ของ average
    {
        Print("Volume insufficient: ", currentVolume, " vs avg: ", avgVolume);
        return false;
    }
    
    // 2. MACD ต้องแสดง momentum (แก้ iMACD usage)
    ...
    // ตรวจสอบว่า MACD มี momentum หรือไม่
    double macdDiff = MathAbs(macdMain - macdSignal);
    
    if(macdDiff <= 0.0001) // MACD แทบไม่แตกต่าง (sideways)
    {
        Print("MACD shows no momentum. Diff: ", macdDiff);
        return false;
    }
    
    // 3. เพิ่มตรวจสอบ RSI เพื่อยืนยัน momentum
    double rsi = iRSI(Symbol(), PERIOD_M5, 14, PRICE_CLOSE, 0);
    
    // ใช้ CopyBuffer สำหรับ RSI
    ...
    if(rsiHandle != INVALID_HANDLE)
    {
        ...
        {
            rsi = rsiArr[0];
            // RSI ควรจะไม่อยู่ใน extreme zone (20-80 ดีที่สุด)
            if(rsi < 20 || rsi > 80)
            {
    ...
    Print("✅ Momentum/Volume check passed. MACD diff: ", macdDiff, ", RSI: ", rsi);
    return true;
}
...
{
    // Placeholder - ในความเป็นจริงต้องเชื่อมต่อกับ news feed
...
{
    // ✅ ตรวจสอบว่าไม่บล็อกการเทรด
    // 1. Spread ต้องต่ำ (เหมาะสำหรับ scalp)
    int spread = (int)SymbolInfoInteger(Symbol(), SYMBOL_SPREAD);
    if(spread > 5) // มากกว่า 5 pips ไม่เหมาะ
    {
        Print("Spread too high: ", spread);
        return false;
    }
    
    // 2. Market ต้องเปิดและมีสภาพคล่อง
    if(!IsMarketActive())
        return false;
    
    // 3. ไม่ใช่ช่วง news ที่สำคัญ
    if(IsHighImpactNews())
    {
...
{
    // 🔥 SMART SCALP 1.0 - เทรดฉลาด รู้เทรนด์ จับสวิงได้มากขึ้น
    
    // 1. ✅ ตรวจสอบสภาพตลาดพื้นฐาน (ไม่บล็อกการเทรด)
    if(!CheckMarketConditions()) 
    {
        Print("❌ Market conditions not suitable for smart scalp");
        return false;
    }
    
    // 2. ✅ รู้เทรนด์ใหญ่ (Higher Timeframe Analysis)
    ENUM_TREND_DIRECTION majorTrend = GetMajorTrend(PERIOD_H1);
    ENUM_TREND_DIRECTION minorTrend = GetMinorTrend(PERIOD_M5);
    
    // 3. ✅ หาโอกาส entry ที่ฉลาด (Smart Entry)
    bool smartEntryFound = FindSmartScalpEntry(majorTrend, minorTrend);
    
    if(!smartEntryFound)
    {
        Print("❌ No smart entry found");
        return false;
    }
    
    // 4. ✅ ตรวจสอบ risk/reward ที่เหมาะสม
    if(!CheckRiskRewardRatio(1.5)) // 1:1.5 minimum
    {
        Print("❌ Risk/Reward ratio not favorable");
        return false;
    }
    
    // 5. ✅ ตรวจสอบ momentum และ volume
    if(!CheckMomentumVolume())
    {
        Print("❌ Momentum or volume insufficient");
        return false;
    }
    
    Print("✅ SMART SCALP: Conditions met! Major Trend: ", majorTrend, 
...
bool IsGoodMarketForScalp(string symbol)
{
    ...
    // เทรดเฉพาะวันจันทร์-ศุกร์
    if(timeNow.day_of_week == 0 || timeNow.day_of_week == 6) // อาทิตย์หรือเสาร์
        return false;
    
    // เทรดเฉพาะช่วงเวลาที่มี liquidity สูง (ปรับตามตลาด)
    // Forex: 8:00-17:00 (เวลา server)
    if(timeNow.hour < 8 || timeNow.hour >= 17)
        return false;
    
    // ตรวจสอบว่า market เปิด
...
// 🔥 ตรวจสอบว่าเทรดกลยุทธ์ไหนในสภาพตลาดไหน
bool IsStrategySuitableForMarket(string strategyType, string marketCondition)
{
    ...
    {
        // Recovery ทำงานได้ในทุกสภาพ แต่ระวังตลาดผันผวน
        if(marketCondition == "HIGH_VOLATILITY") {
            Print("⚠️ " + strategyType + ": Recovery in high volatility market - be cautious");
        }
        return true;
    }
    else if(strategyType == "Hedge" || strategyType == "CounterPosition") 
    {
        // Hedge ทำงานได้ดีในตลาดผันผวน
        return (marketCondition == "HIGH_VOLATILITY" || marketCondition == "RANGING");
    }
    
    // Default - อนุญาตทั้งหมดถ้าไม่รู้จัก strategy
    return true;
}

// 🔥 **เพิ่มฟังก์ชันตรวจสอบเงื่อนไขเฉพาะกลยุทธ์**
bool CheckStrategySpecificConditions(string strategyName, string symbol)
{
    ...
    // ถ้าไม่มีกลยุทธ์ที่ระบุ หรือเป็นกลยุทธ์ทั่วไป
    return CheckBasicTradingConditions(symbol);
}

// 🔥 **ฟังก์ชันตรวจสอบเงื่อนไขพื้นฐานเพิ่มเติม**
bool CheckBasicTradingConditions(string symbol)
{
    ...
    // ตรวจสอบการอนุญาตการเทรด
    if(!TerminalInfoInteger(TERMINAL_TRADE_ALLOWED)) {
        Print("Trading not allowed");
        return false;
    }
    
    // 🔧 **เวอร์ชันแบบง่าย: ใช้ spread แบบ points โดยตรง**
    long spreadPoints = SymbolInfoInteger(symbol, SYMBOL_SPREAD);
    
    // กำหนดค่า spread สูงสุดที่ยอมได้ (ใน points)
    long maxAllowedSpreadPoints = 30; // 3 pips สำหรับ forex
...
// 🔥 เวอร์ชันเก่า (สำหรับ backward compatibility)
bool CheckTrendConditionsLegacy(bool strongTrend, string symbol)
{
    // Convert bool to regime
    ENUM_MARKET_REGIME_DETAILED regime = strongTrend ? MARKET_REGIME_TRENDING_STRONG_DETAILED : MARKET_REGIME_NORMAL;
    return CheckTrendConditions(regime, symbol);
}

// 🔥 **ฟังก์ชันตรวจสอบเงื่อนไขเทรนด์**
bool CheckTrendConditions(ENUM_MARKET_REGIME_DETAILED regime, string symbol)
{
    // 🔧 **ใช้ MQL5 syntax ที่ถูกต้องสำหรับ indicators**
    ...
    {
        emaFastPrev[0] = emaFast[0]; // ใช้ค่าเดียวกันถ้าไม่สามารถดึงค่าเก่าได้
    }
    
    // ตรวจสอบการเรียงตัวของ EMA
    ...
    // 🔥 ปรับเงื่อนไขตาม Market Regime
    switch(regime)
    {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // สำหรับเทรนด์แรง ตรวจสอบความชันเข้มงวด
        ...
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            // สำหรับเทรนด์อ่อน ยอมรับความชันน้อยกว่า
        ...
        case MARKET_REGIME_RANGING_CALM_DETAILED:
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // ใน sideways market ไม่ควรใช้ trend strategy
            IndicatorRelease(emaFastHandle);
            IndicatorRelease(emaMediumHandle);
            IndicatorRelease(emaSlowHandle);
            return false;
        ...
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            // สำหรับ breakout ต้องการการยืนยันเพิ่มเติม
...
bool CheckWeakTrendConditions(double emaFast, double emaFastPrev, string symbol)
{
    ...
    // สำหรับเทรนด์อ่อน ยอมรับความชันน้อยกว่า
    if(slope < 20) // threshold ต่ำกว่า
    {
        if(EnableDebugMode) Print("Slope too low even for weak trend: ", slope, " < 20");
        return false;
    }
    
    // ADX อาจต่ำกว่าได้สำหรับ weak trend
    int adxHandle = iADX(symbol, PERIOD_H1, 14);
    double adxArray[1];
    
    if(CopyBuffer(adxHandle, 0, 0, 1, adxArray) > 0)
    {
        if(adxArray[0] < 20) // threshold ต่ำกว่า
        {
...
bool CheckNormalTrendConditions(double emaFast, double emaFastPrev, string symbol)
{
    ...
    // สำหรับสภาพปกติ ใช้เงื่อนไขกลางๆ
    if(slope < 50)
    {
...
{
    // สำหรับ breakout ต้องการการบีบตัวของ EMA
    ...
    // ตรวจสอบว่า EMA กำลังบีบตัวหรือไม่ (สำหรับ breakout)
    ...
    // ตรวจสอบ volatility ลดลง
    // สร้าง atrHandle ที่นี่ (แก้จุด error)
    ...
    if(CopyBuffer(atrHandle, 0, 0, 1, atrArray) > 0)
    {
        // ใช้ GetATRAverage ถ้ามีอยู่แล้ว
        double atr50 = GetATRAverage(symbol, PERIOD_H1, 14, 50);
        
        if(atrArray[0] > atr50 * 0.8) // ATR ไม่ต่ำพอ
        {
            #ifdef ENABLE_DEBUG_MODE
...
// 🔥 **ฟังก์ชันตรวจสอบเงื่อนไข Scalp**
bool CheckScalpConditions(string symbol)
{
    // 🔧 **ใช้ MQL5 syntax ที่ถูกต้องสำหรับ iATR**
    ...
    // Scalp ต้องการ volatility ที่เหมาะสม
    if(atrPercent < 0.02 || atrPercent > 0.1) {
        return false;
    }
    
    // ตรวจสอบเวลา (scalp ได้ผลใน session หลัก)
    MqlDateTime timeNow;
    TimeCurrent(timeNow); // 🔧 **แก้ไข: ใช้ TimeCurrent() แบบ MQL5**
    int hour = timeNow.hour;
...
bool CheckRangeConditions(string symbol)
{
    ...
    // เปิดออร์เดอร์เฉพาะเมื่อราคาใกล้ขอบ range
    double rangeMid = (upperBand + lowerBand) / 2;
    double bandWidth = upperBand - lowerBand;
    
    // ตรวจสอบว่า bandWidth ไม่เป็นศูนย์
    if(bandWidth == 0) {
        IndicatorRelease(bbHandle);
        return false;
    }
...
// 🔥 **ฟังก์ชันตรวจสอบเงื่อนไข Breakout**
// เวอร์ชันใหม่ที่รับ regime ด้วย
bool CheckBreakoutConditions(string symbol, ENUM_MARKET_REGIME_DETAILED regime = MARKET_REGIME_NORMAL)
{
    // ตรวจสอบ volume สำหรับ breakout
    ...
    {
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            volumeMultiplier = 1.5; // ต้องการ volume สูงกว่า
            break;
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            volumeMultiplier = 1.3; // volume สูงปานกลาง
            break;
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            volumeMultiplier = 1.8; // ต้องการ volume สูงมากใน calm range
            break;
        default:
...
bool IsBadTradingTime()
{
    ...
    if(hour < 2 || hour > 22) return true; // กลางคืน
...
void AdjustStrategyAggression(double progress)
{
    ...
    for(int i = 0; i < 5; i++) {
        ...
        if(targetProgress < 50) {
            ...
            Print("🚀 BOOSTING " + strategies[i].name + " (Under-performing: " + DoubleToString(targetProgress, 1) + "%)");
        }
        else if(targetProgress > 120) {
            // Over-performing: reduce aggression  
            aggressionMultiplier = 0.5;
            Print("🛑 SLOWING " + strategies[i].name + " (Over-performing: " + DoubleToString(targetProgress, 1) + "%)");
        }
...
void ResetCollaborativeSystem()
{
    ...
    // ✅ Reset strategy parameters to original
    Current_Scalp1_Lot_Multiplier = Original_Scalp1_Multiplier;
    Current_Scalp2_LotMultiplier = Original_Scalp2_Multiplier;
    Current_TrendScalp_TP_Points = Original_Trend_TP;
    
    Print("🔄 Cooperative system reset - ready for next session");
}
//+------------------------------------------------------------------+
...
{
    string dashboard = "";
    dashboard += "=== 🤝 COOPERATIVE TRADING SYSTEM ===\n";
    ...
    for(int i = 0; i < 5; i++) {
        string emoji = "🟢"; // Green - on target
        double progress = (strategies[i].currentProfit / strategies[i].targetProfit) * 100;
        
        if(progress < 50) emoji = "🔴"; // Red - behind
        else if(progress < 80) emoji = "🟡"; // Yellow - catching up
        else if(progress >= 100) emoji = "✅"; // Check - target achieved
...
// 📍 เพิ่มฟังก์ชันตรวจสอบตลาดแบบเรียลไทม์
void CheckMarketConditionsRealTime()
{
   if(TimeCurrent() - lastMarketConditionCheck < 300) return; // ตรวจสอบทุก 5 นาที
   
   ENUM_MARKET_REGIME newRegime = DetectMarketRegime();
   int newScore = CalculateMarketConditionScore();
   
   // ถ้าสภาพตลาดเปลี่ยนไป
   if(newRegime != currentMarketRegime || MathAbs(newScore - marketConditionScore) > 20) {
      Print("🔄 MARKET REGIME CHANGED: " + GetRegimeName(currentMarketRegime) + 
            " → " + GetRegimeName(newRegime) + " | Score: " + IntegerToString(marketConditionScore) +
            " → " + IntegerToString(newScore));
      
      currentMarketRegime = newRegime;
      marketConditionScore = newScore;
      
      // ปรับแผนการเทรดตามสภาพตลาดใหม่
...
//+------------------------------------------------------------------+
void AdjustTradingPlanForNewRegime(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    Print("🔄 ADJUSTING TRADING PLAN FOR: " + GetRegimeName(newRegime));
    
    switch(newRegime) {
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // เทรนด์แรง → เพิ่มขนาดเทรดในกลยุทธ์เทรนด์
            Current_TrendScalp_TP_Points = (int)(Original_Trend_TP * 1.3);
            Print("   📈 Trend TP increased to: " + IntegerToString(Current_TrendScalp_TP_Points));
            break;
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            // เทรนด์อ่อน → ลดขนาดเทรด
            Current_Scalp1_Lot_Multiplier = Original_Scalp1_Multiplier * 0.7;
            Print("   📉 Scalp1 lot reduced to: " + DoubleToString(Current_Scalp1_Lot_Multiplier, 2));
            break;
            
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // ไซด์เวย์ผันผวน → เน้น Scalp
            Current_Scalp1_Lot_Multiplier = Original_Scalp1_Multiplier * 1.2;
            Print("   🔄 Scalp1 lot increased for volatile range: " + DoubleToString(Current_Scalp1_Lot_Multiplier, 2));
            break;
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            // ไซด์เวย์เงียบ → ลดกิจกรรม
            Current_Scalp1_Lot_Multiplier = Original_Scalp1_Multiplier * 0.5;
            Print("   😴 Reduced activity for calm range");
            break;
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            // เตรียมเบรกเอ้าท์ → เตรียมกลยุทธ์ Breakout
            Print("   🚀 Preparing for potential breakout");
            break;
            
        case MARKET_REGIME_NORMAL:
            // โหมดปกติ → ใช้การตั้งค่าปกติ
            Current_TrendScalp_TP_Points = Original_Trend_TP;
            Current_Scalp1_Lot_Multiplier = Original_Scalp1_Multiplier;
            Print("   📊 Normal market - using standard settings");
            break;
    }
...
//+------------------------------------------------------------------+/
//| Enable Aggressive Recovery - เปิดใช้งาน Recovery แบบ Aggressive|
//+------------------------------------------------------------------+
void EnableAggressiveRecovery(bool enable)
{
    if(enable) {
        currentRecoveryMultiplier = 1.8;
        currentMaxRecovery = 6;
        Print("   💪 AGGRESSIVE RECOVERY ENABLED");
    } else {
        currentRecoveryMultiplier = 1.5;
        currentMaxRecovery = 4;
        Print("   💪 AGGRESSIVE RECOVERY DISABLED");
    }
}

//+------------------------------------------------------------------+
//| Enable Conservative Recovery - เปิดใช้งาน Recovery แบบระมัดระวัง|
//+------------------------------------------------------------------+
void EnableConservativeRecovery(bool enable)
{
   ...
   //int currentMaxRecovery = localMaxRecovery;
    if(enable) {
        currentRecoveryMultiplier = 1.2;
        currentMaxRecovery = 3;
        Print("   🛡️ CONSERVATIVE RECOVERY: Multiplier=" + DoubleToString(currentRecoveryMultiplier, 1) + 
...
double CalculateTemporaryTP(ulong ticket)
{
    if(PositionSelectByTicket(ticket)) {
        ...
        double currentPrice = PositionGetDouble(POSITION_PRICE_CURRENT);
        long positionType = PositionGetInteger(POSITION_TYPE);  // เปลี่ยนจาก int เป็น long
        
        if(positionType == POSITION_TYPE_BUY) {
            // สำหรับ Long: TP = Entry + เล็กน้อย
            return entryPrice + 0.0005;  // ปรับตามสกุลเงิน
        } else if(positionType == POSITION_TYPE_SELL) {  // เพิ่มตรวจสอบ SELL
            // สำหรับ Short: TP = Entry - เล็กน้อย  
            return entryPrice - 0.0005;  // ปรับตามสกุลเงิน
        }
    }
    return 0;
...
//+------------------------------------------------------------------+
//| Set Temporary Breakeven TPs - ตั้งค่า TP ชั่วคราว            |
//+------------------------------------------------------------------+
void SetTemporaryBreakevenTPs()
{
    ...
    for(int i = 0; i < PositionsTotal(); i++) {
        g_currentTicket = PositionGetTicket(i);  // เปลี่ยน ticket เป็น g_currentTicket
        if(PositionSelectByTicket(g_currentTicket)) {  // เปลี่ยน ticket เป็น g_currentTicket
            double profit = PositionGetDouble(POSITION_PROFIT);
            double currentTP = PositionGetDouble(POSITION_TP);
            
            // ถ้ายังไม่มี TP และกำลังขาดทุนน้อย
            if(currentTP == 0 && profit > -5 && profit < 2) {
                // ตั้ง TP ชั่วคราวที่ Breakeven + เล็กน้อย
                double newTP = CalculateTemporaryTP(g_currentTicket);  // เปลี่ยน ticket เป็น g_currentTicket
                if(trade.PositionModify(g_currentTicket, 0, newTP)) {  // เปลี่ยน ticket เป็น g_currentTicket
                    modifiedCount++;
                }
            }
        }
    ...
    if(modifiedCount > 0) {
        Print("   ✅ Set temporary TP for " + IntegerToString(modifiedCount) + " positions");
    }
}
//+------------------------------------------------------------------+
//| Update Team Missions - อัพเดทภารกิจทีม                       |
//+------------------------------------------------------------------+
void UpdateTradingTeamMissions()
{
    // ใส่ logic การอัพเดท Trading Teams ที่นี่
    if(EnableDebugMode) Print("   📊 Updating Trading Team Missions");
}

void UpdateRecoveryTeamMissions()
{
    // ใส่ logic การอัพเดท Recovery Teams ที่นี่
    if(EnableDebugMode) Print("   🔄 Updating Recovery Team Missions");
}

void UpdateSpecialTeamMissions() 
{
    // ใส่ logic การอัพเดท Special Teams ที่นี่
    if(EnableDebugMode) Print("   ⭐ Updating Special Team Missions");
}
void UpdateTeamMissions()
{
    Print("👨‍🚀 UPDATING TEAM MISSIONS:");
    
    // 1. อัพเดท Trading Teams
    UpdateTradingTeamMissions();
    
    // 2. อัพเดท Recovery Teams  
    UpdateRecoveryTeamMissions();
    
    // 3. อัพเดท Special Teams
    UpdateSpecialTeamMissions();
}
//+------------------------------------------------------------------+
//| Analyze Current Momentum - วิเคราะห์โมเมนตัมทั้งสองฝั่ง       |
//+------------------------------------------------------------------+
// เพิ่มฟังก์ชันเหล่านี้ถ้ายังไม่มี
void UpdateMomentumHistory(double momentum)
{
    if(EnableDebugMode) 
        Print("   📈 Momentum history updated: " + DoubleToString(momentum, 2));
}
...
{
    // ตรวจสอบ Volume ปัจจุบัน > Volume ก่อนหน้า
...
void AnalyzeCurrentMomentum()
{
    ...
    // บันทึกค่าโมเมนตัมสำหรับใช้ใน Recovery
...
//+------------------------------------------------------------------+
//| Plan Breakeven Strategy - วางแผนจุด Breakeven                  |
//+------------------------------------------------------------------+
void PlanBreakevenStrategy()
{
    Print("🎯 BREAKEVEN STRATEGY PLANNING:");
    
    // 1. คำนวณจุด Breakeven สำหรับแต่ละฝั่ง
...
double CalculateBreakevenForSide(ENUM_POSITION_TYPE side)
{
    ...
    // ปรับตาม Spread และ Commission
    double spread = SymbolInfoInteger(_Symbol, SYMBOL_SPREAD) * _Point;
    if(side == POSITION_TYPE_BUY) {
        breakevenPrice += spread * 1.5; // เพิ่ม buffer
    } else {
...
{
    return g_netMomentum; // ค่าจาก AnalyzeCurrentMomentum()
}
//+------------------------------------------------------------------+
//| Execute Trending Strong Plan - แผนเทรนด์แรง                   |
//+------------------------------------------------------------------+
void ExecuteTrendingStrongPlan()
{
    Print("📈 EXECUTING TRENDING STRONG PLAN");
    
    // 1. เพิ่มน้ำหนักกลยุทธ์เทรนด์
    Current_TrendScalp_TP_Points = (int)(Original_Trend_TP * 1.3);
    
    // 2. กระจายทีมตามโมเมนตัม
    double netMomentum = GetNetMomentum();
    if(netMomentum > 10) {
        // โมเมนตัมบวก → เน้นทีม Buy
        AssignTeamWeights(60, 20, 10, 5, 5); // TREND:60%, BREAKOUT:20%, etc.
        Print("   🟢 Momentum Positive - Focusing on BUY teams");
    } else if(netMomentum < -10) {
        // โมเมนตัมลบ → เน้นทีม Sell  
    ...
    // 3. เปิดรับความเสี่ยงมากขึ้นแต่มี Recovery Plan
    EnableAggressiveRecovery(true);
}
//+------------------------------------------------------------------+
//| Execute Trending Weak Plan - แผนเทรนด์อ่อน                    |
//+------------------------------------------------------------------+
void ExecuteTrendingWeakPlan()
{
    Print("📉 EXECUTING TRENDING WEAK PLAN");
    
    // ใช้เวอร์ชั่นที่สองเพราะสมบูรณ์กว่า
    Current_Scalp1_Lot_Multiplier = Original_Scalp1_Multiplier * 0.7;
    ...
    Print("   🛡️ Conservative approach with balanced teams");
    Print("   💰 Reduced lot sizes for risk management");
}

//+------------------------------------------------------------------+
//| Execute Ranging Volatile Plan - แผนไซด์เวย์ผันผวน            |
//+------------------------------------------------------------------+
void EnableQuickRecovery(bool enable)
{
    // ลบบรรทัดเหล่านี้เพราะมี global variables อยู่แล้ว:
    // double currentRecoveryMultiplier = localRecoveryMultiplier;
    // int currentMaxRecovery = localMaxRecovery;
    ...
    if(enable) {
        ...
        currentRecoveryMultiplier = 1.8;  // ใช้ global variable
        currentMaxRecovery = 6;           // ใช้ global variable
    } else {
        Print("   ⚡ QUICK RECOVERY DISABLED");
        currentRecoveryMultiplier = localRecoveryMultiplier;  // ใช้ global variable
        currentMaxRecovery = localMaxRecovery;                // ใช้ global variable
    }
}
...
void EnableCounterPositionMode(bool enable)
{
    ...
    if(enable) {
        Print("   🔄 COUNTER POSITION: ENABLED");
        // สามารถเพิ่ม logic อื่นๆ เมื่อเปิดโหมดได้ที่นี่
    } else {
        Print("   🔄 COUNTER POSITION: DISABLED");
        // สามารถเพิ่ม logic อื่นๆ เมื่อปิดโหมดได้ที่นี่
    }
}
...
void SetCounterPosition(bool enable)
{
    ...
    Print("   🔄 COUNTER POSITION: " + string(enable ? "ENABLED" : "DISABLED"));
}


//+------------------------------------------------------------------+
//| Execute Ranging Calm Plan - แผนไซด์เวย์เงียบ                 |
//+------------------------------------------------------------------+
void EnablePassiveRecovery(bool enable)
{
    if(enable) {
        currentRecoveryMultiplier = 1.1;  // ค่า recovery ต่ำ
        currentMaxRecovery = 2;           // จำนวนขั้นต่ำ
        Print("   😴 PASSIVE RECOVERY: Multiplier=" + DoubleToString(currentRecoveryMultiplier, 1) + 
              ", Max Steps=" + IntegerToString(currentMaxRecovery));
    } else {
        currentRecoveryMultiplier = localRecoveryMultiplier;
        currentMaxRecovery = localMaxRecovery;
        Print("   😴 PASSIVE RECOVERY DISABLED");
    }
}
void ExecuteRangingCalmPlan()
{
    Print("😴 EXECUTING RANGING CALM PLAN");
    
    // 1. ลดกิจกรรมทั้งหมด
    Current_Scalp1_Lot_Multiplier = Original_Scalp1_Multiplier * 0.5;
    
    // 2. เน้นทีม Breakout และ Trend (รอสัญญาณ)
    AssignTeamWeights(30, 30, 10, 20, 10);
    
    // 3. Recovery แบบ passive
    EnablePassiveRecovery(true);
    
    Print("   💤 Reduced activity - waiting for signals");
}
//+------------------------------------------------------------------+
//| Execute Breakout Potential Plan - แผนเตรียมเบรกเอ้าท์         |
//+------------------------------------------------------------------+
void ExecuteBreakoutPotentialPlan()
{
    Print("🚀 EXECUTING BREAKOUT POTENTIAL PLAN");
    
    // 1. เตรียมทีม Breakout
    AssignTeamWeights(20, 20, 15, 15, 30); // BREAKOUT:30%
    
    // 2. ตั้งค่า TP ที่ aggressive
    Current_TrendScalp_TP_Points = (int)(Original_Trend_TP * 1.5);
    
    // 3. Recovery แบบรอโอกาส
    EnableOpportunisticRecovery(true);
    
    Print("   🎯 Breakout teams ready - aggressive TP settings");
}
void EnableOpportunisticRecovery(bool enable)
{
    if(enable) {
        Print("   🎯 OPPORTUNISTIC RECOVERY ENABLED");
        // ตั้งค่าสำหรับ Opportunistic Recovery
    } else {
        Print("   🎯 OPPORTUNISTIC RECOVERY DISABLED");
    }
}
//+------------------------------------------------------------------+
//| Assign Team Weights - กระจายน้ำหนักทีม                        |
//+------------------------------------------------------------------+
void AssignTeamWeights(int trendWeight, int breakoutWeight, int scalp1Weight, int scalp2Weight, int scalp3Weight)
{
    Print("   👥 ASSIGNING TEAM WEIGHTS:");
    ...
    // TODO: สามารถบันทึกค่าเหล่านี้ไปใช้จริงได้
}
void AdjustStrategyWeights(double scalp1, double scalp2, double scalp3, double trend, double breakout)
{
    ...
    Print("   ⚙️ STRATEGY WEIGHTS UPDATED:");
...
void EnableCooperativeMode(bool enable)
{
    ...
    Print("   🤝 COOPERATIVE SYSTEM: " + string(enable ? "ENABLED" : "DISABLED"));
}
//+------------------------------------------------------------------+
//| Plan Recovery For Buy Side - วางแผน Recovery ฝั่ง Buy         |
//+------------------------------------------------------------------+
void PlanRecoveryForBuySide(double breakevenPrice)
{
    ...
    if(currentPrice < breakevenPrice) {
        Print("   🆘 BUY SIDE RECOVERY NEEDED:");
...
void PlanRecoveryForSellSide(double breakevenPrice)
{
    ...
    if(currentPrice > breakevenPrice) {
        Print("   🆘 SELL SIDE RECOVERY NEEDED:");
        Print("      Current: " + DoubleToString(currentPrice, 5) + 
              " | Target: " + DoubleToString(breakevenPrice, 5) +
...
//+------------------------------------------------------------------+
//| Assign Recovery Mission - มอบหมายภารกิจให้ทีม Recovery      |
//+------------------------------------------------------------------+
void AssignRecoveryMission(string side, double targetPrice)
{
    string localMissionId = side + "_" + IntegerToString((int)TimeCurrent());  // เปลี่ยนชื่อ local
    
    Print("   🎯 RECOVERY MISSION: " + localMissionId);
    Print("      Target Price: " + DoubleToString(targetPrice, 5));
    Print("      Assigned Teams: INDIVIDUAL_RECOVERY_TEAM, STEPUP_RECOVERY_TEAM");
    
    // บันทึกภารกิจ Recovery
    SaveRecoveryMission(localMissionId, side, targetPrice);  // ใช้ชื่อใหม่
}
void SaveRecoveryMission(string missionIdParam, string side, double targetPrice)  // เปลี่ยนชื่อ parameter
{
    if(EnableDebugMode) {
        Print("   💾 SAVING RECOVERY MISSION:");
        Print("      ID: " + missionIdParam);  // ใช้ชื่อ parameter ใหม่
        Print("      Side: " + side);
        Print("      Target Price: " + DoubleToString(targetPrice, 5));
    }
    
    // สามารถเพิ่ม logic การบันทึกได้ที่นี่
}
//-----//
//+------------------------------------------------------------------+
...
{
    // ตรวจสอบหลาย timeframe เพื่อยืนยันสัญญาณเทรนด์ที่ชัดเจน
    ...
    // ต้องมีเทรนด์ที่สอดคล้องกันในอย่างน้อย 2 timeframe
    ...
    // สำหรับ uptrend: RSI ควรไม่ overbought, ADX ควรแสดงเทรนด์แรง
    if(trendH1 == TREND_UP) {
        if(rsiH1 > 75) {
    ...
    // สำหรับ downtrend: RSI ควรไม่ oversold, ADX ควรแสดงเทรนด์แรง
    ...
    Print("✅ Clear trend signal detected: " + GetTrendDirectionName(trendH1) + 
...
bool HasClearTradingSignal()
{
    ...
    // ต้องมีอย่างน้อย 2 สัญญาณที่ชัดเจนจาก 3 ชนิด
...
bool HasClearScalpSignal()
{
    ...
    // เงื่อนไขที่เข้มงวดกว่าเดิมสำหรับ scalp
    bool clearBuySignal = (
    ...
    if(clearBuySignal || clearSellSignal) {
        ...
        Print("✅ Clear scalp signal: " + direction + 
...
bool HasClearBreakoutSignal()
{
    ...
    // หาค่า High/Low สำหรับ breakout detection
    ...
    if(clearBreakoutUp || clearBreakoutDown) {
        ...
        Print("✅ Clear breakout signal: " + direction + 
...
bool IsPriceAtExtreme()
{
    ...
    // ถ้าราคาอยู่ใกล้ขอบ Band เกินไป (น้อยกว่า 10 pips)
...
int CountAllOrders()
{
    ...
    // สำหรับ MQL5 (ใช้ PositionsTotal)
    #ifdef __MQL5__
    total = PositionsTotal();
    
    // สำหรับ MQL4 (ใช้ OrdersTotal)
    #else 
    total = OrdersTotal();
    #endif
...
bool ShouldTradeInVolatileMarket(ENUM_MARKET_REGIME_DETAILED marketRegime = MARKET_REGIME_UNKNOWN_DETAILED)
{
    // ตรวจสอบเฉพาะเมื่อเป็น volatile market
    if(marketRegime == MARKET_REGIME_RANGING_VOLATILE_DETAILED) {
        Print("⚡ VOLATILE MARKET DETECTED");
        
        // 1. ตรวจสอบ volatility
        if(currentVolatility >= 150) {
            Print("   ❌ Volatility too high: ", currentVolatility);
            return false;
        }
        
        // 2. ตรวจสอบจำนวนออเดอร์
        ...
        if(profit < -100) {
            Print("   ❌ Loss too high: ", profit);
            return false;
        }
        
        // 4. ตรวจสอบระยะห่างออเดอร์ (ถ้าต้องการ)
        if(buyOrders > 0 && sellOrders > 0) {
            double avgBuy = GetAverageBuyPrice();
            double avgSell = GetAverageSellPrice();
            if(avgBuy > 0 && avgSell > 0) {
                double distance = MathAbs(avgBuy - avgSell) / _Point;
                if(distance < 50) {
                    Print("   ❌ Order distance too small: ", distance);
                    return false;
                }
            }
        }
        
        Print("   ✅ Conditions met for volatile market");
        return true;
    }
    
    return false;
}
double GetAverageBuyPrice(int specificMagic = -1)
{
    double totalLots = 0;
...
double GetAverageSellPrice(int specificMagic = -1)
{
    double totalLots = 0;
    double weightedPrice = 0;
    int sellCount = 0;
...
// 🔥 ฟังก์ชันตรวจสอบระยะห่างของออเดอร์
bool CheckOrderDistance()
{
    double minDistancePoints = 50; // จุดต่ำสุดระหว่างออเดอร์
    ...
    if(buyOrders > 0 && sellOrders > 0) {
        ...
        if(avgBuyPrice == 0 || avgSellPrice == 0) {
            Print("⚠️ Cannot calculate average prices");
            return true;
        }
        ...
        if(distance < minDistancePoints) {
            Print("⚠️ Order distance too small: ", distance, " points");
            Print("   Buy Avg: ", avgBuyPrice, " | Sell Avg: ", avgSellPrice);
            return false;
        }
        
        Print("✅ Order distance OK: ", distance, " points");
        Print("   Buy Avg: ", avgBuyPrice, " | Sell Avg: ", avgSellPrice);
    } else {
        Print("✅ No hedging positions to check distance");
    }
...
bool IsSafeTradingTime()
{
    ...
    // หลีกเลี่ยงเวลา news หรือช่วงเวลาเสี่ยง
    if((currentHour >= 0 && currentHour <= 1) ||    // กรุงเทพฯ เช้ามืด
       (currentHour >= 12 && currentHour <= 14) ||   // ช่วงพักเที่ยง
       (currentHour >= 21 && currentHour <= 23)) {   // กรุงเทพฯ ดึก
        Print("⚠️ Unsafe trading hour: ", currentHour);
        return false;
    }
    
    return true;
}
// 🔥 ฟังก์ชันตรวจสอบ risk management
bool CheckRiskManagement(double volatility = 0)
{
    ...
    if(drawdownPercent > 5) { // จำกัด drawdown ที่ 5%
        Print("⚠️ Drawdown too high: ", drawdownPercent, "%");
        return false;
    }
    
    double marginLevel = AccountInfoDouble(ACCOUNT_MARGIN_LEVEL);
    if(marginLevel > 0 && marginLevel < 200) { // Margin level ต่ำกว่า 200%
        Print("⚠️ Margin level too low: ", marginLevel, "%");
        return false;
    }
    
    // 🔥 ปรับ risk ตาม volatility
    if(volatility > 100) {
        // หาก volatility สูง ให้ใช้ risk ที่ต่ำลง
        double maxRiskPercent = 1.0; // 1% สำหรับ high volatility
        double currentRisk = CalculateCurrentRiskPercent();
        
        if(currentRisk > maxRiskPercent) {
            Print("⚠️ Risk too high for volatile market: ", currentRisk, "% > ", maxRiskPercent, "%");
            return false;
        }
    }
...
double CalculateCurrentRiskPercent()
{
    ...
    for(int i = 0; i < PositionsTotal(); i++) {
        if(PositionGetSymbol(i) == _Symbol) {
            ...
            // ตรวจสอบว่า magic อยู่ในรายการของคุณหรือไม่
            bool magicFound = false;
            for(int m = 0; m < ArraySize(magicList); m++) {
                if(positionMagic == (ulong)magicList[m]) {
                    magicFound = true;
                    break;
                }
            }
    ...
    // สูตรคำนวณ risk
    double contractSize = 100000; // Standard lot size for forex
    double riskPercent = (totalLots * contractSize) / accountBalance * 100;
    
    Print("Risk Calculation: Lots=", totalLots, " | Balance=", accountBalance, 
          " | Risk=", riskPercent, "%");
...
{
    double highPrice = iHigh(_Symbol, _Period, iHighest(_Symbol, _Period, MODE_HIGH, period, 0));
    double lowPrice = iLow(_Symbol, _Period, iLowest(_Symbol, _Period, MODE_LOW, period, 0));
    
    if(lowPrice == 0) return 0;
    
    double volatility = ((highPrice - lowPrice) / lowPrice) * 100;
    return NormalizeDouble(volatility, 2);
}

// 🔥 ฟังก์ชัน AnalyzeMarketCondition ที่ถูกต้อง
ENUM_MARKET_REGIME_DETAILED AnalyzeMarketCondition()
{
    double trendStrength = GetTrendStrengthNormalized();
    double volatility = CalculateVolatility();
    double priceChange = CalculatePriceChange();
    
    // ตรรกะการวิเคราะห์ตลาด
    if(volatility > 200) {
        return MARKET_REGIME_HIGH_VOLATILITY_DETAILED;
    }
    else if(IsNewsEvent()) {  // ต้องมีฟังก์ชันนี้
        return MARKET_REGIME_NEWS_EVENT_DETAILED;
    }
    else if(trendStrength > 0.7) {
        return MARKET_REGIME_TRENDING_STRONG_DETAILED;
    }
    else if(trendStrength > 0.3) {
        return MARKET_REGIME_TRENDING_WEAK_DETAILED;
    }
    else if(volatility < 80 && IsRangingMarket()) {
        return MARKET_REGIME_RANGING_CALM_DETAILED;
    }
    else if(volatility > 80 && volatility <= 150) {
        return MARKET_REGIME_RANGING_VOLATILE_DETAILED;
    }
    else if(IsBreakoutPotential()) {
        return MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED;
    }
...
{
    // ใช้ Bollinger Bands หรือ ADX ตรวจสอบ ranging market
    ...
    // ถ้า bandwidth แคบและราคาอยู่กลาง bands
    return (bandWidth < currentPrice * 0.01);
}
bool IsBreakoutPotential()
{
    // ตรวจสอบว่าราคาอยู่ใกล้ระดับ Support/Resistance หรือไม่
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    double atrValue = GetATRValue();
    
    // ค้นหา High/Low ล่าสุด
    double recentHigh = iHigh(_Symbol, _Period, iHighest(_Symbol, _Period, MODE_HIGH, 20, 0));
    double recentLow = iLow(_Symbol, _Period, iLowest(_Symbol, _Period, MODE_LOW, 20, 0));
    
    double distanceToHigh = MathAbs(recentHigh - currentPrice);
    double distanceToLow = MathAbs(currentPrice - recentLow);
    
    // ถ้าราคาอยู่ใกล้ระดับสูงสุดหรือต่ำสุด
    return (distanceToHigh < atrValue || distanceToLow < atrValue);
}
bool IsTrendingMarket()
{
    ...
    // ใช้ iMA ให้ถูกต้อง
    int maFastHandle = iMA(_Symbol, _Period, 10, 0, MODE_SMA, PRICE_CLOSE);
    int maSlowHandle = iMA(_Symbol, _Period, 50, 0, MODE_SMA, PRICE_CLOSE);
    
    if(maFastHandle == INVALID_HANDLE || maSlowHandle == INVALID_HANDLE) 
        return false;
    
    CopyBuffer(maFastHandle, 0, 0, 1, maFastArray);
    CopyBuffer(maSlowHandle, 0, 0, 1, maSlowArray);
    
    double maFast = maFastArray[0];
    double maSlow = maSlowArray[0];
    
    // ตรวจสอบความต่างของ MA
    double maDiff = MathAbs(maFast - maSlow);
    double priceDiff = MathAbs(SymbolInfoDouble(_Symbol, SYMBOL_BID) - maSlow);
    double atrValue = GetATRValue();  // ต้องมีฟังก์ชันนี้
    
    return (maDiff > atrValue * 0.5);
}
// 🔥 ฟังก์ชัน CalculatePriceChange
double CalculatePriceChange(int period = 20)
{
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    
    // ใช้ iClose ต้องมี handle ก่อน
    MqlRates rates[];
    ArraySetAsSeries(rates, true);
    ...
    double pastPrice = rates[period].close;
    
    if(pastPrice == 0) return 0;
    
    double changePercent = ((currentPrice - pastPrice) / pastPrice) * 100;
    
    return NormalizeDouble(changePercent, 2);
}



// 🔥 ตัวอย่างการใช้งานในโค้ดของคุณ:
void CheckMarketAndTrade()
{
    // วิเคราะห์สภาพตลาด
    ENUM_MARKET_REGIME marketRegime = AnalyzeMarketCondition();
    
    if(marketRegime == MARKET_REGIME_RANGING_VOLATILE_DETAILED) {
        Print("🌊 DETECTED: VOLATILE RANGING MARKET");
        
        double volatility = CalculateVolatility();
        
        // 🔥 เรียกใช้ฟังก์ชันที่แก้ไขแล้ว
        if(volatility < 150 && ShouldTradeInVolatileMarket(MARKET_REGIME_RANGING_VOLATILE_DETAILED)) {
            Print("✅ Trading in volatile market allowed");
            ExecuteVolatileMarketStrategy();
        } else {
            Print("⛔ Trading in volatile market NOT allowed");
            Print("   Volatility: ", volatility, " | ShouldTrade: ", 
                  ShouldTradeInVolatileMarket(MARKET_REGIME_RANGING_VOLATILE_DETAILED));
        }
    }
    else if(marketRegime == MARKET_REGIME_RANGING_CALM_DETAILED) {
        Print("🌊 DETECTED: CALM RANGING MARKET");
        UseRangeStrategy(MARKET_REGIME_RANGING_CALM_DETAILED);
    }
    else if(marketRegime == MARKET_REGIME_TRENDING) {
        Print("📈 DETECTED: TRENDING MARKET");
        UseTrendStrategy();
    }
    else if(marketRegime == MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED) {
        Print("⚡ DETECTED: BREAKOUT POTENTIAL");
        if(IsBreakoutConfirmed(MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED)) {
            UseBreakoutStrategy();
        }
    }
    else if(marketRegime == MARKET_REGIME_HIGH_VOLATILITY_DETAILED) {
        Print("⚠️ DETECTED: HIGH VOLATILITY - PAUSING TRADES");
    }
    else {
        Print("❓ UNKNOWN MARKET REGIME: ", marketRegime);
    }
}
void ExecuteVolatileMarketStrategy()
{
    Print("Executing volatile market strategy...");
    
    // ตรรกะการเทรดในตลาด volatile
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    int atrHandle = iATR(_Symbol, _Period, 14);
    
    // ตัวอย่าง: เทรดแบบ mean reversion ในตลาด volatile
    if(IsAtSupport()) {
        OpenBuyOrderWithRiskManagement();
    }
    else if(IsAtResistance()) {
        OpenSellOrderWithRiskManagement();
    }
}
void CheckForTrade()
{
    ...
    if(marketCondition == "TRENDING" && trendStrength > 0.6)
    {
        ...
        {
            // ✅ แก้: ใช้ MARKET_REGIME_TRENDING_STRONG_DETAILED แทน marketCondition
            if(ShouldOpenTrade(ORDER_TYPE_BUY, MARKET_REGIME_TRENDING_STRONG_DETAILED))
            {
                OpenBuyOrder();
        ...
        {
            // ✅ แก้: ใช้ MARKET_REGIME_TRENDING_STRONG_DETAILED แทน marketCondition
            if(ShouldOpenTrade(ORDER_TYPE_SELL, MARKET_REGIME_TRENDING_STRONG_DETAILED))
            {
                OpenSellOrder();
    ...
    else if(marketCondition == "VOLATILE")
    {
        ...
        // ✅ ใช้ MARKET_REGIME_RANGING_VOLATILE_DETAILED
        if(volatility < 150 && ShouldTradeInVolatileMarket(MARKET_REGIME_RANGING_VOLATILE_DETAILED))
        {
            UseVolatileMarketStrategy(MARKET_REGIME_RANGING_VOLATILE_DETAILED);
        }
    }
    else
    {
        Print("Waiting for better market condition: ", marketCondition);
        // ✅ ใช้ MARKET_REGIME_NORMAL
    }
}
bool IsMarketSuitableForTrading(string condition, double spread, double volatility)
{
    // ตรวจสอบว่า Spread อยู่ในเกณฑ์ที่ยอมรับได้
...
// ฟังก์ชัน OpenBuyOrder ที่ใช้ Lot Dynamic
void OpenBuyOrder()
{
   MqlTradeRequest request;
   MqlTradeResult result;
...
// ฟังก์ชัน OpenSellOrder ที่ใช้ Lot Dynamic
void OpenSellOrder()
{
   MqlTradeRequest request;
   MqlTradeResult result;
...
{
    // ใช้ข้อมูลหลาย timeframe เพื่อยืนยัน breakout
    ...
    // ต้องการการยืนยันจากทั้งสอง timeframe
    bool isConfirmed = breakoutM15 && breakoutH1;
    
    if(isConfirmed) {
        Print("🎯 STRONG BREAKOUT CONFIRMED | M15 & H1 alignment");
...
void CheckAndHandleBreakout()
{
    ...
    int marketRegime = AnalyzeMarketCondition(); // ฟังก์ชันที่ต้องมีในโค้ด
    
    // หากมีแนวโน้ม breakout
    if(marketRegime == MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED) {
        Print("⚡ BREAKOUT POTENTIAL DETECTED - Initiating confirmation check");
        
        // เรียกใช้ IsBreakoutConfirmed ด้วยพารามิเตอร์
        bool breakoutConfirmed = IsBreakoutConfirmed(MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED);
        
        if(breakoutConfirmed) {
            // ทำการจัดการเมื่อยืนยัน breakout
            HandleConfirmedBreakout();
        } else {
            Print("⏳ Breakout not yet confirmed, continuing monitoring...");
        }
    }
}
...
bool IsBreakoutConfirmed(ENUM_MARKET_REGIME_DETAILED currentRegime)
{
    ...
    // ตรวจสอบ price action สำหรับ breakout
    double currentHigh = iHigh(_Symbol, _Period, 0);
    double currentLow = iLow(_Symbol, _Period, 0);
    double previousHigh = iHigh(_Symbol, _Period, 1);
    double previousLow = iLow(_Symbol, _Period, 1);
    ...
    long previousVolume = iVolume(_Symbol, _Period, 1);
    
    // เงื่อนไข breakout: ราคา breakout พร้อม volume สูงขึ้น
    bool priceBreakout = (currentHigh > previousHigh && currentClose > previousHigh) || 
                         (currentLow < previousLow && currentClose < previousLow);
    
    bool volumeConfirmation = (currentVolume > previousVolume * 1.2);
    
    return (priceBreakout && volumeConfirmation);
}
void RecordBreakoutEvent()
{
    string filename = "breakout_log_" + Symbol() + ".txt";
    int handle = FileOpen(filename, FILE_READ|FILE_WRITE|FILE_TXT|FILE_SHARE_READ);
    
    if(handle != INVALID_HANDLE)
    {
        FileSeek(handle, 0, SEEK_END);
        string record = TimeToString(TimeCurrent()) + 
                       " - Breakout detected on " + Symbol() + 
                       " at price: " + DoubleToString(SymbolInfoDouble(Symbol(), SYMBOL_BID), Digits());
        FileWrite(handle, record);
        FileClose(handle);
    }
}

// 3. ฟังก์ชันจัดการ confirmed breakout
void HandleConfirmedBreakout()
{
    Print("✅ CONFIRMED BREAKOUT - Executing breakout strategy");
    
    // เรียกใช้ฟังก์ชัน breakout ที่มีอยู่
    ExecuteAdvancedBreakout(MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED);
    
    // หรือใช้กลยุทธ์เฉพาะ breakout
    ExecuteBreakoutStrategy();
    
    // บันทึกลงไฟล์หรือ log
    RecordBreakoutEvent();
}
bool IsEconomicNewsTime()
{
    // ตรวจสอบช่วงข่าวเศรษฐกิจ (ตัวอย่าง)
    MqlDateTime time;
    TimeCurrent(time);
    
    // ตัวอย่าง: หลีกเลี่ยงช่วง NFP (วันศุกร์แรกของเดือน 13:30 UTC)
    if(time.day_of_week == 5 && time.hour == 13 && time.min >= 25 && time.min <= 45)
    {
        // ตรวจสอบว่าเป็นวันศุกร์แรกของเดือนไหม
...
{
    // ใช้ฟังก์ชันวิเคราะห์ตลาดที่มีอยู่
    string marketCondition = AnalyzeMarket();
    double trendStrength = GetTrendStrength();
    double volatility = GetVolatility();
    ...
    // เงื่อนไขการเทรดตามสภาพตลาด
    if(marketCondition == "TRENDING" && trendStrength > 0.7)
    {
        // ตลาดมีเทรนด์ชัด - เทรดตามเทรนด์
        if(IsUptrend() && CountBuy < MaxTrade)
        {
    ...
    {
        // ตลาดผันผวน - ลดการเปิดออร์เดอร์หรือใช้ Hedge
    ...
    {
        // สภาพตลาดไม่แน่นอน - รอสัญญาณชัดเจน
...
bool IsTooCloseToPreviousOrders(string tradeType)
{
    ...
    for(int i = 0; i < PositionsTotal(); i++)
    {
        ...
        if(PositionGetString(POSITION_SYMBOL) == _Symbol)
        {
            ...
            // นับออร์เดอร์ที่อยู่ใกล้เกินไป
            if(distance < MinTradeDistance)
            {
                nearbyOrders++;
            }
            
            // ถ้ามีออร์เดอร์ในระยะที่ใกล้มากๆ
    ...
    // ถ้ามีออร์เดอร์ใกล้ๆ มากกว่า 2 ออร์เดอร์
    if(nearbyOrders >= 2)
    {
...
bool ShouldOpenTrade(ENUM_ORDER_TYPE type, ENUM_MARKET_REGIME_DETAILED regime) 
{
    // ===== โค้ดพื้นฐาน =====
    ...
    // ตรวจสอบข่าวเศรษฐกิจ (ถ้ามีฟังก์ชันนี้)
    ...
    // ตรวจสอบเวลาที่เหมาะสม (ถ้ามีฟังก์ชันนี้)
    ...
    // ตรวจสอบจำนวนออร์เดอร์สูงสุด (ปรับตาม regime)
    int maxPositions = GetMaxPositionsForRegime(ENUM_MARKET_REGIME_DETAILED regime);
    if(PositionsTotal() >= maxPositions) 
    {
        Print("Maximum positions reached for ", EnumToString(ENUM_MARKET_REGIME_DETAILED regime));
        return false;
    }
    
    // ===== เงื่อนไขใหม่ตาม Regime =====
    
    // ตรวจสอบ regime-specific conditions
    switch(regime) 
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            // ในตลาดผันผวน ให้ตรวจสอบเพิ่ม
        ...
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            // ในเทรนด์แรง ต้องเทรดตามเทรนด์เท่านั้น
        ...
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            // ใน sideways เงียบ ตรวจสอบ range boundaries
        ...
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            // เทรนด์อ่อนต้องการ confirmation มากขึ้น
    ...
    // ===== ตรวจสอบระยะห่างจากออร์เดอร์ล่าสุด =====
    // ถ้ามีฟังก์ชัน IsTooCloseToLastTrade
    
    return true;
}
// Helper functions
// ฟังก์ชันตรวจสอบสภาพตลาดปกติ
...
int GetMaxPositionsForRegime(ENUM_MARKET_REGIME_DETAILED regime)
{
    switch(regime)
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return 3;  // น้อยลงในตลาดผันผวน
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return 4;  // ปานกลางในเทรนด์แรง
            
        case MARKET_REGIME_BREAKOUT_POTENTIAL_DETAILED:
            return 2;  // น้อยมากใน breakout
            
        case MARKET_REGIME_RANGING_CALM_DETAILED:
            return 5;  // มากใน sideways เงียบ
            
        case MARKET_REGIME_TRENDING_WEAK_DETAILED:
            return 4;  // ปานกลางในเทรนด์อ่อน
            
        case MARKET_REGIME_NORMAL:
        default:
            return MaxTotalPositions; // ค่าจาก input parameter
    }
}
...
// ฟังก์ชันตรวจสอบว่าเทรดตามเทรนด์หรือไม่
...
// ฟังก์ชันหาทิศทางเทรนด์
int GetTrendDirection()
{
    double uptrendStrength = GetTrendStrength(DIRECTION_UP);
    double downtrendStrength = GetTrendStrength(DIRECTION_DOWN);
    
    if(uptrendStrength > downtrendStrength) return DIRECTION_UP;
    else if(downtrendStrength > uptrendStrength) return DIRECTION_DOWN;
    else return DIRECTION_NEUTRAL;
}

// ฟังก์ชันตรวจสอบ breakout confirmation
bool IsConfirmedBreakout(ENUM_ORDER_TYPE orderType = ORDER_TYPE_BUY)
{
    // ตรวจสอบว่าราคาผ่าน key level และยืนเหนือ/ใต้ได้
    double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    
    // ใช้ iHigh และ iLow แทนฟังก์ชันที่อาจยังไม่มี
    double previousHigh = iHigh(_Symbol, PERIOD_CURRENT, 1);  // High ของแท่งก่อนหน้า
    double previousLow = iLow(_Symbol, PERIOD_CURRENT, 1);    // Low ของแท่งก่อนหน้า
    
    // ต้องใช้ orderType ไม่ใช่ type
    if(orderType == ORDER_TYPE_BUY) 
    {
        // Breakout ขึ้น: ราคาปัจจุบันต้องสูงกว่า previous high
        bool breakoutUp = (currentPrice > previousHigh);
        
        // เพิ่มเงื่อนไข confirmation: volume และ momentum
        if(breakoutUp)
        {
            // ตรวจสอบ volume confirmation
            bool volumeOK = CheckBreakoutVolume();
            
            // ตรวจสอบ momentum ด้วย RSI หรืออื่นๆ
    ...
    {
        // Breakout ลง: ราคาปัจจุบันต้องต่ำกว่า previous low
...
double GetTrendStrengthNormalized()
{
    double uptrendStrength = GetTrendStrength(DIRECTION_UP);
    double downtrendStrength = GetTrendStrength(DIRECTION_DOWN);
    
    // หาค่าสูงสุด
    double maxStrength = MathMax(uptrendStrength, downtrendStrength);
    
    // แปลงเป็นค่า 0-1
    if(maxStrength == 30.0) return 1.0;    // Strong trend
    else if(maxStrength == 20.0) return 0.5; // Moderate trend
    else return 0.0;                        // Weak or no trend
}
string GetTrendStatusString()
{
    int direction = GetTrendDirection();
    double strength = 0;
    
    if(direction == DIRECTION_UP) {
        strength = GetTrendStrength(DIRECTION_UP);
    } else if(direction == DIRECTION_DOWN) {
        strength = GetTrendStrength(DIRECTION_DOWN);
    }
    
    if(strength == 30.0) {
        return (direction == DIRECTION_UP ? "STRONG UPTREND" : "STRONG DOWNTREND");
    } else if(strength == 20.0) {
        return (direction == DIRECTION_UP ? "MODERATE UPTREND" : "MODERATE DOWNTREND");
    } else {
        return "NO CLEAR TREND";
    }
}

bool IsMarketGoodForTrading(string symbol, ENUM_MARKET_REGIME_DETAILED regime)
{
    // ตรวจสอบพื้นฐาน
    ...
    {
        case MARKET_REGIME_RANGING_VOLATILE_DETAILED:
            return (volatility < 0.02); // ไม่ผันผวนเกินไป
            
        case MARKET_REGIME_TRENDING_STRONG_DETAILED:
            return true; // เทรนด์แรงเทรดได้ปกติ
...
{
    // ✅ ใช้ PositionsTotal() แทน (built-in ของ MQL5)
...
bool ShouldOpenNewTradeInRange()
{
    ...
    // ถ้ากำไร: เปิดออร์เดอร์ได้ตามปกติ
    if(profit > 0) return true;
    
    // ถ้าขาดทุนน้อย: เปิดได้แต่ลดล็อต
    if(profit >= -20) return true;
    
    // ถ้าขาดทุนปานกลาง: เปิดได้เฉพาะโอกาสดีจริงๆ
    if(profit >= -50 && riskPressure < 0.3) return true;
    
    // ถ้าขาดทุนมาก: ไม่เปิดออร์เดอร์ใหม่
    return false;
}
// 🔥 คำนวณล็อตแบบง่ายๆ
// 🔥 ใช้ฟังก์ชันคำนวณล็อตที่มีอยู่ในระบบคุณ
double CalculateSimpleRecoveryLot(double currentProfit, string marketCondition)
{
    // ✅ ใช้ฟังก์ชันคำนวณล็อตที่มีอยู่จริง
    double baseLot = CalculateSimpleLot();  // ใช้ฟังก์ชันที่มีอยู่
    
    // ใช้ RecoveryTarget ในการคำนวณปรับล็อต
    if(currentProfit < -RecoveryTarget * 2) {
        return baseLot * 1.5; // ขาดทุนหนัก → เพิ่มล็อต
    }
    else if(currentProfit < -RecoveryTarget) {
        return baseLot * 1.2; // ขาดทุนปานกลาง → เพิ่มล็อตเล็กน้อย
    }
    else if(currentProfit > RecoveryTarget) {
        return baseLot * 0.8; // กำไรดี → ลดล็อต
    }
    
    return baseLot; // สภาพปกติ
}

// 🔥 กำหนดโหมดการเทรดแบบง่ายๆ
string GetSimpleTradingMode(double profit)
{
    // ใช้ RecoveryTarget ในการกำหนดโหมด
...
void UseRangeStrategy(int marketRegime = -1)
{
    ...
    // 🔥 แปลงตัวเลข regime เป็นสตริง
    if(marketRegime == MARKET_REGIME_RANGING_CALM_DETAILED) {
        marketCondition = "MARKET_REGIME_RANGING_CALM_DETAILED";
        Print("📊 Using CALM RANGING strategy (Regime: ", marketRegime, ")");
    }
    else if(marketRegime == MARKET_REGIME_RANGING_VOLATILE_DETAILED) {
        marketCondition = "MARKET_REGIME_RANGING_VOLATILE_DETAILED";
        Print("📊 Using VOLATILE RANGING strategy (Regime: ", marketRegime, ")");
    }
    else if(marketRegime == MARKET_REGIME_TRENDING) {
        marketCondition = "MARKET_REGIME_TRENDING";
        Print("📊 Using TRENDING strategy (Regime: ", marketRegime, ")");
    }
    else {
        // ใช้การวิเคราะห์ภายในถ้าไม่ได้รับค่าที่ถูกต้อง
        marketCondition = AnalyzeMarket();
        Print("📊 Using auto-detected market: ", marketCondition);
    }
    
    bool isCalmRanging = (marketCondition == "MARKET_REGIME_RANGING_CALM_DETAILED");
    int rangeBuffer = isCalmRanging ? 8 : 5; // ขยาย buffer สำหรับตลาดเงียบ
    double calmLotMultiplier = isCalmRanging ? 0.7 : 1.0; // ลดล็อตสำหรับตลาดเงียบ
    
    int currentCountBuy = CountBuyOrders();
    int currentCountSell = CountSellOrders();
    
    // 🔥 เรียกใช้ฟังก์ชัน Cooperative ที่มีอยู่จริง
    if(EnableCooperativeSystem) {
        ManageCooperativeStrategies();   // ✅ มีอยู่จริง
        DisplayCooperativeDashboard();   // ✅ มีอยู่จริง
    }
    
    // 🔥 ใช้ RecoveryTarget เดียวกันทั้งระบบ
    double teamProfitTarget = RecoveryTarget;  // ใช้ตัวแปรเดียวกัน
    bool teamAllowsNewTrades = EnableCooperativeSystem;
    
    // 🔥 คำนวณล็อต (ปรับตามเงื่อนไขตลาด)
    double recoveryLot = CalculateSimpleRecoveryLot(totalProfit, marketCondition);
    recoveryLot *= calmLotMultiplier; // ปรับล็อตสำหรับตลาดเงียบ
    ...
    // 🔥 เพิ่มเงื่อนไขพิเศษสำหรับตลาดเงียบ
    bool allowCalmTrading = isCalmRanging ? (totalProfit > -50) : true; // อนุญาตเทรดเฉพาะเมื่อขาดทุนไม่มากในตลาดเงียบ
    ...
    if((currentPrice >= rangeHigh - (rangeBuffer * _Point)) && canOpenSell && allowCalmTrading)
    {
        if(ShouldOpenTrade("SELL", marketCondition))
        {
            ...
            // 🔥 รายงานไปยังระบบ Cooperative (ถ้าเปิดใช้งาน)
            if(EnableCooperativeSystem) {
                ExecuteRecoveryCoordination();  // ✅ มีอยู่จริง
            }
            
            Print("🎯 COOP SELL | Regime: ", marketCondition, " | Mode: ", tradingMode);
    ...
    if((currentPrice <= rangeLow + (rangeBuffer * _Point)) && canOpenBuy && allowCalmTrading)
    {
        if(ShouldOpenTrade("BUY", marketCondition))
        {
            ...
            Print("🎯 COOP BUY | Regime: ", marketCondition, " | Mode: ", tradingMode);
            Print("Lot: ", finalLot, " | Target: ", RecoveryTarget, " | Price: ", currentPrice);
        }
    }
    
    // 🔥 ปรับปรุงการแสดงผลเมื่อไม่ได้เปิดออเดอร์
    if(!(currentPrice >= rangeHigh - (rangeBuffer * _Point) || currentPrice <= rangeLow + (rangeBuffer * _Point))) {
        Print("⏸️ Waiting for entry signal | Price: ", currentPrice, 
...
void OpenBuyOrderWithLot(double lotSize)
{
   ...
   // ✅ ตรวจสอบผลลัพธ์ของ OrderSend
   if(OrderSend(request, result))
   {
      Print("✅ BUY Order opened successfully | Lot: ", lotSize, " | Ticket: ", result.order);
   }
   else
   {
      Print("❌ BUY Order failed | Error: ", GetLastError(), " | Lot: ", lotSize);
   }
}
...
void OpenSellOrderWithLot(double lotSize)
{
   ...
   if(OrderSend(request, result))
   {
      Print("✅ SELL Order opened successfully | Lot: ", lotSize, " | Ticket: ", result.order);
   }
   else
   {
      Print("❌ SELL Order failed | Error: ", GetLastError(), " | Lot: ", lotSize);
   }
}
// 🔥 สร้างฟังก์ชันนับออร์เดอร์ใหม่
int CountBuyOrders(int specificMagic = -1)
{
    int count = 0;
...
int CountSellOrders(int specificMagic = -1)
{
    int count = 0;
...
// 🔥 เพิ่มฟังก์ชันที่ UseRangeStrategy() เรียกใช้
double GetRangeHigh()
{
    double highBuffer[];
    ArraySetAsSeries(highBuffer, true);
    CopyHigh(_Symbol, _Period, 0, 20, highBuffer); // ใช้ 20 แท่งย้อนหลัง
...
double GetRangeLow()
{
    double lowBuffer[];
    ArraySetAsSeries(lowBuffer, true);
    CopyLow(_Symbol, _Period, 0, 20, lowBuffer); // ใช้ 20 แท่งย้อนหลัง
    
    return lowBuffer[ArrayMinimum(lowBuffer, 0, 20)];
}
// เปลี่ยนฟังก์ชันให้รับพารามิเตอร์
void UseVolatileMarketStrategy(ENUM_MARKET_REGIME_DETAILED currentRegime = MARKET_REGIME_UNKNOWN_DETAILED)
{
    Print("Using volatile market strategy for regime: " + RegimeToString(currentRegime));
    
    // ตรวจสอบว่าเป็น volatile market จริงหรือไม่
    if(currentRegime != MARKET_REGIME_RANGING_VOLATILE_DETAILED && 
       currentRegime != MARKET_REGIME_HIGH_VOLATILITY_DETAILED)
    {
        Print("Not a volatile market regime");
        return;
    }
    
    // ใช้กลยุทธ์ระมัดระวังในตลาดผันผวน
    if(CountBuy < (MaxTrade / 2) && IsUptrend() && CalculateTotalProfit() > -20)
    {
        OpenBuyOrder(); // ใช้ฟังก์ชันเปิดออร์เดอร์ปกติ
        Print("Cautious BUY in volatile market");
    }
    else if(CountSell < (MaxTrade / 2) && IsDowntrend() && CalculateTotalProfit() > -20)
    {
        OpenSellOrder(); // ใช้ฟังก์ชันเปิดออร์เดอร์ปกติ
        Print("Cautious SELL in volatile market");
    }
    else
    {
        Print("No trade in volatile market - conditions not met");
    }
}
// ========== ADD THIS FUNCTION ==========
...
ENUM_TREND_DIRECTION GetCurrentTrendDirection()
{
    ...
    if(maFast > maSlow + threshold)
        return TREND_UP;
    else if(maFast < maSlow - threshold)
        return TREND_DOWN;
    else
        return TREND_NONE;
}// =======================================
void ExecuteTrendCounterTrade(double emergencyLot)
{
    if(emergencyLot <= 0) 
    {
        Print("❌ Invalid emergency lot size: ", emergencyLot);
        return;
    }
    ...
    Print("📈 Executing TREND Counter Trade");
    Print("   Lot Size: ", emergencyLot);
    Print("   Trend: ", TrendToString(currentTrend));
    Print("   Strength: ", trendStrength);
    
    if(currentTrend == TREND_UP && trendStrength > 5.0)
    {
        // Counter trade ใน uptrend - อาจเปิด buy หรือ sell ตามกลยุทธ์
        // ตัวเลือก A: เปิดตามเทรนด์ (buy)
        OpenCounterBuy(emergencyLot, "Trend Counter - Buy");
        
        // ตัวเลือก B: เปิด逆向 (sell) สำหรับบางกลยุทธ์
        // OpenCounterSell(emergencyLot, "Trend Counter - Sell (Contrarian)");
    }
    else if(currentTrend == TREND_DOWN && trendStrength > 5.0)
    {
        // Counter trade ใน downtrend
        OpenCounterSell(emergencyLot, "Trend Counter - Sell");
        
        // หรือเปิด逆向
        // OpenCounterBuy(emergencyLot, "Trend Counter - Buy (Contrarian)");
    }
    else
    {
        // No clear trend - wait or use range strategy
        Print("   ⚠️ No clear trend for counter trade");
        ExecuteRangeCounterTrade(emergencyLot);
    }
}
...
void OpenCounterBuy(double lot, string comment)
{
    ...
    if(OrderSend(request, result))
    {
        Print("✅ Counter BUY opened: Ticket #", result.order, 
              " Lot: ", lot, " SL: ", sl, " TP: ", tp);
    }
    else
    {
        Print("❌ Counter BUY failed: Error ", result.retcode);
    }
}
...
void OpenCounterSell(double lot, string comment)
{
    ...
    if(OrderSend(request, result))
    {
        Print("✅ Counter SELL opened: Ticket #", result.order, 
              " Lot: ", lot, " SL: ", sl, " TP: ", tp);
    }
    else
    {
        Print("❌ Counter SELL failed: Error ", result.retcode);
    }
}
void ExecuteRangeCounterTrade(double emergencyLot)
{
    Print("📊 Executing RANGE Counter Trade");
    Print("   Lot Size: ", emergencyLot);
    ...
    if(currentPrice > rangeMiddle)
    {
        // ราคาอยู่ครึ่งบนของ range -> โอกาส sell ดีกว่า
        if(currentPrice > rangeHigh - (atrValue * 0.5))
        {
            // ใกล้ขอบบน -> sell
            OpenCounterSell(emergencyLot * 0.7, "Range Counter - Sell at top");
        }
        else
        {
            // กลาง range ครึ่งบน -> อาจ wait
            Print("   ⏸️ Near range top, consider waiting");
        }
    }
    else
    {
        // ราคาอยู่ครึ่งล่างของ range -> โอกาส buy ดีกว่า
        if(currentPrice < rangeLow + (atrValue * 0.5))
        {
            // ใกล้ขอบล่าง -> buy
            OpenCounterBuy(emergencyLot * 0.7, "Range Counter - Buy at bottom");
        }
        else
        {
            // กลาง range ครึ่งล่าง -> อาจ wait
            Print("   ⏸️ Near range bottom, consider waiting");
        }
    }
}
...
void ExecuteBreakoutCounterTrade(double emergencyLot)
{
    Print("🚀 Executing BREAKOUT Counter Trade");
    Print("   Lot Size: ", emergencyLot);
    
    // ตรวจสอบ breakout direction
    bool bullishBreakout = CheckBullishBreakout();
    bool bearishBreakout = CheckBearishBreakout();
    
    if(bullishBreakout)
    {
        Print("   📈 Bullish breakout detected");
        OpenCounterBuy(emergencyLot * 1.2, "Breakout Counter - Buy"); // Larger lot for breakout
    }
    else if(bearishBreakout)
    {
        Print("   📉 Bearish breakout detected");
        OpenCounterSell(emergencyLot * 1.2, "Breakout Counter - Sell");
    }
    else
    {
        Print("   ⚠️ No clear breakout - using default counter");
        ExecuteDefaultCounterTrade(emergencyLot);
    }
}
...
bool CheckBullishBreakout()
{
    ...
    return (currentHigh > previousHigh + (atrValue * 0.3));
}
...
bool CheckBearishBreakout()
{
    ...
    return (currentLow < previousLow - (atrValue * 0.3));
}
void ExecuteDefaultCounterTrade(double emergencyLot)
{
    Print("🔄 Executing DEFAULT Counter Trade");
    Print("   Lot Size: ", emergencyLot);
    
    // Simple counter trade - opposite of worst position
    ENUM_POSITION_TYPE worstType = GetWorstPositionType();
    
    if(worstType == POSITION_TYPE_BUY)
    {
        OpenCounterSell(emergencyLot, "Default Counter - Opposite of worst BUY");
    }
    else if(worstType == POSITION_TYPE_SELL)
    {
        OpenCounterBuy(emergencyLot, "Default Counter - Opposite of worst SELL");
    }
    else
    {
        // No positions or can't determine - use trend
        ExecuteTrendCounterTrade(emergencyLot);
    }
}

ENUM_POSITION_TYPE GetWorstPositionType()
{
    ulong worstTicket = 0;
    double worstProfit = 0;
    ENUM_POSITION_TYPE worstType = WRONG_VALUE;
